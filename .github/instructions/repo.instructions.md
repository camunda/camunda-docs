...
- The **GitHub CLI** (`gh`) is the expected tool for PR and issue operations. It must be installed and authenticated (`gh auth status`) to create PRs, fetch checks, or read review comments.
...
- HTML minifier `diagnostic - error` warnings emitted by the SWC minifier (under `experimental_faster: true`) are **non-fatal**. A clean build can still log them for many paths. The only fatal failure to act on is `Docusaurus static site generation failed for N paths`.
...
### Folder to URL prefix mapping

The current released version is set in `src/versions.js` (`currentVersion`) and Docusaurus is configured with `lastVersion: currentVersion`, so the released version is served with no URL prefix. The mapping for the versions currently in the repo is:

| Repo path                     | URL prefix                                       |
| ----------------------------- | ------------------------------------------------ |
| `docs/`                       | `/docs/next/...` (the unreleased "Next" version) |
| `versioned_docs/version-8.9/` | `/docs/...` (default, no version prefix)         |
| `versioned_docs/version-8.8/` | `/docs/8.8/...`                                  |
| `versioned_docs/version-8.7/` | `/docs/8.7/...`                                  |
| `src/pages/`                  | Site root, not versioned                         |

When a new version is cut, `currentVersion` and the unprefixed version shift accordingly. Re-check `src/versions.js` before relying on this mapping.

### Navbar links resolve across every version

Navbar entries with `type: "doc"` in `docusaurus.config.js` resolve the same `docId` against **every** version, including `docs/` (Next). If the `docId` is missing from any one of `docs/`, `versioned_docs/version-8.9/`, `versioned_docs/version-8.8/`, or `versioned_docs/version-8.7/`, the build fails with `Docusaurus static site generation failed for ~N paths` and no underlying stack trace. When adding or renaming a navbar doc link, confirm the `docId` exists in every version.

### Doc IDs can diverge between versions

The same conceptual page can have different filenames (and therefore different doc IDs) across versions — for example, an "overview" page may have been renamed when a section was restructured. Verify the target file name in each version before back- or forward-porting a link; do not assume the path from one version exists in another.
