...
- Edit only the files named in the task, plus any sibling files explicitly required by repo conventions (for example, the corresponding versioned file when editing `/docs/` — see `repo.instructions.md` §6).
...
- Run the full docs build (`npm run build`) only after final changes, and only when the build trigger conditions in `repo.instructions.md` §5 apply or the user explicitly asks for a full build.
