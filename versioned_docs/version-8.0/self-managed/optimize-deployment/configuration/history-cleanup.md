...
The age of ingested event data is determined by the [`time`](../../../apis-clients/optimize-api/event-ingestion.md#request-body) field provided for each event at the time of ingestion.
...
:::note
The ingested event cleanup does not cascade down to potentially existing [event-based processes]($optimize$/components/userguide/additional-features/event-based-processes) that may contain data originating from ingested events. To make sure data of ingested events is also removed from event-based processes, you need to enable the [Process Data Cleanup](#process-data-cleanup) as well.
:::
