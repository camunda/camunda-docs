...
2. **Local filesystem (Camunda Platform 8 or newer):** You can fetch the JSON templates from [here](https://docs.camunda.io/docs/next/self-managed/connectors-deployment/install-and-start/#connector-templates). Store element templates as `.json` file in the `resources/element-templates` folder, relative to the modelers executable _or_ relative to the modelers data directory ([see below](#example-setup)). Alternatively, they can be stored in a `.camunda/element-templates` directory that resides, relative to the currently opened diagram, anywhere in the diagrams path hierarchy.
