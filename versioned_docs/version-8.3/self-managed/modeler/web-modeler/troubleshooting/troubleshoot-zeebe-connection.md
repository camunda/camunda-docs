...
To resolve this issue, check if you can connect to Zeebe through another client, i.e., [`zbctl`](/versioned_docs/version-8.3/apis-tools/cli-client/index.md).
If that doesn't work, resolve the general connection issue first (see [the platform deployment troubleshooting section](/versioned_docs/version-8.3/self-managed/platform-deployment/troubleshooting.md), for example.)
