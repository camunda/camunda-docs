...
This is implemented by a [sequence flow](/components/modeler/bpmn/bpmn-primer/#sequence-flow-controlling-the-flow-of-execution) connecting two activities:
...
You can read more about it in [our BPMN primer: sequence flows - controlling the flow of execution](/components/modeler/bpmn/bpmn-primer/#sequence-flow-controlling-the-flow-of-execution).
...
This is implemented by an [exclusive gateway (XOR)](/components/modeler/bpmn/exclusive-gateways/):
...
You can read more about it in [our BPMN primer: gateways - steering flow](/components/modeler/bpmn/bpmn-primer/#gateways-steering-flow).

### Invoke subworkflows

You need to invoke another process as part of your process.

This is implemented by a [call activity](/components/modeler/bpmn/call-activities/):
...
In BPMN, this is implemented using [parallel gateways (AND)](/components/modeler/bpmn/parallel-gateways/):
...
In BPMN, this is implemented using [multiple instance activities](/components/modeler/bpmn/multi-instance/):
...
In BPMN, this is implemented using [events](/components/modeler/bpmn/events/) (or [receive tasks](/components/modeler/bpmn/receive-tasks/)):

<div bpmn="workflow-patterns/wait.bpmn" callouts="timer, message" />

<span className="callout">1</span>

The timer event causes the process to wait, in this case until a specific point in time is due or some duration has elapsed. Refer to [timer events](/components/modeler/bpmn/timer-events/) for more details.

<span className="callout">2</span>

The process will wait for a message to arrive. The message is an external trigger provided by API and can technically be anything, from a callback (e.g. via REST), over real messaging (like AMQP), or to notifications within your system. Refer to [message events](/components/modeler/bpmn/message-events/) for more details.
...
In BPMN, you can leverage [boundary events](/components/modeler/bpmn/events/#boundary-events) or [event subprocesses](/components/modeler/bpmn/event-subprocesses/).
...
:::note
The above process is not necessarily modeled following all of our [modeling best practices](/components/best-practices/modeling/creating-readable-process-models/), but intentionally shows different ways to use BPMN to implement certain workflow patterns.
:::
...
As with timers, you can leverage [boundary events](/components/modeler/bpmn/events/#boundary-events) or [event subprocesses](/components/modeler/bpmn/event-subprocesses/).

<div bpmn="workflow-patterns/reactive-message-events.bpmn" callouts="subprocess1, event1, event2, event3" />

Assume that an order cancelation message comes in for the current process instance using [message correlation](/components/concepts/messages/).
...
Mapping external messages to an existing process instance is called [message correlation](/components/concepts/messages/). This is a crucial functionality to ensure you can communicate with process instances from the outside.
...
You can find more information in [our documentation about messages](/components/concepts/messages/).

### Events from subprocesses

Sometimes, a subprocess needs to communicate with its parent process without ending the subprocess yet. BPMN allows this by an [escalation event](/components/modeler/bpmn/bpmn-coverage/).
...
You might also want to look into our [best practice: modeling beyond the happy path](/components/best-practices/modeling/modeling-beyond-the-happy-path/) to understand possibilities.

### Error scopes

The reaction to errors might need to be different depending on the current state of the process. This can be achieved by using [subprocesses](/components/modeler/bpmn/embedded-subprocesses/) in combination with either [boundary events](/components/modeler/bpmn/events/#boundary-events) or [event subprocesses](/components/modeler/bpmn/event-subprocesses/).
...
You might need to react to different event types differently, which is possible by using the [error type](/components/modeler/bpmn/error-events/#defining-the-error) known to BPMN:
...
In BPMN, you can use [compensation events](/components/modeler/bpmn/bpmn-coverage/) to easily implement compensations in your processes.
