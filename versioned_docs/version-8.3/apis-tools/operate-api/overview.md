...
The query API of the Camunda 8 API v.2 (available as an alpha feature starting from Camunda 8.6 and to be released in Camunda 8.8) returns `id` and `key` values as strings, so this limitation does not apply.
:::
