...
2. Configure script behavior using the following environment variables:
   ...
   - `CAMUNDA_DISTRO_USER`: Camunda Enterprise account username (Harbor registry credentials) for authenticating against `artifacts.camunda.com` (Artifactory). Required to download Camunda artifacts.
   - `CAMUNDA_DISTRO_PASSWORD`: Camunda Enterprise account password (Harbor registry credentials) for authenticating against `artifacts.camunda.com` (Artifactory). Required to download Camunda artifacts.
