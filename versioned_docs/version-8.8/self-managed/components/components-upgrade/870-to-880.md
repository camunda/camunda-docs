...
To ease the effort for migrating data from [Management Identity](../management-identity/overview.md) to the new [Orchestration Cluster Identity](/self-managed/components/orchestration-cluster/identity/initial-setup.md), a Identity Migration Application is provided.
...
When you enable the Identity migration, the helm charts automatically configure the required `migration` client for you.

**Manual configuration** (when needed):
If you manually configure Keycloak clients in your `values.yaml`, you must also define the migration client:
...
:::note Client indexing
Adjust the client index number (`2` in this example) based on your existing client configuration. Each client requires a unique index.
:::
