...
| archiveByIdEnabled           | When `true`, archiving moves documents in small, targeted batches instead of attempting to move all relevant records in a single attempt. This incrementally continues until all documents for the selected process instances have been archived to the relevant dated indices. There are no changes to the core archiving concept - data is still moved to the same dated destination indices. When archiving by ID, archiving is executed in a less resource-intensive manner, which improves stability. Configure using `ZEEBE_BROKER_EXPORTERS_CAMUNDAEXPORTER_ARGS_HISTORY_ARCHIVEBYIDENABLED`.                         | `true`  |
| reindexBatchSize             | Number of individual Elasticsearch/OpenSearch documents archived in each targeted batch when `archiveByIdEnabled` is `true`. Configure using `ZEEBE_BROKER_EXPORTERS_CAMUNDAEXPORTER_ARGS_HISTORY_REINDEXBATCHSIZE`.                                                                                                                                                                                                                                                                                                                                                                                                         | `2500`  |
