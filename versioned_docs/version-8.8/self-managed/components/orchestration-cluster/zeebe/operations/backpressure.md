...
Alternatively, [flow control write rate limits](/self-managed/operational-guides/configure-flow-control/configure-flow-control.md) can also be used with static write rate limits or throttling. This prevents the
...
The default values can be found in the [Zeebe Broker standalone configuration template](https://github.com/camunda/camunda/blob/8.8.0/dist/src/main/config/broker.standalone.yaml.template) or in the [Zeebe Broker configuration template](https://github.com/camunda/camunda/blob/8.8.0/dist/src/main/config/broker.yaml.template) in the `# backpressure` section.
