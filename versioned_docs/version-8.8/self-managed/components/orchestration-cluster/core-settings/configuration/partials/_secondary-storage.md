...
:::note
Retention settings must be configured in **two** places, with **matching values**: `camunda.database.retention.*` (or the equivalent `CAMUNDA_DATABASE_RETENTION_*` environment variable) applies the retention/ILM policy to Orchestration Cluster indices, while `zeebe.broker.exporters.camundaexporter.args.history.retention.*` (or the equivalent `ZEEBE_BROKER_EXPORTERS_CAMUNDAEXPORTER_ARGS_HISTORY_RETENTION_*` environment variable) applies the same policy for the Camunda Exporter. If the two are not kept in sync, data may be deleted earlier or later than expected.
:::
...
| `CAMUNDA_DATABASE_RETENTION_ENABLED`<br/><br/>`ZEEBE_BROKER_EXPORTERS_CAMUNDAEXPORTER_ARGS_HISTORY_RETENTION_ENABLED`                               | Enables creation and application of retention/ILM policies. Both variables must be set to the same value.    | `false`                                  |
| `CAMUNDA_DATABASE_RETENTION_MINIMUMAGE`<br/><br/>`ZEEBE_BROKER_EXPORTERS_CAMUNDAEXPORTER_ARGS_HISTORY_RETENTION_MINIMUMAGE`                         | Minimum age before data is eligible for deletion. Both variables must be set to the same value.              | `30d`                                    |
| `CAMUNDA_DATABASE_RETENTION_POLICYNAME`<br/><br/>`ZEEBE_BROKER_EXPORTERS_CAMUNDAEXPORTER_ARGS_HISTORY_RETENTION_POLICYNAME`                         | Name of the retention policy applied to standard indices. Both variables must be set to the same value.      | `camunda-retention-policy`               |
| `CAMUNDA_DATABASE_RETENTION_USAGEMETRICSMINIMUMAGE`<br/><br/>`ZEEBE_BROKER_EXPORTERS_CAMUNDAEXPORTER_ARGS_HISTORY_RETENTION_USAGEMETRICSMINIMUMAGE` | Minimum age before usage metrics indices are deleted. Both variables must be set to the same value.          | `730d`                                   |
| `CAMUNDA_DATABASE_RETENTION_USAGEMETRICSPOLICYNAME`<br/><br/>`ZEEBE_BROKER_EXPORTERS_CAMUNDAEXPORTER_ARGS_HISTORY_RETENTION_USAGEMETRICSPOLICYNAME` | Name of the retention policy applied to usage metrics indices. Both variables must be set to the same value. | `camunda-usage-metrics-retention-policy` |
...
| `camunda.database.retention.enabled`<br/><br/>`zeebe.broker.exporters.camundaexporter.args.history.retention.enabled`                                  | Enables creation and application of retention/ILM policies. Both properties must be set to the same value.    | `false`                                  |
| `camunda.database.retention.minimum-age`<br/><br/>`zeebe.broker.exporters.camundaexporter.args.history.retention.minimumAge`                           | Minimum age before data is eligible for deletion. Both properties must be set to the same value.              | `30d`                                    |
| `camunda.database.retention.policy-name`<br/><br/>`zeebe.broker.exporters.camundaexporter.args.history.retention.policyName`                           | Name of the retention policy applied to standard indices. Both properties must be set to the same value.      | `camunda-retention-policy`               |
| `camunda.database.retention.usage-metrics-minimum-age`<br/><br/>`zeebe.broker.exporters.camundaexporter.args.history.retention.usageMetricsMinimumAge` | Minimum age before usage metrics indices are deleted. Both properties must be set to the same value.          | `730d`                                   |
| `camunda.database.retention.usage-metrics-policy-name`<br/><br/>`zeebe.broker.exporters.camundaexporter.args.history.retention.usageMetricsPolicyName` | Name of the retention policy applied to usage metrics indices. Both properties must be set to the same value. | `camunda-usage-metrics-retention-policy` |
