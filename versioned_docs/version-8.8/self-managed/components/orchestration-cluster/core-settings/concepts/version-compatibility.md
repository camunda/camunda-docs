...
1. **Upgrade one minor version at a time.** For example, `8.7.x → 8.8.y` is supported, but `8.6.x → 8.8.y` is not.

2. Do not skip minor versions and do not downgrade minor or major versions.

3. For best stability and coverage of fixes, **use the latest available patch** in each minor before and after a minor upgrade.
...
All minor version upgrades must proceed strictly minor-by-minor. For minor upgrades, the source minor must be at its latest patch so that schema version metadata is present for the compatibility check to succeed.
...
| Minor upgrade (single step) | Update schema | Yes                  | Source minor must be at latest patch for schema version check |
