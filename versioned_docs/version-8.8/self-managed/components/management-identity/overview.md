...
Management Identity is included in the full and standalone Web Modeler [Docker Compose configurations](/self-managed/quickstart/developer-quickstart/docker-compose.md#docker-compose-configurations) and in the default [Helm chart deployment](/self-managed/deployment/helm/install/quick-install.md). These configurations use the packaged Keycloak instance as an identity provider (IdP). The lightweight Docker Compose configuration uses the integrated Orchestration Cluster Identity and does not start Management Identity or Keycloak.
