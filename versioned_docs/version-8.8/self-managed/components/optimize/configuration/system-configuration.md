...
| entity.nameMaxLength       | CAMUNDA_OPTIMIZE_ENTITY_NAME_MAX_LENGTH | 32768         | Maximum number of characters allowed in the name of a report, collection, dashboard, or alert. Longer names are rejected with a validation error when the entity is created or updated. The default matches the orchestration cluster's equivalent limit for name fields. Note that a name well below this limit can still be too large to index, as Elasticsearch and OpenSearch cap a term at 32766 bytes and a character takes up to 3 bytes in UTF-8. Lower this value to enforce a stricter naming policy. Existing entities with longer names are unaffected and remain readable. |
