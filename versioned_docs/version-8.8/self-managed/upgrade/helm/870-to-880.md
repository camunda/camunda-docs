...
| :--------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Confirm `8.7.x` baseline                       | Confirm your deployment is running an `8.7.x` version before upgrading to 8.8. If you are upgrading from a version earlier than 8.7, see [Upgrading from an earlier version](/self-managed/upgrade/index.md#upgrading-from-an-earlier-version). Upgrading to the latest available `8.7.x` patch first is strongly recommended for fix coverage. |
...
### Zeebe data path and custom mounts

In Camunda 8.8, the Zeebe data volume mounts at `/usr/local/camunda/data` instead of `/usr/local/zeebe/data`. A Helm upgrade changes the chart-managed mount configuration, but doesn't copy or move files between directories. Existing persistent volume claim data is reused through the new mount path.

If you configured custom `initContainers`, `extraVolumeMounts`, or restore scripts, update them and verify that they all use `/usr/local/camunda/data`.
...
1. Confirm the Zeebe data mount path:
   - Inspect the rendered StatefulSet and verify the data mount path for the main Orchestration Cluster container and every `initContainer`.
   - Confirm any restore scripts write to the same path as the broker.

   See [verify the Zeebe data mount path](../../../self-managed/operational-guides/backup-restore/restore.md#restore-zeebe-cluster) for an example command and expected output.
