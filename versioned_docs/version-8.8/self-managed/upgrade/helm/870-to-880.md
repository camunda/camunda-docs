...
| :--------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Confirm `8.7.x` baseline                       | Confirm your deployment is running an `8.7.x` version before upgrading to 8.8. If you are upgrading from a version earlier than 8.7, see [Upgrading from an earlier version](/self-managed/upgrade/index.md#upgrading-from-an-earlier-version). For best stability and fix coverage, use the latest available `8.7.x` patch. |
| Confirm upgrade eligibility                    | Review the [prepare for upgrade](/self-managed/upgrade/prepare-for-upgrade.md) guide to confirm upgrade eligibility and complete any required pre-upgrade actions for 8.8.                                                                                                                                                   |
| Confirm Helm chart support                     | <p>Review the Camunda Helm chart [version matrix](https://helm.camunda.io/camunda-platform/version-matrix/) and confirm:</p><p><ul><li>The Helm chart version deploying 8.8 is supported for your Kubernetes version.</li><li>Your Helm client version is supported for that chart.</li></ul></p>                            |
...
| Check your `values.yaml`                       | Review your existing `values.yaml` for custom configuration you must migrate to `orchestration.*` in 8.8 (for example, Ingress rules, external Elasticsearch/OpenSearch connections, index prefixes, exporters, and component overrides).                                                                                    |
| Consolidate Elasticsearch/OpenSearch instances | If your environment uses more than one Elasticsearch or OpenSearch instance for exported Zeebe, Operate, and Tasklist data, consolidate to a single instance before upgrading. See [Single Elasticsearch/OpenSearch instance](/self-managed/upgrade/components/870-to-880.md#single-elasticsearchopensearch-instance).       |
| Prefix changes                                 | If you use custom index prefixes, review and apply the required prefix changes before upgrading. See [Update any custom prefixes](/self-managed/upgrade/components/870-to-880.md#update-any-custom-prefixes).                                                                                                                |
...
Start from a fresh 8.8 values file and then bring forward only the customizations you still need from 8.7. This is usually faster and less error-prone than trying to convert an 8.7 file line by line.

1. Download the default values for the Camunda 8.8 Helm chart directly into your new `values-8.8.yaml` file:

   ```bash
   helm repo update
   helm show values camunda/camunda-platform --version <CHART_VERSION> > values-8.8.yaml
   ```
...
1. Export the effective values from your current 8.7 release so you can compare and copy over only the settings you intentionally customized:

```bash
helm get values <RELEASE_NAME> -n <NAMESPACE> -a > values-8.7-effective.yaml
```

If you do not have access to the running release values, use your existing 8.7 `values.yaml` as the comparison file instead.

1. Compare `values-8.7-effective.yaml` with `values-8.8.yaml` to identify:
...
1. If you prefer to work from a finished 8.8 example instead of piecing together individual snippets, jump to [complete 8.8 values file examples](#example-upgrade-valuesyaml-from-87-to-88).
...
If you are moving away from Keycloak-centric authentication during the upgrade, prefer the `OIDC` scenario below and align Management Identity with the same external provider where possible.
...
If you want a finished 8.8 baseline instead of assembling your file only from the mapping tables above, start here and then tailor the example to your environment.
...
- Migration jobs include the following log lines:
  - Identity: "Completed \<ClassName\>"
  - Process: "Process Migration completed"
  - Task: "Task Migration completed successfully" or "Task Migration is not needed"
- Importer logs show batches of records being imported, and do not contain errors after import has started. It is expected for the importer to continue running after all migration jobs have completed, but after the migration jobs are completed, it is safe to scale down the importer.
