...
- **Optimize startup migration**  
  Optimize performs a startup data migration that can increase startup time. Plan a maintenance window if Optimize is enabled.

- **Identity HTTP port change**  
  Identity now listens on port `8084` instead of `8080`. Update network policies, firewall rules, and Ingress configurations if needed.
...
<TabItem value="after" label="8.8 values.yaml (after)">
...
To complete the upgrade, migrate plaintext values to Kubernetes secrets and reference them from your `values.yaml` as described in the secret management guide.
