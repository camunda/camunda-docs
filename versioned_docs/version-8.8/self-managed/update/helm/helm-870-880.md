...
Upgrade from the latest patch of your current Camunda release to the latest patch of the next Camunda release.

For example:

- If you are running Camunda `8.7.x`, upgrade to the latest `8.7.x` patch first.
- Then upgrade to the latest `8.8.x` release.

Do not skip releases (for example, do not upgrade directly from 8.6 to 8.8).
...
- Confirm your deployment is running the latest `8.7.x` patch.
...
- Review your existing `values.yaml` for custom configuration such as Ingress rules, external Elasticsearch/OpenSearch connections, custom index prefixes, exporters, and any `operate`, `tasklist`, or `zeebeGateway` overrides. You will migrate these settings to `orchestration.*` in 8.8.
  - If you use more than one Elasticsearch/OpenSearch instance, see [Single Elasticsearch/OpenSearch instance](/self-managed/update/components/870-to-880.md#single-elasticsearchopensearch-instance).
  - If you use custom index prefixes, see [Update any custom prefixes](/self-managed/update/components/870-to-880.md#update-any-custom-prefixes).
...
- If your `values.yaml` contains plaintext secrets, migrate them to Kubernetes secrets before upgrading. See
  [Extract plaintext values and reference them as Kubernetes secrets](/self-managed/deployment/helm/configure/secret-management.md#extract-plaintext-values-and-reference-them-as-kubernetes-secrets).
...
See the [sizing guidelines](../../../reference/supported-environments.md#sizing) and [benchmarking recommendations](../../../reference/supported-environments.md#sizing).
:::

## Changes to review before upgrading

Review the following changes before upgrading a Helm-managed Camunda 8 Self-Managed deployment from 8.7 to 8.8.

- **Orchestration Cluster consolidation**  
  Zeebe, Operate, Tasklist, and Identity are consolidated into a single Orchestration Cluster. Review how this affects resource usage, configuration, and endpoints.  
  See [Orchestration Cluster in What’s new in Camunda 8.8](/reference/announcements-release-notes/880/whats-new-in-88.md#orchestration-cluster).

- **Unified configuration model**  
  Several component-specific configuration keys are relocated or merged under `orchestration.*`. Review the configuration mapping before updating your `values.yaml`.  
  See [Camunda 8.8 property changes](/self-managed/components/orchestration-cluster/core-settings/configuration/configuration-mapping.md).

- **Tasklist API mode**  
  Tasklist UI supports both V1 (deprecated Tasklist API) and V2 (Orchestration Cluster API) modes. Tasklist UI in V1 API mode is available as a configuration option, which allows you to access legacy features during the transition. We recommend migrating to the V2 API.  
  See [Tasklist API versions](/components/tasklist/api-versions.md).

- **Elasticsearch/OpenSearch replica defaults (multi-node clusters)**  
  In 8.8, the default replica count for indices changes and can increase disk usage.  
  See [Multi-node cluster](/self-managed/update/components/870-to-880.md#multi-node-cluster).

- **Web Modeler cluster configuration changes**  
  If you use Web Modeler, review cluster authentication and configuration key changes for 8.8.  
  See [Web Modeler](/self-managed/update/components/870-to-880.md#web-modeler).

- **Optimize startup migration**  
  Optimize performs a startup data migration that can increase startup time. Plan a maintenance window if Optimize is enabled. See [Upgrade Optimize from 8.7 to 8.8](/self-managed/update/components/optimize/870-to-880.md).

- **Identity HTTP port change**  
  In 8.8, Identity listens on port `8084` instead of `8080`. If you expose Identity (Ingress, Service, NetworkPolicy, firewall rules), update any port mappings that reference `8080`.

For a complete list of changes in Camunda 8.8, see [What’s new in Camunda 8.8](/reference/announcements-release-notes/880/whats-new-in-88.md) and the [8.8 release notes](/reference/announcements-release-notes/880/880-release-notes.md).
...
For additional configuration details, see [Data migration](/self-managed/update/components/870-to-880.md#data-migration) in the components upgrade guide.
...
For more information about what is migrated and what you may still need to recreate after the upgrade, see [Identity authorization and permission changes](/self-managed/update/components/870-to-880.md#identity-authorization-and-permission-changes).
...
The Helm chart creates a dedicated Keycloak client with read-only access to your Identity Resource Server. If you override your Identity clients, follow the Helm chart section in [Common configuration for the Identity Migration application](/self-managed/update/components/870-to-880.md#common-configuration-for-the-identity-migration-application) to set up the required `migration` client.

For more information, see [Camunda Orchestration Cluster Identity data migration](/self-managed/update/components/870-to-880.md#camunda-orchestration-cluster-identity-data-migration).
...
If you use custom prefixes for Camunda indices, complete the [Update any custom prefixes](/self-managed/update/components/870-to-880.md#update-any-custom-prefixes) steps before upgrading.
...
For more information, see [Single Elasticsearch/OpenSearch instance](/self-managed/update/components/870-to-880.md#single-elasticsearchopensearch-instance).
