...
- Confirm your deployment is running the latest `8.7.x` patch. If you are upgrading from a version earlier than 8.7, see [Upgrading from an earlier version](/self-managed/update/overview.md#upgrading-from-an-earlier-version).
- Review [Prepare for upgrade](/self-managed/update/prepare-for-upgrade.md) to confirm upgrade eligibility, understand 8.8 changes that affect upgrades, and verify infrastructure requirements.
...
- If your environment uses more than one Elasticsearch or OpenSearch instance for exported Zeebe, Operate, and Tasklist data, consolidate to a single instance before upgrading. See [Single Elasticsearch/OpenSearch instance](/self-managed/update/components/870-to-880.md#single-elasticsearchopensearch-instance).
- If you use custom index prefixes, review and apply the required prefix changes before upgrading. See [Update any custom prefixes](/self-managed/update/components/870-to-880.md#update-any-custom-prefixes).
