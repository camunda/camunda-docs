...
- The single most effective mitigation is to **keep variables out of Optimize**: this recovers around 60% of the storage and 65% of the CPU (and, from Camunda 8.9, most of the lost throughput as well), at the cost of variable-based analytics.
...
If you see persistent import lag, raise the import throughput (see mitigations below).
:::
...
- **Disable variable import in Optimize.** [Disable variable import](/self-managed/components/optimize/configuration/variable-import.md) if variables are not needed in Optimize reports. This removes the variable cost from Optimize's analytics indices.
- **Disable or scope variable export at the exporter (from Camunda 8.9).** Dropping variables or restricting them to specific name prefixes at the Elasticsearch/OpenSearch exporter additionally recovers throughput, because the exporter write path is the bottleneck at maximum load.
