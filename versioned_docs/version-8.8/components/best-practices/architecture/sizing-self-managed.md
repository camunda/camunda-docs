...
Zeebe supports three RocksDB memory allocation strategies, configured using `ZEEBE_BROKER_EXPERIMENTAL_ROCKSDB_MEMORYALLOCATIONSTRATEGY`:

- **`PARTITION`** (shipped default for Self-Managed): Total RocksDB memory is calculated by multiplying the number of partitions on the broker by `ZEEBE_BROKER_EXPERIMENTAL_ROCKSDB_MEMORYLIMIT` (default: 512 MB).
...
| RocksDB            |  512 MB x partition count |
| OS page cache      |                    20-30% |
| **Sum**            | **x MB + 50% of max RAM** |

When using `FRACTION`, replace the RocksDB row with `memoryFraction x total memory` (10% by default) instead.
...
All Zeebe partitions currently export into the same Elasticsearch indices. Write throughput for an index scales with primary shard count, not with the number of Elasticsearch nodes, and scaling Zeebe partitions doesn't reshard indices automatically. Reassess shard count for your indices when you scale partitions. See the [Elasticsearch shards documentation](/self-managed/concepts/secondary-storage-management.md#shards).

If you are using a different backend:

- **OpenSearch:** Similar resource profile to Elasticsearch. The tables above generally apply, including the index-sharding consideration above.
