...
- **Disk type:** Use SSDs for Elasticsearch storage. Disk **latency** — not throughput — is the critical factor. HDD-backed Elasticsearch has been observed to cause flush durations of 8–10 seconds, leading to a growing export backlog, increased broker memory usage from in-flight records, and overall cluster throughput degradation of up to ~70% compared to an equivalent SSD-backed setup. Cloud providers often show similar throughput numbers for HDD and SSD, which can be misleading — the latency difference is what matters for Camunda's export pipeline. See the [slow disk chaos day experiment](https://camunda.github.io/zeebe-chaos/2026/06/19/Using-slow-disk-with-Camunda) for details.
