...
:::important
Only available since version 8.8.16
:::
...
- Mapping instructions can only change the user task implementation from a job-worker user task to a Camunda user task, but not vice versa (since 8.8.16, before that no user task implementation migration is possible)
