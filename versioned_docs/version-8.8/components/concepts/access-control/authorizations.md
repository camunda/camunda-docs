...
| `app-integrations` | Technical role for executing app integration (ex. Slack integration) calls.         | `READ_PROCESS_DEFINITION` on **Process Definition** (`*`), `CREATE_PROCESS_INSTANCE`, `READ_PROCESS_INSTANCE`, `UPDATE_PROCESS_INSTANCE` on **Process Definition** (`*`), `READ_USER_TASK`, `UPDATE_USER_TASK` on **Process Definition** (`*`) `CREATE` on **Document** |
...
- **app-integrations**: Automatically assigned to App integration clients (ex. Camunda for Microsoft Teams and Slack) in Cluster deployment
