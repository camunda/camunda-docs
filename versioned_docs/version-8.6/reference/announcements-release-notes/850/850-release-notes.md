...
:::warning
The Spring Zeebe SDK was released with some differences from the Spring-Zeebe Client it was forked from. Specifically relevant is a change in the configuration defaults, where [job streaming is disabled by default](/apis-tools/spring-zeebe-sdk/configuration.md#enable-job-streaming) for the Spring Zeebe SDK but was enabled by default in Spring-Zeebe client versions up to `8.5.3`.
:::
