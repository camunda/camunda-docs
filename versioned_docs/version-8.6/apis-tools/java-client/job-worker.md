...
The 8.6.0 Java client cannot maintain the long-lived polling connections required for job polling via the <!--- Which REST API? ---> REST API. For example, this applies when:
...
activation via the <!--- Which REST API? ---> REST API with long polling disabled.
:::
