...
As of Optimize 8.6, instances of Optimize running with Camunda 8 exclusively use the `8.x.x` versioning scheme. Instances of Optimize running with Camunda 7 exclusively use the `3.x.x` versioning scheme. This means you will only be able to update to Optimize 8.6 if you currently use Optimize 8.5/3.13 with Camunda 8. Optimize instances of versions 8.5/3.13 running with Camunda 7 cannot be updated to Optimize 8.6.

To ensure that Optimize 7 updates are not applied to Operate instances using Camunda 8, the 8.6 update runs a check against the connected database before executing, and exits the update if any Camunda 7 data is present in your setup. Specifically, it validates that there is no data present in the `license-index`, which is exclusively used in Camunda 7 environments.

Contact [Camunda support](https://camunda.com/services/support/) if you encounter issues updating to 8.6 in your Camunda Platform 8 environment.
