...
- **Recommendation**: For production environments, Camunda recommends using managed services rather than Bitnami subcharts.
  :::
...
Each Camunda Helm chart version lists its chart dependencies in the `Chart.yaml` file. For example, see the [Camunda 8.6 Chart.yaml](https://github.com/camunda/camunda-platform-helm/blob/main/charts/camunda-platform-8.6/Chart.yaml) file for a complete list of dependent charts.
:::
...
helm install camunda camunda/camunda-platform --version $HELM_CHART_VERSION \
  --values https://raw.githubusercontent.com/camunda/camunda-platform-helm/main/charts/camunda-platform-8.6/values-enterprise.yaml
```
...
All dependency versions listed in [supported environments](/docs/8.6/reference/supported-environments/#component-requirements) are supported. If you need to use the exact image versions from a specific chart release, reference the release tag instead of `main`:

```shell
helm install camunda camunda/camunda-platform --version $HELM_CHART_VERSION \
  --values https://raw.githubusercontent.com/camunda/camunda-platform-helm/camunda-platform-8.6-$HELM_CHART_VERSION/charts/camunda-platform-8.6/values-enterprise.yaml
```
