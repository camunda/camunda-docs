...
To resolve this issue, check if you can connect to Zeebe through another client/SDK or directly with the [Camunda 8 API](/apis-tools/camunda-api-rest/camunda-api-rest-overview.md). If that works, [further debug your Zeebe connection](#debug-zeebe-connection-issues). If that does not work, resolve the [general connection issue](#resolve-a-general-zeebe-connection-issue) first.

## Resolve a general Zeebe connection issue

You try to connect to Zeebe from both Desktop Modeler _and_ another client/SDK or directly with the [Camunda 8 API](/apis-tools/camunda-api-rest/camunda-api-rest-overview.md), and neither of them works. General connection failures can have a couple of reasons:
...
You can connect to Zeebe via another client/SDK or directly with the [Camunda 8 API](/apis-tools/camunda-api-rest/camunda-api-rest-overview.md). However, connecting through Desktop Modeler fails.
