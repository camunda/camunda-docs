...
When an exclusive gateway is entered, it evaluates the `conditionExpression` of each outgoing sequence flow in the order the flows are defined in the BPMN XML. The process instance takes the first sequence flow whose condition is fulfilled.
