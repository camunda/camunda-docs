...
The max throughput numbers should to be considered as peak loads and data retention configuration should be considered to define the amount of data that is kept for completed instances in your cluster. See [Camunda 8 SaaS data retention](/components/concepts/data-retention.md) for the default retention times for Zeebe, Tasklist, Operate and Optimize. If process instances are completed and older than the configured retention time of an application, the data will get removed. If a process instance is older than the configured retention time, but still active and not completed, it is fully functioning in runtime and does _not_ get removed. Up to certain limits data retention can be adjusted by Camunda on request. Consider retention time adjustments or storage capacity increases in case you plan to run more than [max PI stored in ES]/ [configured retention time].
