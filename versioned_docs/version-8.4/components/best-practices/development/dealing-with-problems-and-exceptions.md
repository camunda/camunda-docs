...
Whenever the worker has finished whatever it needs to do (like invoking the REST endpoint), it sends another call to the workflow engine, which [can be one of these three](/components/concepts/job-workers/#completing-or-failing-jobs):
...
Learn more about the usage of [error events](/components/modeler/bpmn/error-events/) in the user guide.
