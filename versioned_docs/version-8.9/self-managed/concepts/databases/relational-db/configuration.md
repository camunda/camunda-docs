...
Setting `camunda.data.secondary-storage.type: rdbms` activates the full RDBMS backend in a single step: it automatically enables the RDBMS exporter (which streams workflow data to the database) and configures the application layer (Operate, Tasklist, Identity, REST API) to use RDBMS for secondary storage.
