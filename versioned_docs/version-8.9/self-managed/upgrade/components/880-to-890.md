...
## Orchestration cluster web applications

### Frontend application URLs now require explicit application prefix (breaking)

In Camunda 8.9, Operate and Tasklist run in a single unified frontend application rather than as separate deployments. This architectural change requires explicit path prefixes to route requests to the correct handler.

- **Before 8.9**: URLs without an explicit application prefix (for example, `<base-url>/processes/<process-id>`) were automatically redirected to the correct application.
- **From 8.9**: The explicit application prefix is required (for example, `<base-url>/operate/processes/<process-id>` or `<base-url>/tasklist/tasks/<task-id>`). Prefix-less paths now return `404 Not Found`.

This is a **breaking change** for any bookmarks, external links, scripts, or applications that use prefix-less frontend URLs.
...
For the SaaS equivalent of this change, see [Frontend application URLs now require explicit application prefix](/reference/announcements-release-notes/890/890-announcements.md#frontend-path-prefix-required).
...
:::note
The name of the environment variable for the username claim has changed from `CAMUNDA_IDENTITY_USERNAMECLAIM` (version 8.8 / `webapp`) to `CAMUNDA_MODELER_OAUTH2_TOKEN_USERNAMECLAIM` (version 8.9 / `restapi`).
:::
...
| `CAMUNDA_MODELER_OAUTH2_TOKEN_USERNAMECLAIM` | `camunda.modeler.oauth2.token.username-claim`             | no        | `name`        |
