...
The embedded gateway is the default since Camunda 8.8, where the Orchestration Cluster components are bundled into a single application. `zeebe.broker.gateway.enable` defaults to `true`. Every Orchestration Cluster node then accepts client traffic on port `26500` for gRPC and port `8080` for REST, and you distribute that traffic across the nodes with a load balancer. Both the [Kubernetes reference architecture](/self-managed/reference-architecture/kubernetes.md) and the [manual reference architecture](/self-managed/reference-architecture/manual.md) run in this mode.
