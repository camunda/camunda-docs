...
| `IDENTITY_AUTH_PROVIDER_USEBACKENDAUTHURL` | Whether logout requests are sent to `IDENTITY_AUTH_PROVIDER_BACKEND_URL` (`true`) or `IDENTITY_AUTH_PROVIDER_ISSUER_URL` (`false`). Token refresh always uses `IDENTITY_AUTH_PROVIDER_BACKEND_URL`. | false                                                                                                                                                                    |
...
| `CAMUNDA_IDENTITY_ISSUER`            | `camunda.identity.issuer`               | The front-channel URL to the Identity provider, used for the login redirect. Also used for logout when `camunda.identity.use-backend-auth-url` is `false`.                                                                                                                                                                                                          | -                                                    |
...
| `CAMUNDA_IDENTITY_USEBACKENDAUTHURL` | `camunda.identity.use-backend-auth-url` | Whether logout is performed against the `issuer-backend-url` (`true`) or the `issuer` (`false`). Fetching refresh tokens always uses the `issuer-backend-url`, falling back to the `issuer` when it is not set.                                                                                                                                                     | `false`                                              |
