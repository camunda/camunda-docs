...
The `global.tls.caBundle` key and the `values-tls.yaml` overlay are available in:

- Camunda 8.8 — Helm chart 13.11.0+
- Camunda 8.9 — Helm chart 14.5.0+
- Camunda 8.10 — Helm chart 15.x+

They are not available in Camunda 8.7 (Helm chart 12.x); use the legacy per-component JKS truststore fields there instead (see the 'Legacy: per-component JKS truststore' section below).
...
The per-component JKS truststore fields below remain fully supported in chart 14.x. `global.tls.caBundle` is the recommended alternative, since it configures CA trust for all components from a single PEM bundle. Fields:
