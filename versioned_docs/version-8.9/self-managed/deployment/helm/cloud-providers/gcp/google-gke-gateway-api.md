...
Disable the chart-managed gRPC route with `orchestration.gateway.grpc.enabled: false`, then create a separate internal `Gateway` with the `gke-l7-rilb` class and your own `GRPCRoute`. To make the chart skip its Gateway resources so you can manage them yourself, set `global.gateway.external: true`, as described in [Gateway API setup](../../configure/ingress/gateway-api-setup.md#configure-the-helm-chart).
