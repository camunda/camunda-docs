...
| `.history.legacy-id-prefix`    | `string`  | Optional prefix prepended to migrated Camunda 7 historical definition IDs (process, decision, and form definitions). If not set, the default `c7-legacy-` prefix is used. Configure a different prefix only when you are sure your Camunda 8 definition IDs cannot clash with migrated IDs. The value must start with a letter or underscore and contain only letters, digits, `.`, `-`, or `_` (maximum 100 characters). A blank value is rejected; omit the property to use the default. Invalid values stop the migrator on startup, and the effective prefix is logged at startup. |
