...
You need a running SaaS cluster and an API client with the **Orchestration Cluster API** scope. If your connector uses cluster secrets, also add the **Secrets** scope and enable the [SaaS secret provider](/components/connectors/use-connectors-in-hybrid-mode.md#using-saas-secrets). Learn more about [how to create an API client](/components/console/manage-clusters/manage-api-clients.md).
