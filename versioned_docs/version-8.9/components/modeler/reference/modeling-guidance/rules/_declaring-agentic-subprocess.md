...
- Its `zeebe:taskDefinition` type starts with `io.camunda.agenticai:aiagent-job-worker:`, which is the type assigned by the AI Agent Sub-Process template. This covers custom element templates that use a different `zeebe:modelerTemplate` ID but assign the same type, for example an agent template you publish to your organization.
