...
Camunda 8.9.15 introduced a regression affecting elements with two or more input mappings that write to different nested fields of the same parent variable. Only one of the mapped fields ends up with the expected value — the other is silently set to `null`, with no warning or incident raised.
...
- **Upgrading to 8.9.16+:** the regression is reverted, so mapping individual fields of the same variable behaves correctly again. The workaround above is no longer required, but is harmless to keep.
...
- **Running 8.9.15:** combine the mappings into one, so the duration is created and read in the same expression instead of being written to a variable and read back later — for example, target `ageDays` with source `=@"P1D".days` instead of separate `age` and `ageDays` mappings.
- **Upgrading to 8.9.16+:** the regression is reverted, so chained mappings on temporal values behave correctly again.
