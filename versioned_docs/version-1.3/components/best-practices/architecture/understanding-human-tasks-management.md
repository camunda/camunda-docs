...
Then, require individual members of that group to explicitly *claim tasks* before working on them. This way, you avoid different people working on the same task at the same time. See [`claimTask`](../../../apis-tools/tasklist-api/mutations/claim-task.mdx).
...
- [Camunda Tasklist](/components/tasklist/introduction.md): The Tasklist application shipped with Camunda. This works out-of-the-box and has a low development effort. However, it is limited in terms of customizability and how much you can influence the user experience.
