...
| `.history.legacy-id-prefix`    | `string`  | Optional prefix added to the start of migrated Camunda 7 history definition IDs (process, decision, and form definitions) to avoid collisions with native Camunda 8 definition IDs. The value must start with a letter or underscore and contain only letters, digits, `.`, `-`, or `_` (maximum 100 characters). A blank value is rejected. Invalid values stop the migrator on startup, and the effective prefix is logged. Configure a different prefix only when you are sure your Camunda 8 definition IDs cannot clash with migrated IDs. Default: `c7-legacy-`. |
