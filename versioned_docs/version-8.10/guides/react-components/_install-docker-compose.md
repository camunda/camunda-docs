...
1. Download and extract the complete Docker Compose distribution by following the [install and start guide](/self-managed/quickstart/developer-quickstart/docker-compose/install-start.md).
1. From the extracted directory, run `docker compose up -d`.
1. Run `docker compose ps` and wait for the `orchestration` and `connectors` services to become healthy.
