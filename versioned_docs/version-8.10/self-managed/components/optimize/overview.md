...
Starting with Camunda 8.9, you can optionally reduce the amount of data exported for Optimize by configuring exporter-side filters (for example, by variable name, variable type, or BPMN process ID) in the Elasticsearch/OpenSearch exporters. See [Optimize export filtering](./configuration/optimize-export-filtering.md) for configuration options and examples.
