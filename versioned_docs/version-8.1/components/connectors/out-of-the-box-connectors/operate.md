...
Refer to the Operate [API documentation](/apis-tools/operate-api/index.md) for more details on the specific operations.
...
- **Filter**: Allows you to filter objects by fields, e.g. the following filter will return active process instance with key 235 if it contains incidents:
  ...
  If one of the fields doesn't match, an empty response will be returned for this request. Refer to the [Operate API documentation](/apis-tools/operate-api/index.md#filter) for more detailed information.
...
- **Pagination**: Identifier of an item from which the search should start. Copy this `sortValues` value from the previous Operate response here, or leave this field blank if you don't need pagination. See the [API reference](/apis-tools/operate-api/index.md#pagination) for details.
