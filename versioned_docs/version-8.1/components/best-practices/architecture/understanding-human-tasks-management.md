...
Then, require individual members of that group to explicitly _claim tasks_ before working on them. This way, you avoid different people working on the same task at the same time. See [`claimTask`](/docs/apis-tools/tasklist-api/mutations/claim-task/).
