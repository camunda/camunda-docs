...
- In SaaS, the Git sync payload size must not exceed 4MB. In Self-Managed, you can [configure the `MAXINMEMORYSIZE`](/self-managed/modeler/web-modeler/configuration/configuration.md#git-sync). If your sync fails because you've exceeded this limit, contact [Camunda success](https://camunda.com/services/support/) for assistance.
