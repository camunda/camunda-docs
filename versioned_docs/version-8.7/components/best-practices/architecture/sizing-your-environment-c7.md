...
The only way to get reliable figures for your project and environment is to do load testing on a close-to-production environment. We recommend doing this if in doubt. Steering the <!--- Which REST API? ---> REST API via load generator tools like JMeter is relatively easy.
