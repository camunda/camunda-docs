...
If the target upgrade also requires a data or application migration (as documented in [Camunda 8 Helm upgrade](/self-managed/setup/upgrade.md)), follow the migration sequence:
...
Supported settings (see the configuration references for [Operate](../../operate-deployment/operate-configuration/#settings-for-shards-and-replicas), [Tasklist](../../tasklist-deployment/tasklist-configuration/#settings-for-shards-and-replicas), and the [Elasticsearch exporter](../../zeebe-deployment/exporters/elasticsearch-exporter/#configuration)):
