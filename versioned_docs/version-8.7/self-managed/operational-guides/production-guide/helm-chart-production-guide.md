...
- **External Dependencies**: Provision the following external dependencies:
  ...
  - **Amazon OpenSearch**: The seconday datastore for Zeebe, the Camunda 8 process orchestration engine. For step-by-step instructions, see the [OpenSearch](/self-managed/setup/deploy/amazon/amazon-eks/eksctl.md#4-opensearch-domain) guide.
...
- Secrets should be created prior to installing the Helm chart so they can be referenced as existing secrets when installing the Helm chart. In this scenario, the secrets are auto-generated. The following can be added to both Helm values files:
  ...
  When upgrading the Helm chart, set `global.secrets.autoGenerated` to `false` when upgrading the chart. This will prevent access creadential to overwritten by the auto generation script. The same secrets data will be required on upgrade.
...
- [Network Policies](https://kubernetes.io/docs/concepts/services-networking/network-policies/) could be enabled with Camunda Helm charts if needed by your infrastructure requirements.
