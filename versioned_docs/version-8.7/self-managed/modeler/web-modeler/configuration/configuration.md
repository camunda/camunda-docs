...
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BEARER_TOKEN`       | Web Modeler sends the authenticated user's token with every request to the cluster.                                                                                                     | The cluster accepts the user token. This should be the default case in most setups.                                                                                                                         |
| `CLIENT_CREDENTIALS` | Web Modeler requests an M2M token using the client credentials flow and sends this token with every request. The client ID and client secret have to be provided by the user in the UI. | The cluster does not accept the user token (for example, if [Microsoft Entra ID is used as OIDC provider](../../../setup/guides/connect-to-an-oidc-provider/?authPlatform=microsoftEntraId#configuration)). |
| `NONE`               | Web Modeler does not send any token.                                                                                                                                                    | The cluster does not require any authentication at all.                                                                                                                                                     |
