...
Because each flattened property, plus the raw object itself, is stored as its own variable, enabling this on object-heavy processes can significantly increase Optimize's storage and CPU usage. Consider this when estimating disk usage for Optimize.
