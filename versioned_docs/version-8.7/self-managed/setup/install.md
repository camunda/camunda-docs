...
While upgrading is still possible by following our [upgrade guide](./update.md#identity-enabled), we recommend pre-creating these secrets to ease your future upgrade experience. This is also a recommended option when using CI/CD tools like ArgoCD, FluxCD, Jenkins, etc.
...
For upgrading the Camunda Helm chart from one release to another, perform a [Helm upgrade](/self-managed/setup/update.md).
