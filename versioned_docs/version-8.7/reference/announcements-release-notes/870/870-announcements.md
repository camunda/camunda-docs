...
Starting with Connectors 8.7.25, the connector runtime introduces the [secret filter](/self-managed/connectors-deployment/connectors-configuration.md#secret-filter), defaulting to `STRICT`. In practice, this means a secret in a connector field only resolves at runtime if that same secret was already referenced in that same field at modeling time, in the deployed BPMN.
