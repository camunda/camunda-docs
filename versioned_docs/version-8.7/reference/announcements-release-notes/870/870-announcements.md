...
- `CLIENT_CREDENTIALS`: This method was only introduced as a temporary solution for 8.7 to support deployments from Web Modeler when [Microsoft Entra ID is used as OIDC provider](/self-managed/setup/guides/connect-to-an-oidc-provider.md?authPlatform=microsoftEntraId#configuration).
  It will no longer be necessary in 8.8 as the `BEARER_TOKEN` authentication will be supported for Entra ID as well.
