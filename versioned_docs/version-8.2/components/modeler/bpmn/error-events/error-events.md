...
For error catch events, the `errorCode` can be a [static value](/components/concepts/expressions.md#expressions-vs-static-values) or it can be left empty. An expression can't be used. A
