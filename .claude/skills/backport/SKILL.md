...
- **Which versions**: check whether the feature existed in each target version. See `repo.instructions.md` §6 for the active version list and more versioning details. Do not backport to unmaintained versions; if a user explicitly requests one, explain that no `versioned_docs/version-<N>/` folder exists for it.
