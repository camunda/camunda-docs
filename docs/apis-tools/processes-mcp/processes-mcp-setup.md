...
The Processes MCP Server is enabled by default in [Camunda 8 Run](/self-managed/quickstart/developer/c8run/index.md) and [Docker Compose](/self-managed/quickstart/developer/docker-compose/index.md). For other deployment types, it must be explicitly enabled before MCP clients can connect.
