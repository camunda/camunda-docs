...
This project allows you to leverage Camunda APIs ([gRPC](/apis-tools/zeebe-api/grpc.md) and [REST](/apis-tools/camunda-api-rest/camunda-api-rest-overview.md)) in your Spring Boot project. Later on, we’ll expand the Camunda Spring Boot SDK (formerly the Spring Zeebe SDK) to deliver a Camunda Spring Boot SDK that provides a unified experience for interacting with all Camunda APIs in Java Spring.
...
See [the configuration documentation](/apis-tools/camunda-spring-boot-sdk/configuration.md) for a more in-depth discussion on parameters and configuration options for job workers.
