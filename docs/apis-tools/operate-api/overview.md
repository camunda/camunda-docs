...
The [Orchestration cluster API v.2](../orchestration-cluster-api-rest/orchestration-cluster-api-rest-overview.md) returns `id` and `key` values as strings, so this limitation does not apply.
:::

## Multi-tenancy

:::note
The [multi-tenancy feature](../../self-managed/components/orchestration-cluster/operate-deployment/operate-configuration.md#multi-tenancy) is available in Self-Managed setups only.
:::
