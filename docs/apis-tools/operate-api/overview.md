...
The [Orchestration Cluster API v.2](../camunda-api-rest/camunda-api-rest-overview.md) returns `id` and `key` values as strings, so this limitation does not apply.
:::
