...
The physical tenant ID applied to every outgoing call: sent as the `camunda-physical-tenant` gRPC header, and used to prefix the REST base path (see `camunda.client.prefix-physical-tenant-path`) unless disabled. When `null`, the header is omitted and REST calls target the default physical tenant.
