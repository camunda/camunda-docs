...
| `TimeProvider`       | `TimeProvider`               | Clock used for all SDK runtime cadence: worker poll loops, eventual consistency polling, retry backoff, backpressure decay, and OAuth refresh. Defaults to `CamundaTimeProvider.Live` — the system clock, clamped so it cannot move backwards. Supply a `FakeTimeProvider` to make the client's own timing virtual in tests. Note that this pins the client clock only; pinning the engine clock is a separate, explicit step via `PinClockAsync`. |
...
| `TenantIds`       | `IReadOnlyList<String>` | Tenant ids job workers activate for, from `CAMUNDA_TENANT_IDS` (comma-separated) or the `TenantIds` configuration path. `null` when unset, in which case activation falls back to `CamundaConfig.DefaultTenantId`. |
...
| `TenantIds`               | `IReadOnlyList<String>`      | Restrict job activation to the given tenant IDs (multi-tenant setups). Cannot be combined with `JobWorkerConfig.TenantId` — setting both is rejected with `ArgumentException`. If neither `JobWorkerConfig.TenantIds` nor `JobWorkerConfig.TenantId` is set (or `JobWorkerConfig.TenantIds` is empty), the activation request falls back to `CamundaConfig.TenantIds` (from the `CAMUNDA_TENANT_IDS` environment variable, comma-separated), then to `[CamundaConfig.DefaultTenantId]` (which itself defaults to `""` and can be overridden via the `CAMUNDA_DEFAULT_TENANT_ID` environment variable).                                                                                                                                                                                                              |
...
| `TenantFilter`            | `Nullable<TenantFilterEnum>` | Tenant filtering strategy for job activation. `TenantFilterEnum.PROVIDED` (the server default when this is `null`) activates jobs for the tenants named in `JobWorkerConfig.TenantIds` / `JobWorkerConfig.TenantId`, falling back to the default tenant. `TenantFilterEnum.ASSIGNED` activates jobs for whichever tenants are currently assigned to the authenticated client, re-evaluated by the server on every activation request — so tenant assignment changes take effect without restarting the worker. No tenant IDs are sent, and `JobWorkerConfig.TenantIds` / `JobWorkerConfig.TenantId` must not be set (the server would silently ignore them). Requires multi-tenancy to be enabled on the cluster; otherwise the server rejects the activation request with HTTP 400. Requires Camunda 8.9 or later. |
