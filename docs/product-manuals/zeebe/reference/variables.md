...
The `source` expression defines the **value** of the mapping. Usually, it [accesses a variable](assets/expressions.html#access-variables) of the workflow instance that holds the value. If the variable or the nested property doesn't exist then an [incident](assets/incidents.html) is created.
