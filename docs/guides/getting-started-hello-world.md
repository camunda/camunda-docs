...
5. Deploy your model by clicking the rocket icon in the bottom toolbar. You can use the pre-configured **c8run (local)** connection and click **Deploy process application**. This automatically deploys all resources in the [project](../components/concepts/process-applications.md), including the DMN decision table.
6. Click the play icon in the bottom toolbar to start a new [process instance](/reference/glossary.md#process-instance).
