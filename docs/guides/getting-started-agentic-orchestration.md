...
To add more tools to your agent, follow the same pattern used above. For full guidance on naming tools, writing descriptions, declaring `fromAi()` parameters, and mapping results, see [Add a tool to an AI agent](/components/agentic-orchestration/add-tool-to-ai-agent.md).
