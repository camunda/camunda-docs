...
Camunda Platform 8 lets you orchestrate processes with human tasks of any complexity. Thanks to user tasks, you can create and assing tasks to users. Then, users can perform their work and enter the necessary data to drive the business process.
...
Take the following 5 simple steps to create and run your first process with a human in the loop:

### Step 1: Create a new process

You will design a very simple process that demonstrates how to route the process flow based on a user decision. In this simple example, you will create a process to decide what is for dinner.

#### Create a new file

1. Navigate to Web Modeler and click **New project**.
...
2. Click the **task** icon to append a task.
...
9. You need to connect the splitted process flows again. Append another exclusive gateway to one of the tasks. Select the other task and drag the arrow-shaped sequence flow tool to connect it to the gateway.
...
You have designed the process now, but to allow the user to make the decision, you will design a form. Forms can be added to user tasks and start events to capture user input, and the user input can be used to route the process flow, to make calls to APIs, or to orchestrate your services.
...
Once the form is designed, you have to connect it to your process.
...
Your process is now ready to run. Since it is a human-centric process, it is best suited to be run in Tasklist. In order to make it accessible from Tasklist, the process must be deployed first.
...
1. Click **Deploy** to deploy the process to your cluster.
   ...
   :::
2. After you deployed your process, it can be executed on the cluster. There are multiple ways to run a process. This time, click **Run** in Modeler for a simple test run.

:::tip
Other options to run a process are to start it via Tasklist, test it in the Play mode, or calling it via the API or an inbound trigger. Read more about [run options](/components/modeler/web-modeler/run-or-publish-your-process.md).
:::
...
At this point, you've successfully crafted a simple human-centered process that routes the process flow based on decision made by a user.
...
- Learn to write powerful [expressions](/components/concepts/expressions.md) and utilize [variables](/components/concepts/variables.md) to route complex process flows easily.
- Get an [introduction to Operate](/components/operate/operate-introduction.md).
- Learn how to [setup Tasklist](/components/tasklist/introduction-to-tasklist.md) for efficient task management.
