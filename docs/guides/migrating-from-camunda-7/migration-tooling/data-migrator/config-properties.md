...
| `.history.legacy-id-prefix`    | `string`  | Prefix prepended to migrated Camunda 7 historical definition IDs (process, decision, and form definitions) to avoid ID collisions with native Camunda 8 definitions that share the same ID. Must start with a letter or underscore and contain only letters, digits, `.`, `-`, or `_` (maximum 100 characters); a blank value is rejected and an invalid value stops the migrator on startup. The effective prefix is logged at startup. **Warning:** changing or removing the prefix can cause collisions or consistency issues if you deploy Camunda 8 definitions with matching IDs. Default: `c7-legacy-`. |
