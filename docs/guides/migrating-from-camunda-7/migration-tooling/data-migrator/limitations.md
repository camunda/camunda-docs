...
- To avoid collisions between definitions (process/decision/form), each definition migrated from Camunda 7 to 8 has its ID prefixed. The prefix is configurable and defaults to `c7-legacy-`.
  - Configure the prefix with the `camunda.migrator.history.legacy-id-prefix` property (see the [property reference](config-properties.md#camundamigrator)). The prefix must start with a letter or underscore and may contain only letters, digits, `.`, `-`, or `_` (maximum 100 characters). A blank value is rejected, and an invalid value stops the migrator on startup. The effective prefix is logged when the migrator starts. The same prefix is applied consistently to all migrated history definition types (process, decision, and form) and to the entities that reference them.
  - Do not deploy new definitions in Camunda 8 with IDs starting with the configured prefix to avoid conflicts. Changing or removing the default prefix increases the risk of such collisions, so only change it when you are sure your Camunda 8 definition IDs cannot clash with the migrated IDs.
