...
- **[Data Migrator](#data-migrator)**: Copies runtime instances active in Camunda 7 as well as existing audit trail data (also known as history) to Camunda 8.
...
- **[Camunda 7 Adapter](#camunda-7-adapter)**: Allows running existing Camunda 7 delegation code directly in a Camunda 8 environment.
...
:::info
The [existing diagram converter](https://github.com/camunda-community-hub/camunda-7-to-8-migration/tree/main/backend-diagram-converter) can be used today. UI and reporting will be added. Documentation will be extended. The initial release is **planned for 8.8**. Iterative improvements will follow.
:::
...
Doing this analysis will help you understand what needs to be done to migrate.
...
It provides two important modes, which can be applied separately and will be described next:
...
Audit data migration might need to look at a huge amount of data, which can take time to migrate. In early measurements, migrating 10,000 process instances took around 10 minutes, but the number varies greatly based on the amount of data attached to a process instance (e.g., user task instances, variable instances, etc.).

You can run audit data migration alongside normal operations (e.g., after the successful big bang migration of runtime process instances) so that it doesn't require downtime and as such, the performance might not be as critical as for runtime instance migration.
