...
| `.tenantIds`           | `string`  | Comma-separated list of tenant ids for which process instances should be migrated during runtime migration. For more information: [Tenants in Runtime](/guides/migrating-from-camunda-7/data-migrator/runtime.md#tenants) Default: _(empty)_ (migrate only process instances without assigned tenant id) |
