...
Reusable credentials require Camunda 8.10 or later and a Kafka element template with the optional **Connection** credential chooser. Select the same credential on producer tasks and consumer events, or leave **Connection** empty to keep configuring brokers and authentication inline.
...
:::note
Use secrets to avoid exposing your sensitive data as plain text. To learn more, see [managing secrets](/components/hub/organization/manage-clusters/manage-secrets.md).
:::
...
If you haven't selected a **Connection** credential, optionally set the relevant credentials in the **Authentication** section. For example, `{{secrets.MY_KAFKA_USERNAME}}`.
...
2. If you haven't selected a **Connection** credential, set the URL of the bootstrap server(s). If more than one server is required, use comma-separated values.
...
If you haven't selected a **Connection** credential, select the **Authentication type** in the **Authentication** section. If you selected **Credentials** as the **Authentication type**, set the username and password.
...
- **Schema strategy**: Select the schema strategy for your messages.
  - Select **No schema**, **Inline schema** for Avro serialization.
  - Select **Schema registry** If you have a Confluent Schema Registry.
- **Bootstrap servers**: If you haven't selected a **Connection** credential, set the URL of the bootstrap server(s). If more than one server is required, use comma-separated values.
