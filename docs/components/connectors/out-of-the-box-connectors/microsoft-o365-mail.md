...
:::note
Use SaaS-managed secrets to avoid exposing your Microsoft credentials as plain text.
...
Enter a bearer token in the field **Bearer token**. Use [SaaS-managed secrets](/components/hub/organization/manage-clusters/manage-secrets.md) to avoid exposing sensitive credentials.
