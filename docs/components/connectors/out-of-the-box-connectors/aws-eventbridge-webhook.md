...
title: AWS EventBridge Webhook Connector
...
:::note
If you have used the **AWS EventBridge Webhook Connector** with a Self-Managed Camunda Platform 8 configuration before the Connector SDK [0.7.0 release](https://github.com/camunda/connector-sdk/releases/tag/0.7.0), you might need to manually replace the element template. Refer to the [update guide](/self-managed/operational-guides/update-guide/connectors/060-to-070.md) for more details.
:::

## Create an AWS EventBridge Webhook Connector task

1. Start building your BPMN diagram. You can use the **AWS EventBridge Webhook Connector** with either a **Start Event** or an **Intermediate Catch Event** building block.
2. Select the applicable element and change its template to an **AWS EventBridge Connector**.
3. Fill in all required properties.
4. Complete your BPMN diagram.
5. Deploy the diagram to activate the event trigger.

## Configure the AWS EventBridge Webhook Connector
...
2. Configure the **Webhook ID**. By default, the **Webhook ID** is pre-filled with a random value. This value will be part of the Webhook URL. For more details about Webhook URLs, refer to the section below on [activating the AWS EventBridge Webhook Connector by deploying your diagram](#activate-the-aws-eventbridge-connector-by-deploying-your-diagram).
...
The AWS EventBridge Webhook Connector supports four types of authorization:
...
1. (Optional) Configure the **Activation Condition**. This condition will be used to filter the events from the specified event source. For example, if an incoming AWS EventBridge event has the following body:
...
This condition will trigger the AWS EventBridge Webhook Connector only when the detail-type is "MyEvent" and the status is "received".

2. When using the **AWS EventBridge Webhook Connector** with an **Intermediate Catch Event**, fill in the **Correlation key (process)** and **Correlation key (payload)**.
...
## Example of configuring AWS EventBridge
...
- Learn more about [AWS EventBridge](https://aws.amazon.com/eventbridge/) and its capabilities.
