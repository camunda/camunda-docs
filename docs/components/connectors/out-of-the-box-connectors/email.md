...
To setup Oauth with Outlook
1. Go to the [Azure Portal](https://portal.azure.com/).
2. Create a new App Registration: Select Platform Web and add the redirect URL: `http://localhost/oauth2/callback`
3. Note your clientId and tenantId.
4. Click on `Api Permissions` and add `smtp.send` `imap.accessasuser.all` `pop.accessasuser.all`.
5. Click on `Certificates & Secrets` and create a new client secret, note the value.
6. Replace `<tenant-id>` and `<client-id>` in the following URL and open it in your browser:
```
https://login.microsoftonline.com/<tenant-id>/oauth2/v2.0/authorize?client_id=<client-id>&response_type=code&redirect_uri=http://localhost:8080/oauth/callback&response_mode=query&scope=offline_access%20https%3A%2F%2Foutlook.office365.com%2FIMAP.AccessAsUser.All%20https%3A%2F%2Foutlook.office365.com%2FSMTP.Send%20https%3A%2F%2Foutlook.office365.com%2FPOP.AccessAsUser.All
```
7. Login with your Outlook account.
8. You will then be redirected to a URL like this:
```https://localhost:8080/oauth/callback?code=1.Ab0AZOARe4F77EOMzdndWTh...&session_state=00a855d9-6473-16ac-4f1d-311893cb186a#```
9. Write down the part between `code=` and `&session_state=` in the URL, this is your **authorization code**.
10. Now execute this curl command to get your refresh token, replace `<client-secret>`, `<authorization-code>` and `<tenant-id>` with your actual values:
```
curl -X POST 
-H "Content-Type: application/x-www-form-urlencoded" 
-d "client_secret=<client-secret>&scope=offline_access https://outlook.office365.com/IMAP.AccessAsUser.All https://outlook.office365.com/SMTP.Send https://outlook.office365.com/POP.AccessAsUser.All&code=<authorization-code>&redirect_uri=https://localhost:8080/oauth/callback&grant_type=authorization_code" https://login.microsoftonline.com/<tenant-id>/oauth2/v2.0/token
```
12. In the response you get your refresh token, write it down.
12. In the Email connector configuration, select "OAuth2" as the authentication type and input the values you collected in the previous steps, as the token endpoint use `https://login.microsoftonline.com/<tenant-id>/oauth2/v2.0/token`.
