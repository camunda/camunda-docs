...
- Tasklist V2 is based on the new [Orchestration Cluster REST API](../../apis-tools/orchestration-cluster-api-rest/orchestration-cluster-api-rest-overview.md).
...
- Improved performance: The Orchestration Cluster REST API is optimized for faster performance and response times.
...
- Unified API: It aligns with the [Orchestration Cluster REST API](../../apis-tools/orchestration-cluster-api-rest/orchestration-cluster-api-rest-overview.md) for a consistent development experience and fine-grained [access control](../concepts/access-control/access-control-overview.md) at the process and task level.
...
- Process-level permissions such as `READ_USER_TASK` and `UPDATE_USER_TASK` on the `Process Definition` resource allow managers to see and work on all user tasks of a process.

- Task-level permissions on the `USER_TASK` resource, combined with property-based access control on the `assignee`, `candidateUsers`, and `candidateGroups` task properties, allow task workers to see and work only on tasks where they are involved (for example, they are the assignee, a candidate user, or belong to a candidate group).
...
- Process-level permissions such as `READ_USER_TASK` and `UPDATE_USER_TASK` on the `Process Definition` resource, which allow broad access to all user tasks of a process.

- Task-level permissions on the `USER_TASK` resource (for example, `READ`, `UPDATE`, `CLAIM`, `COMPLETE`), which can be combined with property-based access control on the `assignee`, `candidateUsers`, and `candidateGroups` task properties to restrict visibility and actions to relevant tasks.
