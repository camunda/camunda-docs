...
In this guide, you will learn how to read an agent instance's [state and usage metrics](/components/agentic-orchestration/agent-states-and-metrics.md) to catch this early, and how to respond to limit proximity warnings.
...
## Respond to limit proximity warnings
...
- **Handle the limit explicitly instead of letting it fail the process instance.** Catch the limit-reached error with an error boundary event and an [error expression](/components/connectors/use-connectors/index.md#error-expression), and route it to a human reviewer instead of failing the process instance. See the [guardrail sandwich and human-in-the-loop escalation](/components/agentic-orchestration/design-architecture.md#design-agent-orchestration-workflows) design principles.
