...
Configure which Camunda user task notifications a Microsoft Teams channel or personal chat receives.

## About

**Notification rules** let you control exactly which Camunda user tasks trigger a notification in Microsoft Teams. Each rule subscribes a channel or a personal chat to user task events that match a set of filters (organization, cluster, process definition, specific user task elements, candidate users, and candidate groups).

Rules are managed from the **Notification rules** page inside the Camunda for Microsoft Teams app:

- In a **personal chat** with the Camunda bot, open the app's **Settings** tab.
- In a **channel**, add the **Camunda** tab to the channel and open it to manage channel-wide rules.

## Channel vs. personal rules

The behavior of a notification rule depends on where it is created.
...
## Open the Notification rules page

### In a personal chat

1. Open the **Camunda** app in Microsoft Teams.
2. Select the **Settings** tab.
3. The page lists your existing personal notification rules.

### In a channel

1. Open the channel where you want to receive notifications.
2. Add the **Camunda** tab to the channel (**+** at the top of the channel → select **Camunda**).
3. Open the tab. The page lists the channel's existing notification rules.

## Anatomy of a rule

A notification rule combines an organization and cluster with optional filters. A user task triggers the rule only if it matches **all** of the configured filters.
...
| User tasks         | No       | One or more specific user task elements within the selected process. Pick them visually on the BPMN diagram. Only available once a process is selected. Empty matches all tasks. |
| Candidate users    | No       | Comma-separated list of user identifiers. Only tasks whose candidate users include at least one of these match.                                                                  |
| Candidate groups   | No       | Comma-separated list of group identifiers. Only tasks whose candidate groups include at least one of these match.                                                                |

### Match semantics

- **Empty filters match everything.** A rule with no filters matches every user task in the selected cluster — the broadest possible subscription.
...
1. Open the **Notification rules** page (see above).
2. Click **Create rule** in the page header.
3. Select an **organization** and **cluster** if they are not pre-selected.
4. Optionally choose a **process definition**. To target specific tasks within that process, click them on the BPMN diagram.
5. Optionally provide **candidate users** and/or **candidate groups** as comma-separated lists.
6. Click **Save**.
...
| Assignments   | The combined list of candidate users and groups. Long lists are truncated with **+N more**; hover the cell to see the full list. |
...
When a user task event matches a rule, the bot posts an interactive notification card to the channel or personal chat. From the card you can claim, assign, complete, or open the task without leaving Microsoft Teams. Cards update automatically as the task state changes.
