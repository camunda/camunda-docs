...
Cluster load percentage is based on the cluster's [flow control configuration](/self-managed/manage/flow-control/index.md).

Essentially, if flow control is configured, every partition is assigned a [write rate](/self-managed/manage/flow-control/index.md#exporting-and-write-rate) and a [write rate limit](/self-managed/manage/flow-control/index.md#write-rate-limit).
