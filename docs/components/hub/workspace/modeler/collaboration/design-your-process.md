...
In **Design** mode, access a simplified Camunda Hub modeling interface with a reduced **Properties** panel.

## About

In **Design** mode, only the documentation property and comments are shown in the **Properties** panel, which provides a decluttered user interface. All the technicalities, such as triggers to deploy the diagram or start the instance, are hidden. Linting is disabled, and problem annotations are discarded.
