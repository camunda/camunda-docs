...
When you open Play, the [setup environment overlay](#opening-play) prompts you to configure a cluster and deploy. Once deployed, the process definition view shows deployment problems, active process instances, and the configure scenario overlay.

![play configure scenario view](../img/play-configure-scenario.png)

Choose a start event and optionally provide input variables, then click a **Start** button to begin your process. These variables can also be prefilled from the example data defined for the start event in **Implement** mode.
