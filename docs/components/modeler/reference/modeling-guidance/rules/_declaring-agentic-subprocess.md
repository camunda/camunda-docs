...
- Its `zeebe:taskDefinition` type starts with `io.camunda.agenticai:aiagent-job-worker:`, which is the type the AI Agent Sub-Process template assigns under the hood. This covers forked or custom element templates that use a different `zeebe:modelerTemplate` id but still assign this type.
