...
| **Binding parameters**      | `event`: Value for the `event` attribute<br />`name`: the name of the property, one of `class`, `expression` or `delegateExpression`<br />`scriptFormat`: the format of the script (if script is to be executed)                                                                                                                                                                                                                       |
| **Mapping result**          | If `scriptFormat` is not set:<br />`<camunda:executionListener event="[event]" [name]="[value]" />`<br /><br />If `scriptFormat` is set:<br />`<camunda:executionListener event="[event]"><camunda:script scriptFormat="[scriptFormat]">[value]</camunda:script></camunda:executionListener>`<br /><br />(Notice that `[value]` needs to be set, since only `Hidden` is allowed as a type hence the user can not set a `[userInput]`). |
