...
Resolution priority is the same for every [variable kind](./data-types.md#variable-kinds). The `camunda.vars.env`, `camunda.vars.tenant`, and `camunda.vars.cluster` namespaces select a `SECRET_REFERENCE`-kind variable exactly as they select a `JSON`-kind one. The kind of the selected variable determines whether Camunda resolves its `camunda.secrets.<name>` references.
