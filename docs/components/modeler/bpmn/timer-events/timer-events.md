...
Attached to an ad-hoc sub-process hosting an [AI agent](/reference/glossary.md#ai-agent), a timer boundary event lets the process interrupt or redirect a running agent that takes too long, one of the patterns described in [mixing agents with workflow patterns](/components/agentic-orchestration/design-architecture.md#mix-agents-with-workflow-patterns).
