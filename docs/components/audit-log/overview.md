...
When the audit log is active, a record is written to [secondary storage](/self-managed/manage/databases/secondary-storage/index.md) for every applicable operation instance. By default, only user operations are tracked, not [client](../zeebe/technical-concepts/architecture.md#clients) operations. With this default behavior, you can expect a 3.5% increase in disk usage.
...
- [Self-Managed](../../self-managed/manage/audit-log/configure.md)
