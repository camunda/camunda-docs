...
To start working with the **Google Drive connector**, a relevant OAuth token must be configured and stored as a secret in the user's cluster. The token has to have permission to read and create a folder and/or files from a desired Google Drive instance. Follow the steps from the appendix to find out more about creating an OAuth token and giving relevant permissions.
...
4. _(optional)_ In the **Operation Details** section, set the field **Parent folder ID** to the desired parent, inside which a new folder will be created. Please, keep in mind, that if not specified, a new folder will be created in the Google Drive root folder of a user, who owns the OAuth token.
...
5. _(optional)_ In the **Operation Details** section, set the field **Parent folder ID** to the desired parent, inside which a new file will be created. Please, keep in mind, that if not specified, a new folder will be created in the Google Drive root folder of a user, who owns the OAuth token.
...
While this is highly not recommended due to security concerns, in certain cases, such as learning, exploring possibilities, or preparing a demo, you can still obtain a Google API token. For that, you will need to create a service account, and generate and download its keys. After that, you can execute this python script which will produce your OAuth token.
...
Now, in the Template variables field we can apply the following FEEL JSON object that has to be compatible with the Google Docs Requests API:
