...
[Service tasks](/docs/reference/bpmn-processes/service-tasks/service-tasks) within Camunda Cloud require you to set a task type and implement [job workers](/docs/product-manuals/concepts/job-workers) who perform whatever needs to be performed. This describes, that you might want to 
...
Not transmitting all variables saves resources and bandwidth, but serves another purpose as well: upon job completion, these variables are written to the process and might overwrite existing variables. If you have parallel paths in your process (e.g. [parallel gateway](/docs/reference/bpmn-processes/parallel-gateways/parallel-gateways), [multiple instance](/docs/reference/bpmn-processes/multi-instance/multi-instance)) this can lead to race conditions that you need to think about. The less data you write, the smaller the problem.
