...
Access business logic implemented for the Java VM as well as remote services by means of small pieces of glue code. This glue code maps process input/output to your business logic by means of the best-of-breed libraries of your own choice. In most cases you should use a pull approach, where external worker threads query Camunda for *external tasks*, sometimes you might also attach *JavaDelegates* to your model, and in case you need to define totally self contained BPMN process definitions you may want to leverage scripts or expressions for small pieces of logic.
...
* **Pull:** External worker threads query the process engine API for *external tasks*, they pull the work. Then they do the actual work and notify the process engine of works completion.
...
* *Polyglott architectures*: the pattern can be used to integrate - for example - .NET based services, when it might not be that easy to write Java delegates to call them. Service implementations are possible in any language that can be used to interact with a REST API.
...
Using Java delegates comes with the following advantages:

* If you develop in Java, this is a very simple way to write code and connect it with your process model, especially in embedded engine scenarios.
...
Of course, you will need to define your SQL statement. Consider using prepared statements, if you want to execute a statement object many times.
...
* Use *SAP's Java Connectors (JCo)*. Maybe consider using some frameworks to make this easier like the Open Source Frameworks [Hibersap](https://github.com/hibersap).
...
Learn more about the many ways scripts can be used with Camunda from the [User Guide](https://docs.camunda.org/manual/latest/user-guide/process-engine/scripting/).
