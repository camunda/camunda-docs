...
- **`FRACTION`** (shipped default as of 8.10): total RocksDB memory is `..._MEMORYFRACTION` (default `0.1`, i.e. 10%) of the broker's total system memory, shared across all partitions on the broker.
- **`PARTITION`** (the default before 8.10): total RocksDB memory is the number of partitions on the broker multiplied by `CAMUNDA_DATA_PRIMARYSTORAGE_ROCKSDB_MEMORYLIMIT` (512 MB by default).
