...
| `type`      | (Required) The name of the job type. Used as a reference to specify which job workers request the respective task listener job. For example, `order-items`. `type` can be specified as any static value (`myType`) or as a FEEL expression prefixed by `=` that evaluates to any FEEL string; for example, `= "order-" + priorityGroup`. Note that this is a case sensitive field (if the underlying Operating System permits it) so that type `**o**rderProcess` points to a different worker than `**O**derProcess`. |
