...
Secret resolution replaces a `camunda.secrets.<name>` reference — a Secret reference (Orchestration Cluster) — with the value a configured secret store holds for it, without that value being written into a process model, a job variable literal, or a configuration file.
...
| :------- | :-------------------------------------- | :-------------------------------------------------------------------------------------- |
| Used by  | Job workers, outbound connectors        | Inbound connectors (`POST /v2/secrets/resolve`), the modeller (`POST /v2/secrets/list`) |
...
Listing is different by design. What a store's cache holds is the values it has resolved so far, not the tenant's full set of secrets — so `/v2/secrets/list` always reads the configured stores directly rather than serving from the cache.
...
- Filtering or paginating a `POST /v2/secrets/list` response — see [Secrets](/apis-tools/orchestration-cluster-api-rest/orchestration-cluster-api-rest-secrets.md#listing-secrets).
