...
<!-- Draft. The page title, file name, sidebar placement, and this page's listing on
components/early-access/alpha/alpha-features.md are owned by camunda/camunda#60326 and are not
settled, so this page is deliberately not listed in sidebars.js yet. The v2 secret endpoints are
marked alpha; this page carries no availability marker yet, and one may be required. -->

[Secret resolution](/reference/glossary.md#secret-resolution) lets job workers use secret values at runtime without storing those values in job variables or configuration.
