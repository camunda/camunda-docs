...
This is implemented by a [sequence flow](/docs/bpmn-dmn/bpmn/bpmn-primer/#sequence-flow-controlling-the-flow-of-execution) connecting two activities:
...
You can read more about it in [our BPMN primer: sequence flows - controlling the flow of execution](/docs/bpmn-dmn/bpmn/bpmn-primer/#sequence-flow-controlling-the-flow-of-execution).
...
This is implemented by an [exclusive gateway (XOR)](/docs/bpmn-dmn/bpmn/exclusive-gateways/):

<div bpmn="workflow-patterns/xor.bpmn" callouts="xorGateway,taskB,taskC" />

All outgoing sequence flows of the XOR gateway (<span className="callout">1</span>) have a [condition](/docs/components/concepts/expressions/#boolean-expressions) configured, which decides if the process continues in **Task B** (<span className="callout">2</span> if `x>42`) or **Task C** (<span className="callout">3</span>if `not(x>42)`).

You can read more about it in [our BPMN primer: gateways - steering flow](/docs/bpmn-dmn/bpmn/bpmn-primer/#gateways-steering-flow).

### Invoke subworkflows

You need to invoke another process as part of your process.

This is implemented by a [call activity](/docs/bpmn-dmn/bpmn/call-activities/):
...
In BPMN, this is implemented using [parallel gateways (AND)](/docs/bpmn-dmn/bpmn/parallel-gateways/):
...
In BPMN, this is implemented using [multiple instance activities](/docs/bpmn-dmn/bpmn/multi-instance/):
...
In BPMN, this is implemented using [events](/docs/bpmn-dmn/bpmn/events/) (or [receive tasks](/docs/bpmn-dmn/bpmn/receive-tasks/)):

<div bpmn="workflow-patterns/wait.bpmn" callouts="timer, message" />

<span className="callout">1</span>

The timer event causes the process to wait, in this case until a specific point in time is due or some duration has elapsed. See [timer events](/docs/bpmn-dmn/bpmn/timer-events/) for more details.

<span className="callout">2</span>

The process will wait for a message to arrive. The message is an external trigger provided by API and can technically be anything, from a callback (e.g. via REST), over real messaging (like AMQP), or to notifications within your system. See [message events](/docs/bpmn-dmn/bpmn/message-events/) for more details.

You can read more about events in [our BPMN primer: events - waiting for something to happen](/docs/bpmn-dmn/bpmn/bpmn-primer/#events-waiting-for-something-to-happen).
...
In BPMN, you can leverage [boundary events](/docs/bpmn-dmn/bpmn/events/#boundary-events) or [event subprocesses](/docs/bpmn-dmn/bpmn/event-subprocesses/).
...
As with timers, you can leverage [boundary events](/docs/bpmn-dmn/bpmn/events/#boundary-events) or [event subprocesses](/docs/bpmn-dmn/bpmn/event-subprocesses/).
...
Mapping external messages to an existing process instance is called [message correlation](/docs/components/concepts/messages/). This is a crucial functionality to ensure you can communicate with process instances from the outside.
...
Sometimes, a subprocess needs to communicate with its parent process without ending the subprocess yet. BPMN allows this by an [escalation event](/docs/bpmn-dmn/bpmn/bpmn-coverage/).
...
The reaction to errors might need to be different depending on the current state of the process. This can be achieved by using [subprocesses](/docs/bpmn-dmn/bpmn/embedded-subprocesses/) in combination with either [boundary events](/docs/bpmn-dmn/bpmn/events/#boundary-events) or [event subprocesses](/docs/bpmn-dmn/bpmn/event-subprocesses/).
...
You might need to react to different event types differently, which is possible by using the [error type](/docs/bpmn-dmn/bpmn/error-events/#defining-the-error) known to BPMN:
...
In BPMN, you can use [compensation events](/docs/bpmn-dmn/bpmn/bpmn-coverage/) to easily implement compensations in your processes.
