...
| [Amazon Textract](/components/connectors/out-of-the-box-connectors/amazon-textract.md)           | Can read the input document from the Camunda document store or an external URL (not inline content, which Textract cannot process). Can also return the analysis result as a document reference instead of inline JSON.        |
