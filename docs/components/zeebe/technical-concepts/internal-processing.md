...
To avoid such problems, Zeebe employs [flow control](/self-managed/components/components-upgrade/configure-flow-control/configure-flow-control.md) strategies that apply write rate limits and [backpressure](/self-managed/components/orchestration-cluster/zeebe/operations/backpressure.md) to user requests.
