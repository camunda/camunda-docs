...
A [process model](#process-model), deployed to and versioned by the engine.
Identified by:
...
A [process instance](/components/concepts/process-instance-creation.md) is an instance of a [process defininition](#process-definition), uniquely identified by its **processInstanceKey**. It represents a single (one of any) execution of the process. It can be active (i.e. currently running), completed, or terminated.
