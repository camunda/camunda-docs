...
| **SaaS**              | Optional telemetry is on by default and can be turned off per cluster with the **Enable optional telemetry** setting in cluster settings. Changing this setting restarts the cluster. Contractual telemetry is required to operate the service and cannot be disabled. |
