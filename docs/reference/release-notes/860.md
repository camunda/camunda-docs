...
### Migrate process management endpoints to Camunda 8 REST API <span class="badge badge--long" title="This feature affects Zeebe">Zeebe</span>
...
Users can now easily check details of active and completed execution listeners for process activities and process instances. Operations engineers can efficiently resolve execution listener incidents. This enhancement streamlines operations and ensures smoother handling of incidents, ultimately improving time to unblock process execution.
...
With Camunda 8 REST API for Process Management, we will move the Go and zbctl clients to the Camunda Community Hub. This means that these clients won’t be officially supported by Camunda anymore, as [announced on our blog](https://camunda.com/blog/2024/09/deprecating-zbctl-and-go-clients/).
...
If you are interested in maintaining the community projects, contact us by filling out [this form](https://github.com/Camunda-Community-Hub/community/issues/new/choose) in the Camunda Community Hub!
...
Business Knowledge Models (BKM) can now be implemented in a decision model. Users can extract and reuse expressions in their DMN diagrams. When writing an expression in a decision, the BKM name is autocompleted together with the required parameters.

### Camunda Cluster Integration <span class="badge badge--long" title="This feature affects Play">Play</span>

<!-- https://github.com/camunda/product-hub/issues/1029 -->

Develop and test any process without leaving the Modeler. Play processes at all stages of maturity, from early prototypes with mocks to live UI demos

Play's architecture has changed to use your own Camunda cluster running 8.6 or higher. This allows you to leverage your existing infrastructure, API clients, secrets, and cluster apps like Operate and Tasklist. Architects benefit from a streamlined architecture and secure environment, while developers benefit from BPMN and feature parity with their production environment, plus the ability to debug the integration with their custom applications.

With this change, we will deprecate the alpha Play infrastructure hosted in our SaaS environment. SaaS users will see this change immediately. Self-Managed users running Web Modeler versions 8.5 or below will lose access to Play after the 8.7 release in January.

### Provide Execution listeners comprehensive support <span class="badge badge--long" title="This feature affects Zeebe">Zeebe</span>
...
### Version Migration: Automapping for process activities during migration <span class="badge badge--long" title="This feature affects Operate">Operate</span>
...
### Automatically onboard users on orgs when logging in via external IDP <span class="badge badge--long" title="This feature affects Console">Console</span>

<!-- https://github.com/camunda/product-hub/issues/2385 -->

Default organizations for external identity provider are available for organizations on an Enterprise plan now.
