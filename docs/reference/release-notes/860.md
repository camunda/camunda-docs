...
We introduce a new unified flow control mechanism inside of Zeebe that is able to limit user commands (by default it tries to achieve 200ms response times) and rate limit writes of new records in general.
...
We apply the new write limit in SaaS to our default hosting packages automatically.

For Self-Managed, it is disabled by default - you can activate and tune the write limit if required.
