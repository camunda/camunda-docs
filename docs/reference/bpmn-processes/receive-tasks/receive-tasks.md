...
:::note
An alternative way to receive tasks are [message intermediate catch events](../../bpmn-processes/message-events/message-events.md), which behave the same but can be used together with event-based gateways.
:::
...
The `correlationKey` is an expression that usually [accesses a variable](/product-manuals/concepts/expressions.md#access-variables) of the process instance that holds the correlation key of the message. The expression is evaluated on activating the receive task and must result either in a `string` or `number`.
