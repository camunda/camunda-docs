...
[Orchestration Cluster Identity](/components/identity/identity-introduction.md) is now available for OIDC setups in [Helm chart deployments](/self-managed/deployment/helm/install/index.md). Starting with this alpha version, you can configure the Orchestration Cluster components to use the identity provider (IdP) of your choice and enable single sign-on (SSO).
