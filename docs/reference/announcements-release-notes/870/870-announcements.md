...
- `CLIENT_CREDENTIALS`: This method was only introduced as a temporary solution for 8.7 to support deployments from Web Modeler when [Microsoft Entra ID is used as OIDC provider](../../../../self-managed/setup/guides/connect-to-an-oidc-provider/?authPlatform=microsoftEntraId#configuration).
