...
**Action:** Before upgrading, confirm that every connector field which resolves a secret already references that secret in the deployed BPMN. If a field relies on resolving a secret it doesn't reference, either add the reference or set `camunda.connector.secret-resolver.secret-filter.mode` to `DISABLED` before upgrading. `LAX` doesn't help here — it only changes behavior when the process definition can't be retrieved, not when a field simply doesn't declare the secret. On Camunda 8 SaaS, you can also change this per cluster in [cluster settings](/components/hub/organization/manage-clusters/settings.md#secret-filter-mode) once your cluster is on 8.9.10 or later.
