...
With version `8.8.0` going forward the following core [Orchestration Cluster](../../self-managed/reference-architecture/reference-architecture.md#orchestration-cluster) components must run on the exact same `minor`and `patch` level to ensure compatability: `Zeebe`, `Operate`, `Tasklist`, and `Identity`. Please see [component version matrix](../supported-environments.md#component-version-matrix), and our [Self-Managed reference architecture](../../self-managed/reference-architecture/reference-architecture.md#orchestration-cluster) for an overview of components.
