...
## Orchestration Cluster Components must run on exact same minor and patch levels

With version `8.8.0` going forward, `Identity` is added to the core Orchestration Cluster components (`Zeebe`, `Operate`, `Tasklist`). All of them must run the exact same `minor` and `patch` level to ensure compatability. Please see [component version matrix](../supported-environments.md#component-version-matrix), and our [Self-Managed reference architecture](../../self-managed/reference-architecture/reference-architecture.md#orchestration-cluster) for an overview of components.
