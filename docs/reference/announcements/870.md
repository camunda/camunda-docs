...
Important changes and updates for the Camunda 8.7 release that customers should be aware of are summarized below.

## What's coming in Camunda 8.7?
...
Camunda 8.7 is scheduled for release on 11 February, 2025.
...
The following blog posts also provide more information on the changes included in the Camunda 8.7 release:
...
Several [Zeebe gRPC](/apis-tools/zeebe-api/grpc.md) endpoints will be deprecated.
...
The [Tasklist GraphQL API](/apis-tools/tasklist-api/tasklist-api-overview.md) will be removed from the product once it has been fully deprecated.
...
The built-in Keycloak integration is removed for Self-Managed, allowing customers to use any compatible identity provider (IdP).
...
If you are using the recommended Camunda 8 deployment option (Helm charts), the upgrade path from version 8.6 to 8.7 will be straightforward. Updated Helm charts will be provided to support the upgrade to the new streamlined architecture.
...
A new Exporter architecture removes the need to deploy Operate, Tasklist and Identity as separate services, further streamlining the deployment.
