...
With version `8.6.0` going forward, the core components `Zeebe`, `Operate`, and `Tasklist` must run the exact same `minor` and `patch` level to ensure compatability. Please see [component version matrix](../supported-environments.md#component-version-matrix).
