...
When Camunda activity events are used in event-based processes, Camunda Admin Authorizations are not inherited for the event-based process. The authorization to use an event-based process is solely managed via the access management of event-based processes when [publishing an event-based process]($optimize$/userguide/additional-features/event-based-processes/#publishing-an-event-based-process) or at any time via the [Edit Access Option]($optimize$/userguide/additional-features/event-based-processes/#event-based-process-list---edit-access) in the event-based process List.
...
To publish event-based processes that include [Camunda Event Sources]($optimize$/userguide/additional-features/event-based-processes/#camunda-events), it is required to set [`engines.${engineAlias}.eventImportEnabled`](./system-configuration-platform-7.md) to `true` for the connected engine the Camunda process originates from.
