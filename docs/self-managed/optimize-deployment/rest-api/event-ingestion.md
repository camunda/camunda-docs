...
The following is an example of a valid propertie's `data` value. Each of those properties would be available as a variable in any [event-based process](./../configuration/setup-event-based-processes.md) where an event containing this as `data` was mapped:
