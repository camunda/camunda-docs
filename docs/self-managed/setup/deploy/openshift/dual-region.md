...
```bash reference
https://github.com/camunda/camunda-deployment-references/blob/feat/dual-region-hcp/aws/rosa-hcp-dual-region/camunda-version/8.7/procedure/camunda/create_elasticsearch_secrets.sh
```

Save it as `create_elasticsearch_secrets.sh` and execute it:

```bash
chmod +x create_elasticsearch_secrets.sh
...
Ensure that `CLUSTER_1_NAME` is set to the name of your first cluster. In this example, the value `local-cluster` is used to maintain consistency with the previous [Advanced Cluster Management step](#advanced-cluster-management).
