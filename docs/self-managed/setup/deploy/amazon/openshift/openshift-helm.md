---
id: openshift-helm
title: "Install Camunda 8 on an Red Hat OpenShift on AWS cluster"
description: "Set up the Camunda 8 environment with Helm and an optional Ingress setup on ROSA."
---
...
- An OpenShift cluster; or [Terraform](./terraform-setup.md) guide.
...
When using either standard authentication (network based or username and password) or IRSA authentication, specific environment variables must be set with valid values. Follow the guide for either [eksctl](#) or [Terraform](./terraform-setup.md#export-values-for-the-helm-chart) to set them correctly.
...
Additionally, obtain these values by following the guide for either [eksctl](#) or [Terraform](./terraform-setup.md), as they will be needed in later steps:
...
:::tip
Make sure to have `EXTERNAL_DNS_IRSA_ARN` exported prior by either having followed the [eksctl](#) or [Terraform](./terraform-setup.md#outputs) guide.
:::
...
:::tip
Make sure to have `CERT_MANAGER_IRSA_ARN` exported prior by either having followed the [eksctl](#) or [Terraform](./terraform-setup.md#outputs) guide.
:::
