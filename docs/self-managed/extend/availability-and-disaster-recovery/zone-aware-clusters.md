...
:::note
Converting an existing, non-zone-aware cluster into a zoned cluster is not yet supported. Existing [dual-region](/self-managed/extend/availability-and-disaster-recovery/dual-region.md) deployments continue to use the legacy numbering scheme. Camunda will add a supported way to zone an existing cluster in a future release.
:::
...
In the legacy [dual-region](/self-managed/extend/availability-and-disaster-recovery/dual-region.md) setup, brokers are numbered `0, 1, 2, 3, …` and the region is inferred from the parity of the node ID: even IDs (`0, 2, 4, …`) belong to one region and odd IDs (`1, 3, 5, …`) to the other. This parity-based approach only works for exactly two regions and hides the region in the numbering. Zone awareness replaces it with explicit zone names, which is what makes three or more zones possible.
...
For the full list of properties and their environment-variable equivalents, see the [cluster configuration reference](/self-managed/components/orchestration-cluster/core-settings/configuration/properties.md).

## Related resources

- [Dual-region](/self-managed/extend/availability-and-disaster-recovery/dual-region.md): synchronous two-region setup.
