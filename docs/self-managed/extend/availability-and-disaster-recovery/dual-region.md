...
:::warning
Redirecting traffic without following the full [operational procedure](/self-managed/extend/availability-and-disaster-recovery/dual-region-operational-procedure.md) can cause system inconsistencies and data issues.
:::

## Requirements

:::caution
Running dual-region setups requires you to develop, test, and execute custom [operational procedures](/self-managed/extend/availability-and-disaster-recovery/dual-region-operational-procedure.md) specific to your environment.
:::
...
- Both regions must be able to communicate with each other (for example, via VPC peering). See the [example AWS EKS implementation](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/dual-region.md).
  - Kubernetes services in one cluster must be resolvable and reachable from the other cluster and vice-versa:
    - For AWS EKS, configure DNS chaining. See the [Amazon EKS setup guide](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/dual-region.md).
    - For OpenShift, use [Submariner](https://docs.redhat.com/en/documentation/red_hat_advanced_cluster_management_for_kubernetes/2.11/html/networking/networking#submariner) for multi-cluster networking. See the [OpenShift dual-region setup guide](/self-managed/deploy-to-production/kubernetes/clusters/red-hat-openshift/self-hosted/dual-region.md).
...
When scaling, follow the [cluster scaling steps](/self-managed/components/orchestration-cluster/zeebe/operations/cluster-scaling.md) and ensure you meet the [Zeebe cluster configuration](#zeebe-cluster-configuration) requirements.
...
Follow the upgrade recommendations in the [Camunda Helm chart](/self-managed/manage/upgrade/helm/index.md) and the [component-specific upgrade guides](/self-managed/manage/upgrade/components/index.md).

Review the [upgrade overview](/self-managed/manage/upgrade/index.md) before starting, and always create a [Camunda-supported backup](/self-managed/manage/back-up-and-restore/index.md) first.
...
| **Aspect**                  | **Details**                                                                                                                                                                                                                                                                                                                                                                          |
| :-------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Installation methods        | Only Kubernetes with the [Camunda Helm chart](/self-managed/deploy-to-production/kubernetes/install/index.md) is supported. Alternative installation methods such as docker-compose aren't covered by our guides.                                                                                                                                                                    |
...
See the [operational procedure](/self-managed/extend/availability-and-disaster-recovery/dual-region-operational-procedure.md) for recovery and re-establishment steps.

:::caution
Monitor for region failures and execute the [operational procedures](/self-managed/extend/availability-and-disaster-recovery/dual-region-operational-procedure.md) promptly to ensure smooth recovery.
:::
...
1. **Temporary recovery**: Follow the [operational procedure](/self-managed/extend/availability-and-disaster-recovery/dual-region-operational-procedure.md#failover-phase) to restore functionality and unblock the process automation engine (Zeebe).
...
4. **Permanent region setup**: Follow the [operational procedure](/self-managed/extend/availability-and-disaster-recovery/dual-region-operational-procedure.md#failback-phase) to create a new secondary region.
...
1. **Temporary recovery**: Follow the [operational procedure](/self-managed/extend/availability-and-disaster-recovery/dual-region-operational-procedure.md#failover-phase) to restore processing.
2. **Permanent region setup**: Follow the [operational procedure](/self-managed/extend/availability-and-disaster-recovery/dual-region-operational-procedure.md#failback-phase) to create a new secondary region.
...
- [Multi-region resilience overview](/self-managed/extend/availability-and-disaster-recovery/index.md)
- [Cold Recovery](/self-managed/extend/availability-and-disaster-recovery/cold-recovery.md)
- [Amazon EKS dual-region setup guide](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/dual-region.md): an example blueprint using managed EKS and VPC peering with Terraform. The concepts are mainly cloud-agnostic and can be adopted by other cloud providers.
- [OpenShift dual-region setup guide](/self-managed/deploy-to-production/kubernetes/clusters/red-hat-openshift/self-hosted/dual-region.md)
- [Dual-region operational procedure](/self-managed/extend/availability-and-disaster-recovery/dual-region-operational-procedure.md): covers how to respond to a total region loss and how to prepare your environment for smooth recovery.
- [Camunda backup and restore overview](/self-managed/manage/back-up-and-restore/index.md)
