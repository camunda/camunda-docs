...
Physical Tenants provide a balanced approach to multi-tenancy—offering strong isolation without the operational complexity and cost of running separate clusters. See [Multi-tenancy overview](/self-managed/extend/tenancy/index.md) to compare with other isolation models.
...
See [Logical Tenants](/self-managed/extend/tenancy/logical-tenants.md) for details on the lightweight tenant-ID based model.
...
For detailed technical information about isolation model, architecture, and storage configuration, see [Physical Tenant isolation model](/self-managed/extend/tenancy/physical-tenants/isolation-model.md).

For tenant configuration defaults, overrides, validation, and examples, see [configuration reference](/self-managed/extend/tenancy/physical-tenants/configuration-reference.md).

For adding tenants and lifecycle expectations in 8.10, see [provisioning and lifecycle](/self-managed/extend/tenancy/physical-tenants/provisioning-and-lifecycle.md).

For how REST API requests are routed to Physical Tenants, including default tenant compatibility and HTTP status codes, see [API routing](/self-managed/extend/tenancy/physical-tenants/api-routing.md).

For identity deployment models, token routing, and per-tenant authorization, see [authentication and authorization](/self-managed/extend/tenancy/physical-tenants/authentication-authorization.md).

For how authorization is divided between cluster-wide and tenant-local operations, see [authorization model](/self-managed/extend/tenancy/physical-tenants/authorization-model.md).

For how Physical Tenant storage isolation works across primary storage, secondary storage, and document stores, see [storage isolation](/self-managed/extend/tenancy/physical-tenants/storage-isolation.md).
