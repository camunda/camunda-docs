...
:::info
This is the detailed technical documentation for Physical Tenants. For an overview and key concepts, see [Physical Tenants](/self-managed/extend/tenancy/physical-tenants/index.md).
:::
...
To configure tenant defaults, per-tenant overrides, validation expectations, and property examples, see [configuration reference](/self-managed/extend/tenancy/physical-tenants/configuration-reference.md).

To provision new tenants and understand lifecycle behavior in 8.10, including rolling restart expectations and unsupported operations, see [provisioning and lifecycle](/self-managed/extend/tenancy/physical-tenants/provisioning-and-lifecycle.md).
...
For configuration examples covering shared buckets with per-tenant paths, dedicated buckets per tenant, and GCP prefix isolation, see [document store isolation](/self-managed/extend/tenancy/physical-tenants/configuration-reference.md#document-store-isolation) in the configuration reference.

For the storage backends used by tenant-scoped data, see [secondary storage](/self-managed/manage/databases/secondary-storage/index.md) and [document handling configuration](/self-managed/manage/document-handling/configuration/index.md).
