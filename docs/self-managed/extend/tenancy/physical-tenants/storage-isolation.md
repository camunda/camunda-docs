...
- **[Configuration reference](/self-managed/extend/tenancy/physical-tenants/configuration-reference.md)** — General tenant configuration
- **[Provisioning and lifecycle](/self-managed/extend/tenancy/physical-tenants/provisioning-and-lifecycle.md)** — Tenant operations
  :::
...
:::note
Elasticsearch and OpenSearch backups are created through the [web applications backup endpoint](/self-managed/manage/back-up-and-restore/elasticsearch/backup.md#2-start-the-web-applications-backup-operate--tasklist), which operates at the cluster level. A per-tenant Elasticsearch/OpenSearch backup endpoint is not yet available and is planned as part of the management API.
:::
