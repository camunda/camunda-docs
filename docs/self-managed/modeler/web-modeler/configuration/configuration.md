...
Clusters configured using the following options can be selected when deploying from Web Modeler. If no clusters are configured, you will not be able to peform any actions that require a cluster (e.g. deploy, start an instance, play, etc.). The Camunda 8 [Helm](/docs/self-managed/setup/install.md) and [Docker Compose](/self-managed/setup/deploy/local/docker-compose.md) distributions provide a local Zeebe cluster configured by default.
