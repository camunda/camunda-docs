...
- For a working example configuration showing how the components are correctly wired together, see the [Docker Compose file for Web Modeler](/self-managed/installation/run-local/docker-compose.md).
- If you are using the Camunda 8 [Helm chart](/self-managed/installation/deploy/deploy.md) to set up Web Modeler, read more about the different configuration options in the chart's [values docs](https://artifacthub.io/packages/helm/camunda/camunda-platform#webmodeler-parameters).
