...
When upgrading from a previous version that does not use the Camunda exporter, the importers for Operate and Tasklist
must have completed processing all records generated by the previous version. For an example upgrade from 8.7.x to 8.8.x
the Camunda exporter must wait until the importers have processed all 8.7.x records.

As a result of this it is highly recommended that users reduce the backlog before attempting an upgrade. As the
Camunda exporter will not block and continually fall behind the Elasticsearch/OpenSearch exporters while the previous versioned
records are processed. The Camunda exporter cannot block exporting while waiting for the importers to finish otherwise a
deadlock will occur where the importers will never process all previous versioned records and hence the Camunda exporter will not start.
