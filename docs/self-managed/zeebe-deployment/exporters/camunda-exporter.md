...
| Option                    | Description                                                                                                                                                                                                                                                                                                                                                     | Default |
| ------------------------- |-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| ------- |
| elsRolloverDateFormat     | Defines how the date values are formatted for historical indices using Java DateTimeFormatter syntax, if no format is specified the first date format specified in the field mapping is used.                                                                                                                                                                   | `date`  |
| rolloverInterval          | The rollover period before we rollover an active index. The `elsRolloverDateFormat` must have eneugh resolution as to be possible to compute `rolloverInterval`, for example if the `rolloverInterval` is `1h`, then the `elsRolloverDateFormat` should be `yyyy-mm-dd-HH`. Aditionally the `rolloverInterval` cannot have seconds `s` or minutes `m` as units. | `1d`    |
...
| maxDelayBetweenRuns       | The delay between archive runs uses an exponential backoff strategy in case of an unsucessfull atempt at archiving, this configures the maximum time between runs.                                                                                                                                                                                              | `60000` |
