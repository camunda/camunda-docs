...
| Tenant deletion                                    | Removing a tenant from configuration disables it and retains its data. Permanent deletion is not available. A disabled tenant can be [logically removed](./provisioning-and-lifecycle.md#logically-remove-a-disabled-tenant) from the cluster topology, which deletes no data. |
