...
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/actuator/health/readiness`                      | Node-level readiness where the secondary-storage readiness check is enabled (Elasticsearch or OpenSearch only; not registered for RDBMS). It is `UP` when at least one Physical Tenant is ready and `DOWN` when no tenant is ready, except that a degraded `default` tenant still holds readiness `DOWN` regardless of other tenants' state ([camunda/camunda#51861](https://github.com/camunda/camunda/issues/51861)). It does not mean that every tenant is healthy. |
