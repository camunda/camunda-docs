...
Cluster-wide management endpoints use the cluster-admin role. Broker startup does not fail if the cluster-admin role is not configured. Configure the role to restrict cluster-wide operations, such as backup, restore, and topology management, to authorized operators.
