...
The Optimize API accepts OIDC bearer tokens. A client gets a token from your IdP with the client credentials flow, then sends the token in the `Authorization` header. In `camunda.security.authentication.oidc.audiences`, list the audience that your IdP issues for that client.

Camunda 8.10 no longer accepts the static token `api.accessToken` on this API. The token works only if you opt into the [8.9 component-specific configuration fallback](#fall-back-to-the-89-component-specific-configuration), which Camunda plans to remove in a future release. Migrate the affected API clients to OIDC bearer tokens during 8.10. For the token steps, see [Optimize API authentication](/apis-tools/optimize-api/optimize-api-authentication.md).
