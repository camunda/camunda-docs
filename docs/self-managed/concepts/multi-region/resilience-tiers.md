...
| **Recovery time (RTO)** | ~1–4 hours                                                                              | ~15 minutes                                                                                                                                                       | No recovery procedure. The window is a Raft re-election plus your own client and traffic failover                     |
| **Data loss (RPO)**     | 15 min – 4 hours (backup-interval dependent)                                            | 0 minutes                                                                                                                                                         | 0 minutes, provided replication is configured and monitored as documented                                             |
...
Multi-Region RDBMS removes the recovery procedure rather than the recovery window. No published RTO figure would be meaningful, because most of the elapsed time belongs to your client timeouts, your traffic routing, and your database's failover, not to the architecture.

Multi-Region RDBMS reaches RPO 0 through two different mechanisms, and both have to hold. Engine state loses nothing unconditionally, because Raft commits a record only once a majority of its replicas hold it. Secondary storage loses nothing because the exporter acknowledges a record only after replication confirms it, and Zeebe retains the unacknowledged log so the records replay after a writer promotion. That second half is a property of the replication configuration and of broker disk capacity, not of the architecture alone. See [recovery objectives](./multi-region-rdbms.md#recovery-objectives).
:::
