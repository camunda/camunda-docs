...
| **Data loss (RPO)**     | 15 min – 4 hours (backup-interval dependent)                                            | 0 minutes                                                                                                                                                         | No engine-state loss. Secondary-storage recovery depends on the monitoring strategy, failover target, and retained log |
...
Multi-Region RDBMS removes the recovery procedure rather than the recovery window. No published RTO figure would be meaningful, because most of the elapsed time belongs to your client timeouts, your traffic routing, and your database's failover, not to the architecture. See [recovery objectives](./multi-region-rdbms.md#recovery-objectives).
:::
