...
| **Recovery time (RTO)** | ~1–4 hours                                                                              | ~15 minutes                                                                                                                                                       | No operator step restores processing. Partitions that lost their leader pause only for a Raft re-election                     |
| **Data loss (RPO)**     | 15 min – 4 hours (backup-interval dependent)                                            | 0 minutes                                                                                                                                                         | 0 for the engine. For secondary storage, the database can lose its replication lag, which is then replayed from the Zeebe log |
...
Multi-Region RDBMS does not publish RTO and RPO targets. Neither figure is zero, and both depend on your configuration rather than on the architecture alone. See [recovery objectives](./multi-region-rdbms.md#recovery-objectives).
:::
