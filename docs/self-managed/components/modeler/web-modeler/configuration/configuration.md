...
Web Modeler currently supports PostgreSQL, Oracle and MSSQL as persistent data storage. Oracle drivers are not provided by default and must be downloaded and supplied for the application to load. Refer to the [Oracle database configuration section](/self-managed/components/modeler/web-modeler/configuration/database.md#oracle-database-configuration) for details.
