...
Camunda 8.8 provides dedicated migration applications to help ensure seamless compatibility and data integrity when
upgrading between minor versions.

The migration applications are located in the `camunda/bin/` directory. You can run them locally, targeting the same
Elasticsearch or OpenSearch instance, either as standalone tools or alongside the rest of the Camunda 8 application.

Below are details about the migration applications, including how to run them and the required configuration.

#### Generic Configuration

The migration applications accept the following generic configuration settings.
Note that not every setting applies to all migration applications.
Refer to the inline comments to determine which settings are relevant for each application.

```yaml
camunda:
  migration:
    <configuration-type>:
      # How many processes are migrated at once, only used by process migration
      batchSize: 5
      # The timeout/count down after importer has finished before the migration app should stop
      # Default PT1M
      importerFinishedTimeout: PT1M
      # The timeout for the whole migration operation. Used by all migration applications.
      # Default PT2H
      timeout: PT2H
      # Retry properties, exponential backoff. Used by all migration applications.
      retry:
        ...
        # Default PT1M
        maxRetryDelay: PT1M
  # Database connection configuration. Used by all migration applications.
  database:
    connect:
      type: elasticsearch
      # URL of ES cluster
...
Where `<configuration-type>` can be `process`, `metrics`, `tu-metrics` or `tasks`, depending on the migration application.
See the individual migration sections for more details.
...
The migration application can be found in the `camunda/bin/process-migration` directory.
The `<configuration-type>` for this application is `process`.

##### Metrics

The migration application exposes Prometheus metrics at the `:9600/actuator/prometheus` endpoint,
allowing you to monitor and track the migration process in real time.
...
To ensure the new usage metrics include historical data collected by Operate and Tasklist in earlier Camunda versions, a
dedicated migration application is provided to backfill this data.
This application contains two migrators, one for Operate and one for Tasklist, that can be configured separately.

The migration application can be found in the `camunda/bin/usage-metric-migration` directory.
The `<configuration-type>` for this application is `metrics` for the Operate migration and `tu-metrics` for the Tasklist migration part.
...
- `camunda_migration_usagemetric_operate_reindex_time` - Time taken to send operate reindex task to import data
- `camunda_migration_usagemetric_operate_task_importer_finished` - Time taken for operate task importer to finish
- `camunda_migration_usagemetric_tasklist_reindex_time` - Time taken to send tasklist reindex task to import data
- `camunda_migration_usagemetric_tasklist_task_importer_finished` - Time taken for tasklist task importer to finish
