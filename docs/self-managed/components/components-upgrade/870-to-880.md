...
See also the [logging documentation](../orchestration-cluster/core-settings/configuration/logging.md#default-logging-configuration) for more information.

### Pattern layout

The pattern layout has changed from `[%d{yyyy-MM-dd HH:mm:ss.SSS}] [%t] %notEmpty{[%X] }%-5level%n\t%logger{36} - %msg%n` to `%d{HH:mm:ss.SSS} [%t] %notEmpty{[%X] }%-5level%logger{1.1.1.*} - %msg%n`.
