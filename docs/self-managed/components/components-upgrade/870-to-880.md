...
With the upgrade to 8.8, we're changing the default replica count for Camunda indices of Elasticsearch and OpenSearch from 0 to 1. 
This ensures that if a node goes down, Camunda is not blocked by the temporary outage of the secondary data store.
If you're already overwriting this value, then no action needed from your side.
If not please read through the following to understand the change and how to adjust for it or revert to the old behaviour.

Throughout this section, references to Elasticsearch (ES) also apply to Opensearch (OS). For simplicity,
ES is used as the default term.

---

### Single-Node Cluster

If the current ES cluster only has one ES node, the replica value of 1 will cause the ES cluster state to go yellow.
This represents a scenario where all primary shards are allocated but some replicas are unassigned. To fix this:

- Increase the number of **master-eligible nodes** to at least 2.
- Refer to the [Bitnami Elasticsearch Helm chart values](https://artifacthub.io/packages/helm/bitnami/elasticsearch/21.6.3) for configuration details.
  Specifically the `master.replicaCount` field.
  ⚠️ **Important:** Bitnami recommends scaling the ES cluster down first for consistency reasons. Follow their guidance carefully.
- [Elastic Cloud - Resize Deployment](https://www.elastic.co/docs/deploy-manage/deploy/cloud-enterprise/resize-deployment)

:::note
If you are consuming Elasticsearch through the Camunda Helm Chart, the value should, by default, be `3`, resulting in a three-node cluster.
If not, the same logic as for Bitnami applies and can be overwritten with the `elasticsearch.master.replicaCount` field.
:::

Once done move onto instructions in [Multi-Node Cluster](#multi-node-cluster) section

---

### Multi-Node Cluster

Starting in Camunda 8.8, the number of replicas for indices defaults to 1. If previously the cluster had a replica count
of 0, this will roughly double disk usage. To ensure the ES cluster can reach a green status ensure the disk capacity is at least 2.5x
(To account for watermarks, overheads and growth) the previously used storage. As an example:

- In 8.7 The ES Cluster nodes disk usage is at 30/50GB, 35/50GB and 40/50GB.
- As the most used node at 40GB we must ensure its replicas can fit on other nodes, therefore all disks should be resized to
  a capacity of at least 100GB (40 \* 2.5).

#### Resizing the Elasticsearch disk

If the current disk size is too small for additional replicas, more disk space must be
assigned. Whether a Persistent Volume Claim (PVC) is expandable depends on the underlying `StorageClass`.
Follow the relevant documentation to expand the PVC as per the calculations above.
...
For externally managed services follow the associated documentation:
...
- [AWS OpenSearch - Modifying an EBS Volume](https://docs.aws.amazon.com/ebs/latest/userguide/ebs-modify-volume.html) - This is relevant as AWS Opensearch is
  backed with EBS volumes.

### Rolling back

If problems are encountered during the upgrade and subsequent replica increase, revert the replica configuration back to 0 in the Zeebe deployment
and restart as the Zeebe pods will trigger the processes which update the indices back to 0 replicas.

One of: 

- YAML configuration `camunda.database.index.numberOfReplicas` back to 0.
- Environment variable `CAMUNDA_DATABASE_INDEX_NUMBER_OF_REPLICAS` back to 0.
