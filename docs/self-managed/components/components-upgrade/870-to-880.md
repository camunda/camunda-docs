...
In cases where one Elasticsearch(ES) node goes down Camunda should not be blocked, this will only be the case for operations which
involve ES if the replica count of indices are >= 1.

:::note
Starting in Camunda 8.8, the number of replicas will be defaulted to 1, currently it defaults to 0 for the 
stock self-managed Helm chart. This setting will incur a performance impact as now all data must 
be replicated across two shards for writes.
