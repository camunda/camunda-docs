...
title: Camunda update 8.7 to 8.8
...
- Set `ZEEBE_BROKER_EXPORTERS_CAMUNDA_ARGS_INDEX_PREFIX` (the prefix for the new [Camunda exporter](/self-managed/components/orchestration-cluster/zeebe/exporters/camunda-exporter.md)) to `some-new-prefix`.
...
### Migrate Operate access permissions

The new permissions define what is displayed to users and which actions can be performed in the Operate UI. They provide more fine-grained control, for example, granting a user read access to only a specific process.

#### Migrating previous Operate permissions

To maintain the same access and views in the Operate UI as in previous versions, apply the following mappings:

<table>
  <tr>
    <th rowspan="2">8.7 Permission</th>
    <th colspan="3">[8.8 Authorization](../../../../components/identity/authorization/)</th>
  </tr>
  <tr>
    <th>Resource Type</th>
    <th>Resource ID</th>
  ...
  <tr>
    <td>`BATCH` <small>(see note below)</small></td>
    <td>`*`</td>
    <td>`READ`</td>
  </tr>
  <tr>
    <td rowspan="3">**write**</td>
    <td colspan="3">all **read** permissions</td>
  </tr>
  <tr>
    <td>`PROCESS_DEFINITION`</td>
    <td>`*`</td>
    <td>
      <tr>`CANCEL_PROCESS_INSTANCE`,</tr>
      <tr>`UPDATE_PROCESS_INSTANCE`,</tr>
      <tr>`MODIFY_PROCESS_INSTANCE`,</tr>
      <tr>`DELETE_PROCESS_INSTANCE`</tr>
    </td>
  </tr>
  <tr>
    <td>`RESOURCE`</td>
    <td>`*`</td>
    <td>`DELETE_DECISION_INSTANCE,DELETE_PROCESS`</td>
  </tr>
</table>

:::important
The authorization concept for batch operations has switched from user-based permissions to the new resource permissions. After the upgrade users will not be able to see batch operations anymore (including their own) unless they were explicitly given read permissions for them.
:::

### Leveraging the new authorizations in Operate

The new permissions model allows for more granular access control, enabling permissions for specific processes or actions.

This section details the required permissions for viewing information in Operate and performing operations.

#### Read access and visibility

| Resource                                                                                                                  | Required Authorization `<Resource Type>:<Resource ID>:<Permission(s)>`                                                          |
| ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
...
| Incident statistics for processes                                                                                         | `PROCESS_DEFINITION:{bpmnProcessId}:READ_PROCESS_DEFINITION`<br></br>`PROCESS_DEFINITION:{bpmnProcessId}:READ_PROCESS_INSTANCE` |
| Operations and batch operations                                                                                           | `BATCH:*:READ`                                                                                                                  |
| Decision definitions, instances, and requirements                                                                         | `DECISION_DEFINITION:{decisionId}:READ_DECISION_DEFINITION`                                                                     |

#### Modification and deletion

| Action                     | Required Authorization `<Resource Type>:<Resource ID>:<Permission(s)>` |
| -------------------------- | ---------------------------------------------------------------------- |
...
| Delete decisions           | `RESOURCE:{decisionId}:DELETE_DECISION_INSTANCE`                       |
