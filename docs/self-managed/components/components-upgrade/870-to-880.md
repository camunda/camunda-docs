...
With the changes to the [authorization system](/components/concepts/access-control/authorizations.md) the access control to public APIs and information visible in the component UIs had to be adapted. This means that old user and client permissions need to be adjusted to continue using the deprecated component API endpoints, as well as to still see information in the applications and perform operations.

### Migrate Operate V1 Endpoint Permissions

There is a detailed guide for migrating permissions for the V1 API Endpoints in the [public API migration guides](/apis-tools/migration-manuals/migrate-component-apis.md).

### Migrate Operate access permissions

The new permissions affect what is being displayed to users and which actions can be performed by them via the Operate UI. They now allow for more fine-grained control - for example by granting users read access to only a specific process.

#### Migrating previous Operate permissions

To maintain the same access and views in the Operate UI as in previous versions, the following permissions need to be granted to the user:
...
The new permissions concept allows for more fine-grained access controle that can be used to give users permissions for only specific processes or only specific actions. This section details the permissions needed for viewing information in Operate, as well as performing actions.
