...
Camunda 8.8 provides dedicated migration applications to help ensure seamless compatibility and data integrity when upgrading from previous minor versions.
...
Data migration execution requires the presence of the secondary storage [configuration(this will point to the correct place once docs are completed)](../../installation-methods/manual/install.md#configure-the-secondary-storage). Apart from that, all migrations share the same base configuration but they can be individually tuned depending on your needs. Prometheus metrics are exposed for the migration progress under `:9600/actuator/prometheus` endpoint.
...
2. In the `camunda/bin` directory, the `prefix-migration` script will execute the prefix migration, which can be executed locally.
   ...
   In both cases follow the configuration instructions for the secondary storage for [Elasticsearch](placeholder) or [OpenSearch](placeholder) and supply the respective values for Tasklist and Operate index prefixes.
