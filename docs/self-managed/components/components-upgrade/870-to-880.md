...
In the Helm Charts, the Identity Migration app runs as a **Kubernetes job** that operates independently from other migration processes. Key characteristics include:
...
Adjust the client index number (`2` in this example) based on your existing client configuration. Each client needs a unique index.
:::
