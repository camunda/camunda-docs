...
The `RollingFile` appender is now disabled by default. To enable it, set the environment variable `CAMUNDA_LOG_FILE_APPENDER_ENABLED=true`.
See also the [logging documentation](../orchestration-cluster/zeebe/configuration/logging.md#default-logging-configuration) for more information.

### Pattern layout

The pattern layout also changed from `[%d{yyyy-MM-dd HH:mm:ss.SSS}] [%t] %notEmpty{[%X] }%-5level%n\t%logger{36} - %msg%n` to `%d{HH:mm:ss.SSS} [%t] %notEmpty{[%X] }%-5level%logger{1.1.1.*} - %msg%n`.

**Old:**

- Full date and time `%d{yyyy-MM-dd HH:mm:ss.SSS}`
- Logger name up to 36 characters `%logger{36}`
- Includes a newline `%n` after the level
- Includes a tab `\t` before logger name

**New:**

- Only time `%d{HH:mm:ss.SSS}`
- Logger name shortened to package initials and full class `%logger{1.1.1.\*}`
- No newline after level
- No tab before logger name
