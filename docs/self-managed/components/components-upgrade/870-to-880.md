...
Data migration execution requires the presence of the secondary storage [configuration](../../components/orchestration-cluster/core-settings/configuration/properties.md#camundadatasecondary-storage). Apart from that, all migrations share the same base configuration but they can be individually tuned depending on your needs. Prometheus metrics are exposed for the migration progress under `:9600/actuator/prometheus` endpoint.

Details regarding each of the migrations can be found below.
...
The process migration component is responsible for unifying the previous separate indices of `operate-process` and `tasklist-process`. To achieve this, it must extract the Camunda Form–related data that is present in the deployments but was previously only relevant to Tasklist.
...
:::note
This migration will also version bump all the tasklist-task indices to 8.8.0. The original runtime tasklist-task index (8.5.0) will be kept for data safety reasons. If cluster retention is enabled, an ILM/ISM policy will be applied to this original runtime index (8.5.0) for the desired number of days (default 30).
:::
...
Because this migration actively moves data from indices, some temporary side effects can occur in the Orchestration Cluster:
...
This migration component is responsible for migrating existing usage metrics from Operate and Tasklist into the new data structure of their respective indices.
...
- `camunda_migration_operate_reindex_time` - Duration to reindex Operate's metrics index
- `camunda_migration_operate_task_importer_finished` - Duration until Operate's importer finishes
- `camunda_migration_tasklist_reindex_time` - Duration to reindex Tasklist's metrics index
- `camunda_migration_tasklist_task_importer_finished` - Duration until Tasklist's importer finishes
...
:::note
When using the Camunda 8 Helm charts, the Importer deployment will be present if the upgrade path is selected. Identifying when the Importer is no longer useful and can be scaled down is based in the completion of the `data-migration` Kubernetes Job.

Else, you have to explicitly enable them when upgrading, by setting:
...
:::note
In case custom prefix is only supplied for one of Tasklist or Operate, supply on the respective required `index-prefix` parameter.
:::
