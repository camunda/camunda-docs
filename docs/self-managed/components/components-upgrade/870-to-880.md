...
:::note
This migration will also version bump all the `tasklist-task` indices to 8.8.0 except for the runtime one, which for data safety reasons will remain. If cluster retention is enabled, an ILM/ISM policy will be applied to the runtime index for the desired number of days (default 30).
:::
...
:::note
When using the Camunda 8 Helm charts, the migration application is automatically deployed as a Kubernetes Job alongside the Tasklist and Operate Importer deployments if the upgrade path is selected.  
The Importers can be scaled down once the `data-migration` Kubernetes Job has completed, as this indicates they are no longer required.
:::
