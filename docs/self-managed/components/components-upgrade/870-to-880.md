...
:::note
This step is optional as Camunda 8.8 will function normally with 0 replicas, however in scenarios where
one ES pod goes down, processing will be blocked until the ES cluster can recover.

:::note
By default the replica count of indices is 0, setting it to 1, will incur a performance impact as all
data must now be replicated across two shards.
...
By default, the number of replicas for indices is set to 0. Before increasing the number of replicas
ensure the cluster **disk usage is <= 40%** on all nodes, this is because setting the replicas to 1 will double the
data used.

#### Resizing the Elasticsearch disk

If the current disk size is too small and the data requires >40% of disk space, more disk space must be
assigned. Whether a Persistent Volume Claim (PVC) is expandable depends on the underlying `StorageClass`.
Follow the relevant documentation to expand the PVC such that the current data consumes <=40% of disk space.
...
Set either of the following configuration values to 1 in the `Zeebe` deployment.

- Yaml configuration `camunda.database.index.numberOfReplicas`
- Environment variable `CAMUNDA_DATABASE_INDEX_NUMBER_OF_REPLICAS`
 
⏳ **Note:** Cluster performance may be degraded temporarily during this rebalancing process.

:::note
During the process of assigning replica shards the cluster health will be yellow, this is expected
behaviour.

### Rolling back

If problems are encountered during the replica increase, revert the value back to 0, and restart the 
Zeebe deployment as they will trigger the processes which update the indices.
