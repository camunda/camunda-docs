...
If the current cluster only has one ES node, the replica value of 1 will cause the cluster state to go yellow.
...
of 0, this will roughly double disk usage. To ensure the cluster can reach a green status ensure the disk capacity is at least 2.5x
