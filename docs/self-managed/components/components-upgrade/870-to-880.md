...
1. Shut down your Camunda 8.7 cluster (for example, reduce the replicas to zero). No traffic can be executed against your to-be-migrated Elasticsearch/OpenSearch (secondary storage) prefixes.
...
Throughout this section, references to Elasticsearch or OpenSearch are interchangeable. See the [glossary entry for Elasticsearch/OpenSearch](/reference/glossary.md#elasticsearch--opensearch) for terminology and guidance on secondary storage.
