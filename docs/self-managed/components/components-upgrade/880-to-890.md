...
##### RocksDB default memory settings.

In 8.9.0 the default memory settings for RocksDB have been adjusted to 
improve user experience and RAM usage. The default memory allocation for 
RocksDB strategy is now automatic (`AUTO`), which lets Camunda allocate the 
RocksDB memory based on the available system resources. For more information 
see the [resource planing page](/docs/self-managed/components/orchestration-cluster/zeebe/operations/resource-planning.md).
