...
If your Self-Managed cluster uses the legacy Elasticsearch or OpenSearch exporter as the data source for Optimize, be careful when introducing exporter-side filters in Camunda 8.9 and later.

Do not change exporter filters during upgrade or migration windows. While upgrading components or running importer-based migrations, keep the legacy exporter configuration stable. Changing which records are exported (for example, by filtering variables or processes) during an Optimize import can affect its sequence-based import logic and lead to gaps or inconsistencies in reports.
