...
| `CAMUNDA_LICENSE_KEY`                           | Camunda 8 license key, if required. For Helm installations, configure this in the `values.yaml` file. See [License key](/self-managed/deployment/helm/configure/license-key.md) for details. | N/A                                        |
