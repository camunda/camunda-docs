...
| `CAMUNDA_CONSOLE_REDIRECT_URL`                  | _(Optional)_ By default the origin URL is used as the auth flow redirect URL. This environment variable overrides the default.                                                                                                 | _empty string_                             |
| `CAMUNDA_CONSOLE_REDIRECT_TRAILING_SLASH`       | _(Optional)_ By default a trailing slash is added to the auth flow redirect URL. This can be changed by setting this environment variable to `false`.                                                                          | `true`                                     |
