...
| `CAMUNDA_IDENTITY_USERIDCLAIM`       | `camunda.identity.user-id-claim`        | Defines the claim used to identify a user. If unset, or if the configured claim is missing from both the token and the userinfo response, falls back to the token's `sub` claim. Useful when migrating between identity providers, for example using Azure AD's `oid` claim to preserve user identities carried over from Keycloak. | -                                                    |
...
| `CAMUNDA_ORCHESTRATION_CLUSTER_PING_CLAIM_NAME`  | The claim used to identify the identity that calls the [clusters endpoint](/self-managed/components/orchestration-cluster/zeebe/configuration/broker.md#camunda-hub-ping-configuration) (also known as ping) to register the Orchestration Cluster with Console. | `sub`                         |
| `CAMUNDA_ORCHESTRATION_CLUSTER_PING_CLAIM_VALUE` | The expected value of the claim above. See the note below before relying on the default with a non-Keycloak OIDC provider.                                                                                                                                       | `service-account-<client-id>` |
| `KEYCLOAK_INIT_ORCHESTRATION_CLIENT_ID`          | The client ID used to build the default `CAMUNDA_ORCHESTRATION_CLUSTER_PING_CLAIM_VALUE` (as the `<client-id>` in `service-account-<client-id>`) when `CAMUNDA_ORCHESTRATION_CLIENT_ID` is not set.                                                              | `orchestration`               |

:::note
The default `CAMUNDA_ORCHESTRATION_CLUSTER_PING_CLAIM_VALUE` (`service-account-<client-id>`) is a Keycloak-specific naming convention for a client's service account subject — it is not part of the OIDC specification. Other OIDC providers may populate the `sub` claim differently for a client credentials grant (for example, some reuse the client ID directly), which silently defeats this mapping rule: the ping request stops matching and fails closed.

If you are not using Keycloak, set `CAMUNDA_ORCHESTRATION_CLUSTER_PING_CLAIM_VALUE` (and `CAMUNDA_ORCHESTRATION_CLUSTER_PING_CLAIM_NAME`, if your provider does not use `sub`) to match the actual subject value your provider issues for the Orchestration Cluster's service account.
:::
