...
| `camunda.api.grpc.max-connection-age`       | <p>Set the maximum age of a gRPC connection before the gateway proactively closes it (via GOAWAY), forcing the client to reconnect.</p><p>This lets client-side load balancing pick a different node, keeping request distribution more even, and forces a refresh of the client's name resolution so nodes that weren't yet available when the client's original connections were established can still be discovered.</p><p>If unset, the gateway keeps gRPC's own default of an unbounded connection age.</p> | Null          |
| `camunda.api.grpc.max-connection-age-grace` | <p>Set the grace period, after the maximum connection age elapses, during which existing calls on the connection may finish before it's forcibly terminated. Only takes effect when the maximum connection age is set.</p><p>If unset, the gateway keeps gRPC's own default of an infinite grace period.</p>                                                                                                                                                                                                     | Null          |
...
| `CAMUNDA_API_GRPC_MAXCONNECTIONAGE`      | <p>Set the maximum age of a gRPC connection before the gateway proactively closes it (via GOAWAY), forcing the client to reconnect.</p><p>This lets client-side load balancing pick a different node, keeping request distribution more even, and forces a refresh of the client's name resolution so nodes that weren't yet available when the client's original connections were established can still be discovered.</p><p>If unset, the gateway keeps gRPC's own default of an unbounded connection age.</p> | Null          |
| `CAMUNDA_API_GRPC_MAXCONNECTIONAGEGRACE` | <p>Set the grace period, after the maximum connection age elapses, during which existing calls on the connection may finish before it's forcibly terminated. Only takes effect when the maximum connection age is set.</p><p>If unset, the gateway keeps gRPC's own default of an infinite grace period.</p>                                                                                                                                                                                                     | Null          |
