...
`camunda.secrets.*` sets the root-level defaults, applied to the `default` physical tenant. Override per physical tenant under `camunda.physical-tenants.<id>.secrets.*`. See the [Physical Tenants configuration reference](/self-managed/concepts/physical-tenants/configuration-reference.md).
