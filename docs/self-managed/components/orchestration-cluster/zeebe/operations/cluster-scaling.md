...
:::note
Clusters configured with the `FIXED` partitioning scheme (`zeebe.broker.experimental.partitioning.scheme: FIXED`) do not support broker scaling. Scaling requests are rejected for these clusters. To scale a cluster using fixed partitioning, first switch to the `ROUND_ROBIN` or `ZONE_AWARE` partitioning scheme.
:::
...
:::note
This endpoint does not support clusters using the fixed partitioning scheme configured with `zeebe.broker.experimental.partitioning`. Scaling requests are rejected for clusters with fixed partition distribution. To scale such a cluster, switch to the `ROUND_ROBIN` or `ZONE_AWARE` partitioning scheme first.
:::
