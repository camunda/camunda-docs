...
- `FRACTION` (default): RocksDB memory is allocated as a fraction of total available memory.
- `PARTITION`: Total RocksDB memory equals the actual number of partitions per broker (from the cluster topology) multiplied by the configured memory limit.
...
For the `PARTITION` and `BROKER` strategies, the default value of `CAMUNDA_DATA_PRIMARYSTORAGE_ROCKSDB_MEMORYLIMIT` allocates [512 MB](https://github.com/camunda/camunda/blob/main/dist/src/main/config/defaults.yaml).
...
The amount of OS page cache needed depends on factors such as the number of partitions on a node and system throughput. For most use cases, reserve 20–30% of total memory for the OS page cache, adjusting as needed based on observed performance.
