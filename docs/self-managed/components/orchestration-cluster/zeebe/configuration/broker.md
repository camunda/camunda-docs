...
This feature enables components like the Zeebe Broker, Tasklist, Operate, and Zeebe Gateway to ping Console with license information. For this feature to work, you must enable [dynamic cluster configuration](/self-managed/components/hub/configuration/properties.md#dynamic-cluster-management), which exposes the create cluster API endpoint.
...
| `endpoint`                   | Create cluster API endpoint where pings should be sent. This setting can also be overridden using the environment variable `CAMUNDA_CONSOLE_PING_ENDPOINT`.                            | `https://console.endpoint.com/api/v1/clusters` |
