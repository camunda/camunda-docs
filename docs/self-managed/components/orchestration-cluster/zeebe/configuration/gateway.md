...
| maxConnectionAge      | Sets the maximum age of a gRPC connection before the gateway proactively closes it (via GOAWAY), forcing the client to reconnect. If omitted, the connection age is unbounded (disabled). This setting can also be overridden using the environment variable `ZEEBE_GATEWAY_NETWORK_MAXCONNECTIONAGE`.                                                                      |               |
