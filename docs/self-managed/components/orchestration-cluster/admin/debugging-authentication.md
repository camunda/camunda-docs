...
- An unreachable `jwk-set-uri` fails the token validation, and writes no `Failed to resolve` warning. Set `LOGGING_LEVEL_ORG_SPRINGFRAMEWORK_SECURITY=DEBUG` to see the cause.
