...
:::note
This page describes logical tenants for Optimize. For strong physical isolation of separate teams or organizations within a single cluster, see [Physical Tenants](/self-managed/concepts/multi-tenancy/physical-tenants.md). If you run one Optimize instance per Physical Tenant behind a shared Management Identity, see [Optimize and Physical Tenants](/self-managed/concepts/physical-tenants/optimize.md#known-limitation-logical-tenants-with-the-same-id-across-physical-tenants) for a limitation with reusing the same logical tenant ID across tenants.
:::
