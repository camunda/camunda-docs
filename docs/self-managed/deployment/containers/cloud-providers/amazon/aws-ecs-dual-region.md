...
What pausing does not change: a replication stall holds the acknowledged position back whether or not you pause, so the export backlog grows either way. Self-Managed ships with both the static write limit and backlog throttling disabled, so that backlog does not slow the engine on its own. They are separate switches in [flow control](/self-managed/operational-guides/configure-flow-control/configure-flow-control.md): only `throttling` reacts to the export backlog, so turning on the static limit alone does nothing here. Either way they key on the backlog rather than on this flag, so pausing neither turns them on nor exempts you from them.
