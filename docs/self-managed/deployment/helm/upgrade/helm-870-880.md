...
Generally, anything outlined in this document and the component specific upgrade guide applies the same way to multi-region the same way. The only exception is the manual management of the exporters as they're explicitly defined for dual-region.

The following is just an additional helper to ensure you've covered the steps for dual-region setups.

#### Preparation

Coming from a dual-region setup, as outlined in the [documentation](/self-managed/deployment/helm/cloud-providers/amazon/amazon-eks/dual-region.md), the following changes are required:

1. `global.elasticsearch.disableExporter` is now `orchestration.exporters.camunda.enabled` and `orchestration.exporters.zeebe.enabled`.

Where `orchestration.exporters.camunda.enabled` is for the new [Camunda Exporter](/self-managed/components/orchestration-cluster/zeebe/exporters
/elasticsearch-exporter.md) and `orchestration.exporters.zeebe.enabled` for the old [Elasticsearch / OpenSearch Exporter](/self-managed/components/orchestration-cluster/zeebe/exporters/elasticsearch-exporter.md).

For dual-region those have been defined manually and the same case applies for the migration to 8.8.

The former definitions will need to remain for the 8.7 to 8.8 migration as outlined in the documentation.

2. Define new Camunda exporters for both regions

Explicit configuration options like authentication can be found on [Camunda Exporter documentation](/self-managed/components/orchestration-cluster/zeebe/exporters
/elasticsearch-exporter.md).

Previous old configured exporters like `ZEEBE_BROKER_EXPORTERS_ELASTICSEARCHREGION0_CLASSNAME` should be kept for the migration.
...
3. Migration of former Environment Variables to new ones.

With the re-architecture of 8.8, the way the Zeebe Gateway is configured has changed from previously a standalone gateway to now an embedded gateway since it's not part of the single Orchestration Cluster application.
What that means in detail is that previous environment variables like `ZEEBE_GATEWAY_CLUSTER_MESSAGECOMPRESSION` are now added an additional prefix `BROKER` like `ZEEBE_BROKER_GATEWAY_CLUSTER_MESSAGECOMPRESSION`. In short `ZEEBE_GATEWAY` becomes `ZEEBE_BROKER_GATEWAY`.

4. Additional migration changes required.

If using `cluster message compression`, which is pretty standard in multi-region setups, then for the migration itself, it's required to be added additionally to the `orchestration` configuration.

In `step 2` we migrated the former environment variables to new ones due to the embedded gateway topic. The migration will spawn jobs and deployments that are utilizing a standalone gateway, meaning something like `ZEEBE_GATEWAY_CLUSTER_MESSAGECOMPRESSION` is crucial to be set as otherwise the migration related work won't be able to communicate with the Zeebe cluster.

5. As outlined in this document, enable the migration topic
...
6. If you have previously relied on basic auth and defined your user in `camunda.operate` or `camunda.tasklist`, those won't be migrated and require your manual adjustment to the new embedded Identity that supports proper basic authentication across all components. See the documentation of the new [embedded Identity](/self-managed/components/orchestration-cluster/identity/overview.md) on how to configure initial users on startup to not fallback to `demo:demo`. See also [authentication section](#authentication) on the topic.

Additionally the API, including gRPC and REST API are secured by default. This can be disabled in the Helm Chart via the following to restore previous behavior.
...
See an example [values.yaml](https://github.com/camunda/c8-multi-region/blob/8-8-adjustments-migration/aws/dual-region/kubernetes/camunda-values-migration.yml) containing the 8.7 to 8.8 changes and temporary migration enabled.

#### Edge Cases

1. If you're running Identity, ensure that you read the Identity migration and enable this as well for only one region. The Zeebe cluster is stretched and therefore acting as a single Zeebe Cluster, only requiring it to be run once.
2. If you're not manually configuring the exporters but relying on the Helm Chart, meaning you have a single external secondary storage:
   1. continue relying on the Helm Chart and ensure `orchestration.exporters.zeebe.enabled` is true as well as `orchestration.exporters.orchestration.enabled`.
   2. Deploy the data migration and importer only in one region and keep it disabled in the other.
3. If you're using Optimize, on top of Identity, you will need to keep the old Elasticsearch exporter enabled after migration as it does not utilize the Camunda Exporter yet.
