...
Remove the migration components from [Step 3: Prepare and run the upgrade](#step-3-prepare-and-run-the-upgrade) so they aren’t recreated on your next helm upgrade. First delete the one-off Jobs (and the Identity secret if you created one), then disable the migration features in your `values.yaml`.

```bash
# Delete the data/identity migration Jobs (names vary by release)
kubectl -n <namespace> delete job <RELEASE_NAME_zeebe-migration-data>
kubectl -n <namespace> delete job <RELEASE_NAME-zeebe-migration-identity>

# Remove the identity migration secret if you created it
kubectl -n <namespace> delete secret <IDENTITY_MIGRATION_SECRET_NAME>
```

```yaml
# Remove the migration section or turn off the data and identity migration (this also prevents the importer deployment)
migration:
  data:
    enabled: false
  identity:
...
If the migration job (for data or identity) fails to complete and eventually stops retrying, it likely hit its `backoffLimit` (the maximum number of retry attempts).
This usually happens when the importer job wasn’t finsihing yet before the jobs stops.
