...
1. Scale down the importer to stop it from consuming resources:

```bash
kubectl -n <namespace> scale deploy/<importer-name> --replicas=0
```

2. Optionally delete the migration jobs (data and identity) and the migration secret from step [Identity migration](#identity-migration) to keep the namespace tidy:

```bash
kubectl -n <namespace> delete job *-zeebe-migration-data
kubectl -n <namespace> delete job *-zeebe-migration-identity
kubectl -n <namespace> delete secret identity-migration-secret
```
