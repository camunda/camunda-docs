...
:::caution
If your multi-region setup uses cluster message compression (standard practice), configure it in the `orchestration.env` section. Migration jobs use a standalone gateway and require this variable to communicate with the Zeebe cluster.
:::

#### Enable migration components

Follow the [Data migration](#data-migration-required) and [Importer](#enable-importer-required) sections, ensuring migration components are enabled:
...
- **Identity migration (if using Identity):**  
  Enable [Identity migration](#identity-migration-optional) in **only one region**. The stretched Zeebe cluster requires the migration to run a single time. Review the [migration of secrets](#migrate-secrets-to-new-format).
