...
The [Identity Migration](<#identity-migration-(optional)>) requires a Keycloak client with the ID `migration`.
If you configure clients manually in your `values.yaml` under identity.env or identity.auth.clients, ensure that the migration client is also defined.
If the `migration` client is missing, the migration process will fail.
