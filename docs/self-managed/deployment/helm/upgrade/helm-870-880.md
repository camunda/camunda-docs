...
:::info
The Helm Chart creates an dedicated Keycloak client with read-only access to your Identity Resource Server. If you override your identity clients, follow the `Helm chart` section in [Common Configuration for the Identity Migration Application](../../../components/components-upgrade/870-to-880.md#common-configuration-for-the-identity-migration-application) to setup the needed `migration` client.
:::
