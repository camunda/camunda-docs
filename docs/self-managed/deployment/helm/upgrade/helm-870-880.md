...
The identity migration migrations a Keycloak client with the ID `migration`.
...
**Note:** The client index number (`2` in this example) should be adjusted based on your existing client configuration.
...
The Identity Migration app migrates data from Management Identity (8.7) to the new Orchestration Cluster Identity (8.8).
...
- The identity migration app needs access to the resource serves.
