...
1. [Take a backup](#step-4-take-a-backup): Back up your Camunda 8.7 cluster.
1. [Prefix Migration](#step-5-prefix-migration): Perform the prefix migration, only required for custom prefix installations.
...
Review the new [secret conventions](#extract-secrets) in 8.8 and upgrade your secrets accordingly.
...
Take a full backup before starting the upgrade process. Follow the [backup and restore guide](../../../operational-guides/backup-restore/backup-and-restore.md)

## Step 5: Prefix migration

In case custom prefixes for Tasklist and Operate are used on your installation make sure to perform the [Prefix migration](../../../components/components-upgrade/870-to-880.md#prefix-migration) beforehand. **This step requires cluster downtime.**

## Step 6: Prepare and run the upgrade
...
For updating clusters, the data migration job is required to be executed. To do so, set the following in your `values.yaml`:

```yaml
orchestration.migration.data.enabled: true
```

A Kubernetes job is created that will perform the migration. This job can run for an extended period if needed, waiting for the importers to finish processing any remaining Camunda 8.7 backlog.

This will also enable the Importers deployment, since it's necessary to have the Tasklist and Operate importers running in order to handle left over data from Camunda 8.7.

You can find more information regarding the scope of the migrations in the dedicated [component section](../../../components/components-upgrade/870-to-880.md#data-migration).

:::tip
To speed up migration time and therefore free cluster resources sooner, you can make sure that the 8.7 processing backlog is not very extensive.
:::

:::note
If you are making use of per index replica and/or shard count settings you should specify those via:

```yaml
orchestration:
  env:
    - name: CAMUNDA_DATABASE_INDEX_REPLICASBYINDEXNAME_INDEX-NAME
      value: 1
    - name: CAMUNDA_DATABASE_INDEX_SHARDSBYINDEXNAME_INDEX-NAME
      value: 1
```

where `INDEX-NAME` is the respective index you want to modify. This can be repeated for multiple indices.
:::

### 6.3: Full migration configuration
...
If you have installed the Camunda 8.7 Helm chart before with default values, Identity and the related authentication mechanism are enabled. If not specified on installation, the Helm chart generates random secrets for all components (including Keycloak). To upgrade successfully, these secrets must be extracted as external secret, applied, and referenced in the values file.

#### Extraction

:::note
The following list contains all secrets that are auto-generated by the Helm chart, but the exact list depeds based on the componenets enabled in your setup. For example, if you use an external Keycloak, you can remove it from the list below.
:::

:::caution
Don't change or regenerate any secret during the upgrade process.
:::
...
First we template a secret object that has all secrets which will be used for the Camunda 8.8 chart.
...
Now we apply the secret file:

:::caution
First inspect the `existing-secrets-manifest.yaml` file and ensure it has all secrets based on the componenets enabled in your 8.7 deployment.
:::
...
Now we refrence the existing secret object and the keys in a values file that will be used from now on:

:::caution
If you already use existing secret for any component mentioned here, ensure to remove it from the `existing-secrets-values.yaml`, otherwise it will override it.
:::
...
After exporting all secrets into the secrets manifest `camunda-credentials` and refrenced the `existing-secrets-values.yaml` file, run the following upgrade command:

```shell
helm repo update
helm upgrade camunda camunda/camunda-platform --values existing-secrets-values.yaml
```

In case of the installation, you can generate the secrets of `existing-secrets-manifest.yaml` file directly.
...
Monitor the Helm job logs (`identity-migration`, `data-migration`).

:::note
Connectivity errors and retrying messages are expected during startup as the migrations expect a healthy state for the orchestration cluster.
:::

- For `data-migration` you will see the following log lines
  ```
  ...
  ```
- For `identity-migration` you will see the following log lines
  ```
...
Kubernetes migration related jobs should be completed, verify the cluster's health and readiness status.

### Next steps

You can now scale down the Importer deployment as it will be idling since [CamundaExporter](../../../components/orchestration-cluster/zeebe/exporters/camunda-exporter.md) will handle secondary storage interactions.
