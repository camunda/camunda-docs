...
If you do not want to use authentication, you must explicitly disable it for Orchestration cluster; otherwise, users must be added to all requests. Ensure your `values.yaml` matches the configuration described in [Basic authentication configuration](../configure/authentication-and-authorization.md#basic-authentication).
...
For an internal Keycloak setup, follow the [Internal Keycloak guide](/self-managed/deployment/helm/configure/authentication-and-authorization.md#internal-keycloak).

For an external Keycloak deployment, ensure your `values.yaml` is configured as described in [OpenID connect provider setup](/self-managed/deployment/helm/configure/connect-to-an-oidc-provider.md).

:::warning Migration client required
The identity migration creates a Keycloak client with the ID `migration`.
If you configure clients manually in your `values.yaml` under `identity.env` or `identity.auth.clients`, ensure that the `migration` client is also defined. For more details, read [Identity migration](#identity-migration-optional).
:::
...
The Identity Migration app transfers your existing user authorizations from **Management Identity** (8.7) to the new **Orchestration Cluster Identity** (8.8). This migration is optional but **strongly recommended** to preserve your existing user setup and avoid manual reconfiguration.

:::tip Why migrate Identity data?
Running the identity migration saves you significant manual work by automatically transferring all your users, roles, groups, and permissions to the new architecture. Without this migration, you'll need to manually recreate all authorizations in the Orchestration Cluster.
:::

#### What gets migrated

The migration process transfers different entities depending on your identity provider:
...
- Resource authorizations (if configured)
- Applications and tenants

**For external identity providers** (OIDC like Microsoft Entra ID):
...
The Identity Migration app runs as a **Kubernetes job** that operates independently from other migration processes. Key characteristics include:
...
When you enable the migration, the system automatically creates the required `migration` client for you.
...
If you use [resource authorizations](../../../../../versioned_docs/version-8.7/self-managed/identity/access-management/resource-authorizations.md) to control fine-grained access in Camunda 8.7, enable this setting to preserve your authorization rules:
...
:::warning Important for resource authorizations
Without this setting, the migration creates broader wildcard permissions that may grant users more access than intended. For example, users with limited Operate access might gain full access to all processes.
:::
...
orchestration:
  migration:
    identity:
      enabled: true
      # Only required for external identity providers (OIDC)
      idpApplicationId: "your-application-id"
      # Only required if you use resource authorizations
      resourceAuthorizationsEnabled: true
```

#### Post-migration cleanup

After successful migration, you can clean up the migration resources:

1. Delete the migration Kubernetes job
2. Remove the migration ConfigMap
3. Delete the migration client from Management Identity (if manually configured)
