...
| Browser / external client → Ingress / GatewayAPI (UI, gRPC)                                | Standard Kubernetes Ingress TLS — configured via per-component `*.ingress.tls` or `global.gateway.tls`, **not** `global.tls.caBundle`. See [Ingress configuration](/self-managed/deployment/helm/configure/ingress/ingress-setup). |
