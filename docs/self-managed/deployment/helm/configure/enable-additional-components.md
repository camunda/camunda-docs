...
Starting with Camunda 8.8, the Helm chart reflects a new architecture where Zeebe, Zeebe Gateway, Operate, Tasklist, and Orchestration Cluster Identity are consolidated into a single [Orchestration Cluster](/reference/glossary.md#orchestration-cluster). As a result, the default deployment includes only the Orchestration Cluster and Connectors. This page explains how to enable additional components you may need.

## Default vs. additional components

### Enabled by default (8.8+)

- Orchestration Cluster (Zeebe, Zeebe Gateway, Operate, Tasklist, Orchestration Cluster Identity)
- Connectors
...
- **Orchestration Cluster Identity** - Manages authentication and authorization for Orchestration Cluster applications and APIs (for example, Zeebe access, Operate, and Tasklist). This identity service is built into the Orchestration Cluster and does not require Management Identity.
...
:::info
If you enable Web Modeler, Console, or Optimize without enabling Management Identity, these components will not function properly as they require authentication. The Orchestration Cluster (Zeebe, Operate, Tasklist, and Orchestration Cluster Identity) does not depend on Management Identity.
:::
