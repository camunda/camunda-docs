...
When you use an external OIDC provider, assign each Camunda component its own audience so a token issued for one component is not accepted by another. See [Assign a unique audience to each component](./generic-oidc-provider.md#assign-a-unique-audience-to-each-component).
