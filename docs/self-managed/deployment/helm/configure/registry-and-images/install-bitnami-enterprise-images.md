...
For production environments, consider using official Kubernetes operators for PostgreSQL (CloudNativePG), Elasticsearch/OpenSearch (ECK), and Keycloak instead of Bitnami subcharts. See [Deploy required dependencies with Kubernetes operators](/self-managed/deployment/helm/configure/operator-based-infrastructure.md) for detailed instructions.
:::
