...
:::note
The bundled Elasticsearch Bitnami subchart (`elasticsearch.enabled: true`) is deprecated and will be removed in a future release. For production deployments, use the [ECK (Elastic Cloud on Kubernetes) operator](/self-managed/deploy-to-production/plan-your-deployment/kubernetes-operators.md#elasticsearch-deployment) or a managed Elasticsearch service instead. See [deploy required dependencies with Kubernetes operators](/self-managed/deploy-to-production/plan-your-deployment/kubernetes-operators.md) for details.
:::
