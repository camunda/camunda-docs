...
2. **Create the secrets:**
   ...
   Create a Kubernetes secret to store the credentials referenced in the values file. For development, you can use simple passwords:

   ```bash
   kubectl create secret generic camunda-credentials \
     --namespace camunda \
     --from-literal=identity-firstuser-password=demo \
     --from-literal=identity-optimize-client-token=devtoken \
     --from-literal=identity-orchestration-client-token=devtoken \
     --from-literal=identity-connectors-client-token=devtoken \
     --from-literal=identity-keycloak-admin-password=admin \
     --from-literal=identity-keycloak-postgresql-admin-password=admin \
     --from-literal=identity-keycloak-postgresql-user-password=admin \
     --from-literal=webmodeler-postgresql-admin-password=admin \
     --from-literal=webmodeler-postgresql-user-password=admin
   ```

   For more details on managing secrets, see the [secret management guide](/self-managed/deployment/helm/configure/secret-management.md).
