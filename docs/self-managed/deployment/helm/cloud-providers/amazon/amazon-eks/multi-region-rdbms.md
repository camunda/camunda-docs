...
| `orchestration.partitionDistribution.scheme` | `zone-aware`                             | Zone-aware partitioning                                                 |
...
Brokers are identified as `<zone>_<index>`, so `paris_1` is the second broker in the Paris zone. The zone list is identical in every region; only `orchestration.partitionDistribution.zone` and the advertised host differ.
...
The values file is the same in every region. Only `orchestration.partitionDistribution.zone` and the advertised host differ, which is what makes the topology a single description rather than one per region.
