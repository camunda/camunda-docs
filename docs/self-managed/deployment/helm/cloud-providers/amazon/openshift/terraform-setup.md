...
If you require managed services for PostgreSQL Aurora or OpenSearch, you can refer to the definitions provided in the [EKS setup with Terraform](../amazon-eks/terraform-setup.md) guide. However, please note that these configurations may need adjustments to fit your specific requirements and have not been tested. By default, this guide uses the database services (PostgreSQL and Elasticsearch) integrated into the default Helm chart, but RDBMS (PostgreSQL, MySQL, MariaDB, or Oracle) is also a supported alternative for secondary storage — see [configure RDBMS in Helm](/self-managed/deployment/helm/configure/database/rdbms.md).
