...
| `global.elasticsearch.tls.jks.secret.*` | Removed with the truststore password injection. Use `global.tls.caBundle.secret` (PEM), or pass `-Djavax.net.ssl.trustStorePassword=...` via the component's `javaOpts`. To keep the password out of the values file, add a component `env` entry sourcing it from a Secret (`name: TRUSTSTORE_PASSWORD`, `valueFrom.secretKeyRef`) ahead of `javaOpts` in the same container, then reference `-Djavax.net.ssl.trustStorePassword=$(TRUSTSTORE_PASSWORD)`. |
