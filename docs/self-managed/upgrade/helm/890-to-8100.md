...
The Camunda Hub database migration from 8.9 to 8.10 is not backward compatible. Stop all Hub workloads, take a verified database backup, and complete the migration before restoring traffic. For background on how Hub performs the database migration, see [how Camunda Hub upgrades between versions](/self-managed/components/hub/version-upgrade.md).
