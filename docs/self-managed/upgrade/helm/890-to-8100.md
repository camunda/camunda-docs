...
| `optimize.multitenancy.enabled`                                               | Move to **`global.multitenancy.enabled`** (not `extraConfiguration`). This is a platform-wide switch that requires Identity with an external database — see [multi-tenancy](/self-managed/concepts/multi-tenancy/index.md). |
