...
Create and verify a database backup after Hub is quiesced. This backup is the recovery point if migration fails or you must return to 8.9.

Do not continue until you have confirmed the backup can be restored. See [Backup and restore](/self-managed/operational-guides/backup-restore/backup-and-restore.md).
...
If migration fails, keep the release in `migrate`. To return to 8.9, first restore the verified 8.9 database backup, and then roll back the Helm release.
