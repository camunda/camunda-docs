  ...
  restapi:
    env:
      - name: AWS_ACCESS_KEY_ID
        valueFrom:
          secretKeyRef:
            name: webmodeler-aws-credentials
            key: accessKeyId
      - name: AWS_SECRET_ACCESS_KEY
        valueFrom:
          secretKeyRef:
...
camundaHub:
  serviceAccount:
    annotations:
      eks.amazonaws.com/role-arn: arn:aws:iam::<account-id>:role/webmodeler-role
```
...
For IRSA, the webhook injects `AWS_REGION` from the cluster's region automatically. EKS Pod Identity does not, so set `AWS_REGION` explicitly for Optimize and Camunda Hub when using Pod Identity. For IRSA, set `AWS_REGION` explicitly only if the OpenSearch domain or S3 bucket is in a **different** region than the cluster. Connectors doesn't need `AWS_REGION` because its element templates carry the region per task.
...
- **Orchestration** continues to own `global.documentStore.type.*` for the document store feature itself — no change needed if that is all you use it for.
- **Identity** never had chart-rendered document-store config. If you hand-wrote an `identity.env` override for Aurora/RDS IAM auth using document-store values, that override is untouched and still yours to maintain.
