...
The Camunda Hub database migration from 8.9 to 8.10 is not backward compatible. Stop all Hub workloads, take a verified database backup, and complete the migration before restoring traffic.

:::warning
Do not use `helm upgrade --atomic` for this upgrade. After the database migration starts, do not roll the Helm release back to 8.9 without first restoring the 8.9 database backup. An 8.9 application must not run against a partially or fully migrated 8.10 database.
:::
...
Confirm both Hub Deployments show zero desired and available replicas:
...
Wait for the single Hub REST API pod to become ready. The pod runs the database migration during startup but remains fenced from the Hub Service:
...
1. Set `camundaHub.upgrade.phase` back to `quiesce` and wait for both Hub Deployments to reach zero replicas, as described in [Stop Hub workloads](#stop-hub-workloads).
...
## Install 8.10 without existing Hub data
