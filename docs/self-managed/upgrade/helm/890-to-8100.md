...
| `global.elasticsearch.tls.jks.secret.*` | Removed with the truststore password injection. Use `global.tls.caBundle.secret` (PEM), or pass `-Djavax.net.ssl.trustStorePassword=...` via the component's `javaOpts`.                 |
