...
| Set secondary storage type     | Camunda 8.9 requires `orchestration.data.secondaryStorage.type` to be explicitly set. See [Secondary storage type is now required](#secondary-storage-type-is-now-required).                                                                                                                      |
...
| `connectors.extraConfiguration: {}` (object) | Changed to an array: `connectors.extraConfiguration: []`. Update your configuration format if you use this key. |
...
Camunda 8.9 no longer defaults to a secondary storage type. You must explicitly set `orchestration.data.secondaryStorage.type` in your `values.yaml`. Helm will fail with a validation error if this is not configured.
...
orchestration:
  data:
    secondaryStorage:
      type: elasticsearch # Required — no longer defaults
    ...
    authentication:
      oidc:
        secret:
          existingSecret: connectors-oidc
          existingSecretKey: client-secret
  extraConfiguration: # Changed from object to array
    - customKey: customValue
```
...
If Helm fails with a validation error about secondary storage type, add the required configuration to your values file:
