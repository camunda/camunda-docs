...
| Check your `values.yaml`     | Review your existing `values.yaml` for custom configuration that must be updated for 8.9 (for example, Ingress rules, authentication, exporters, Elasticsearch/OpenSearch settings, index prefixes, and component overrides).                                                                                   |
| Test upgrade                 | Test the upgrade in a non-production environment using a copy of your production configuration.                                                                                                                                                                                                                 |
| Migrate plaintext secrets    | If your `values.yaml` contains plaintext secrets, migrate them to Kubernetes secrets before upgrading. See [Extract plaintext values and reference them as Kubernetes secrets](/self-managed/deployment/helm/configure/secret-management.md#extract-plaintext-values-and-reference-them-as-kubernetes-secrets). |
