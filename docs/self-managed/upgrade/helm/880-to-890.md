...
| `<componentName>.extraConfiguration: {}` (object) | Changed to an array: `<componentName>.extraConfiguration: []`. Affects all components that support this key (connectors, orchestration, identity, console, optimize, webModeler). See [Migrate extraConfiguration format](#migrate-extraconfiguration-format). |
