...
- **Recommended (secure default):** Update your connector secret environment variables to use the `SECRET_` prefix (or your configured prefix), and ensure the prefix is explicitly set:
  ```
  camunda.connector.secret-provider.environment.prefix=SECRET_
  ```
- **Optional (restore old behavior, unsafe):** To allow all environment variables as secrets (previous behavior), set the prefix to an empty value:
  ```
