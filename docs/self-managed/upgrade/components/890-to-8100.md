...
Camunda 8.10 validates the configured OIDC redirect URI while the Orchestration Cluster starts, and refuses to start when the value is not a complete callback URL. This applies to `camunda.security.authentication.oidc.redirect-uri` and, when you configure several providers, to `camunda.security.authentication.providers.oidc.<provider-id>.redirect-uri`.
...
The default value `{baseUrl}/sso-callback` is accepted. Helm deployments that configure `orchestration.security.authentication.oidc.redirectUrl` are also unaffected, because the chart appends `/sso-callback` to that value when it renders the application property.

**Action:** Check the configured redirect URI before you upgrade. A cluster that only serves API clients never follows its redirect URI, so an incomplete value can go unnoticed on 8.9 and surfaces as a cluster that does not start on 8.10.

For the full description of the redirect URI and its `{baseUrl}` placeholder, see [redirect URI](/self-managed/components/orchestration-cluster/admin/connect-external-identity-provider.md#redirect-uri).
