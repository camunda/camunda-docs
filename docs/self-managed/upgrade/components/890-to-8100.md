...
Additionally, when you upgrade, your data is [migrated](#data-migration) to the [new file structure](/reference/announcements-release-notes/8100/whats-new-in-810.md#new-file-structure-and-requirements).
...
There is no rolling upgrade path from Web Modeler 8.9 to Camunda Hub 8.10. Shut down all Web Modeler instances so the database has no more writers, take a database backup, then start one or more Camunda Hub 8.10 instances. Starting Camunda Hub triggers the database migration; once it completes, Hub resumes serving traffic. See [version upgrade](/self-managed/components/hub/version-upgrade.md) for the underlying principles.

If you deploy with Helm, this procedure is already handled for you via the `camundaHub.upgrade.phase` values — see [migrate Camunda Hub](/self-managed/upgrade/helm/890-to-8100.md#migrate-camunda-hub) for the Helm-specific steps. You do not need to perform the manual steps above.
...
When you upgrade to Camunda 8.10, your data is automatically migrated to the [new organizational structure](/reference/announcements-release-notes/8100/whats-new-in-810.md#new-file-structure-and-requirements).
...
:::note
Because you're upgrading to 8.10, you may not yet be familiar with the new [Camunda Hub terminology](/reference/announcements-release-notes/8100/whats-new-in-810.md#terminology). This sections uses Web Modeler terminology, including projects and process applications, to explain the data migration. However, before using the Camunda Hub UI, you should familiarize yourself with the [new terminology](/reference/announcements-release-notes/8100/whats-new-in-810.md#terminology), including workspaces and projects.
:::
...
| `SPRING_SECURITY_OAUTH2_RESOURCESERVER_JWT_JWK_SET_URI` (or `api.jwtSetUri`) | `camunda.security.authentication.oidc.additional-jwk-set-uris`, so the key set that signs your API tokens is accepted in addition to the one that verifies logins, rather than replacing it                                                                     |
| `CAMUNDA_OPTIMIZE_API_AUDIENCE` (or `api.audience`)                          | `camunda.security.authentication.oidc.audiences`                                                                                                                                                                                                                |
...
- **Either spelling works for the two API keys:** `api.audience` and `api.jwtSetUri` are read from Optimize's own configuration file as well as from their environment variables, and both spellings are deprecated on the same schedule.
- **The API key set is no longer per path:** through 8.9 `api.jwtSetUri` validated tokens on the public API and ingestion endpoints only. Under `camunda.security.*` a key set applies to the whole API surface, so a token signed by that key set is accepted on any `/api/**` endpoint, subject to the audience and issuer checks.
- **A separate identity provider for your API needs no `issuer-uri`:** adding its JWKS URI is not sufficient on its own. If `camunda.security.authentication.oidc.issuer-uri` is set, CSL validates the issuer and rejects a token from any other one, whatever key set can verify its signature.
- **The `api.audience` default is not carried over:** Optimize forwards only an audience you set explicitly, never the built-in `optimize` default. Configure no audience at all and any audience is accepted; configure `CAMUNDA_OPTIMIZE_IDENTITY_AUDIENCE` but not the API audience and a token audienced `optimize` is rejected. Set every audience you need explicitly.
