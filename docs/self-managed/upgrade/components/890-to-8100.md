...
In 8.10, Camunda Hub and the Orchestration Cluster authenticate through the same shared implementation, the Camunda Security Library (CSL), so both accept the same `camunda.security.authentication.oidc.*` configuration.
...
- User, group, role, and permission management is unchanged in 8.10, and is still handled by [Management Identity](/self-managed/components/management-identity/overview.md).
- The translation happens silently, with no log output confirming which settings were translated. If you set more than one of the three 8.9 audience properties, set `camunda.security.authentication.oidc.audiences` explicitly to confirm the merged result rather than rely on the translation.
