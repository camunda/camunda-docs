...
**Action:** Change the API clients that send the static token to [OIDC bearer tokens](/apis-tools/optimize-api/optimize-api-authentication.md) before you upgrade. Then remove `api.accessToken` and `OPTIMIZE_API_ACCESS_TOKEN` from your configuration.
