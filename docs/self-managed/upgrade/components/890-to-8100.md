...
With dynamic cluster management, clusters are created automatically when you define them in your Camunda Hub configuration. However, unlike in Console where clusters were cleaned up on restart, Camunda Hub clusters persist through restarts. Therefore, you need to delete them manually using the delete cluster endpoint:
