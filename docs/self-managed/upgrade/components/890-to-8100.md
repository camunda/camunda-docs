...
Physical Tenants are provisioned through static configuration, so adding one is a configuration change followed by a rolling restart. Dynamic creation at runtime is not available. See [provisioning and lifecycle](/self-managed/concepts/physical-tenants/provisioning-and-lifecycle.md), or [set up two isolated Physical Tenants](/self-managed/concepts/physical-tenants/getting-started.md) for a hands-on walkthrough.
