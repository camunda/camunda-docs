...
Physical Tenants are provisioned through static configuration, so adding one is a configuration change followed by a rolling restart. Dynamic creation at runtime is not available in 8.10. See [provisioning and lifecycle](/self-managed/concepts/physical-tenants/provisioning-and-lifecycle.md).

Two behaviors change once a node serves more than one tenant:
...
<!-- TODO: Document downgrade behavior once confirmed. Open question from camunda-docs#8900: if an operator sets `camunda.physical-tenants.default.*` overrides and then downgrades to 8.9, what happens to those overrides, and is a downgrade supported at all? There is no general downgrade policy in the upgrade docs to point at. Review with Lena Schoenburg or Deepthi Devaki. -->
