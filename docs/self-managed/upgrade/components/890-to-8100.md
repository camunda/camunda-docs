...
Camunda 8.10 validates the OIDC redirect URI at startup. The Orchestration Cluster does not start if the value is not a complete callback URL. This applies to `camunda.security.authentication.oidc.redirect-uri`, and to `camunda.security.authentication.providers.oidc.<provider-id>.redirect-uri`.

The startup check accepts a value that:
...
A value that passes this check is not always a working configuration. The Orchestration Cluster serves the callback at `/sso-callback` only. Use that path. A different path passes the startup check, but the cluster does not serve the callback, and the login does not complete.

| Value                                      | Result                                    |
| ------------------------------------------ | ----------------------------------------- |
| `{baseUrl}/sso-callback`                   | Accepted                                  |
| `https://camunda.example.com/sso-callback` | Accepted                                  |
| `https://camunda.example.com`              | Rejected, the callback path is missing    |
| `/sso-callback`                            | Rejected, the host is missing             |
| `https://camunda.example.com/callback`     | Accepted, but the login does not complete |

These rules are not new. On 8.9, such a value also could not complete a browser login, because the IdP sent the browser to a path the cluster does not serve. 8.10 reports the value at startup, and not at the next login.

The default value `{baseUrl}/sso-callback` is accepted. Helm deployments that set `orchestration.security.authentication.oidc.redirectUrl` are also correct, because the chart adds `/sso-callback` to that value.

**Action:** Check the redirect URI before you upgrade. A cluster that only serves API clients never uses its redirect URI, so an incorrect value can stay unknown on 8.9 and stop the start on 8.10.

For more about the redirect URI and the `{baseUrl}` placeholder, see [redirect URI](/self-managed/components/orchestration-cluster/admin/connect-external-identity-provider.md#redirect-uri).
