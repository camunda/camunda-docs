...
The Orchestration Cluster serves the callback at `/sso-callback` only. Use that path. A different path passes the startup check, but the cluster does not serve the callback, and the login does not complete.
