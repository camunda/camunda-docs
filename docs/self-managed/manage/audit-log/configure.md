...
The audit log is enabled by default, and the storage it requires may result in increased costs. To mitigate these resource costs, only user operations are tracked by default, not [client](/components/zeebe/technical-concepts/architecture.md#clients) operations.
...
See [all configuration options](/self-managed/components/orchestration-cluster/core-settings/configuration/properties.md#camundadataaudit-log) to learn more.
...
With Camunda 8 Self-Managed, you control the [secondary storage retention policy](/self-managed/components/orchestration-cluster/core-settings/configuration/properties.md#data---secondary-storage), which applies to audit log records:
...
See [Configure data retention](/self-managed/manage/data-retention.md) for more information about the Helm configuration.
