...
This page covers troubleshooting common issues, TLS configuration, and post-deployment operations for RDBMS deployments. For configuration reference, see [configure RDBMS in Helm charts](/self-managed/manage/databases/relational-database/rdbms.md). To verify a working setup, see [validate RDBMS connectivity](/self-managed/manage/databases/relational-database/validate-rdbms.md).
...
**Symptom:** Pod fails to connect to the database (connection timeout, connection refused, or DNS resolution failure). Logs may include `UnknownHostException`, socket connection failures, or a message such as `Factory method 'databaseProperties' threw exception... Error occurred when getting DB product name.` The application typically fails fast and does not become `Ready`.
...
**Fix:** Confirm the JDBC URL, hostname, port, and database service name are correct, and that network policies, firewall/security group rules, and routing allow traffic from the cluster to the database.

## Authentication errors

**Symptom:** "Authentication failed" or "Invalid password" in logs, for example `SQLInvalidAuthorizationSpecException: Access denied for user ... (using password: YES)`.
...
**Fix:** Ensure the username, password, and secret key reference are correct in your Helm values, and that the database user has the required privileges.

## JDBC driver not found

**Symptom:** ClassNotFoundException or "No suitable JDBC driver" in logs, for example `Failed to load driver class oracle.jdbc.OracleDriver`. Camunda fails to start.
...
**Symptom:** Liquibase errors, or tables are not created. If `autoDDL` is `false` on an empty database, the exporter fails with missing-table errors such as `SQLSyntaxErrorException: Table '...EXPORTER_POSITION' doesn't exist`. This is expected behavior unless the schema is pre-created.
...
If a pod is terminated while Liquibase is still running, a stale lock row can remain in `databasechangeloglock` and block startup. For symptoms, the lock-wait timeout configuration, and how to safely release the lock, see [Liquibase lock recovery](access-sql-liquibase-scripts.md#liquibase-lock-recovery).
...
**Symptom:** `last_exported_position` remains unchanged after generating workload.

**Diagnosis:**

1. Inspect exporter logs for errors:

```bash
kubectl logs <pod-name> | grep RdbmsExporter
```

2. Confirm the database user has write permissions.

3. Review exporter settings (`flushInterval`, `queueSize`).

**Fix:** Resolve any errors found in the exporter logs, grant the database user write permissions, and adjust exporter settings if needed. Enable `DEBUG` logging for more detail.
...
For a complete post-deployment checklist, see [validate RDBMS connectivity](/self-managed/manage/databases/relational-database/validate-rdbms.md).
