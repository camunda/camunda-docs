...
For a short checklist and troubleshooting steps you can run after configuring the database, see [validate RDBMS connectivity (Helm)](/self-managed/manage/databases/relational-database/validate-rdbms.md).
...
RDBMS supports other configuration options that can be configured in the helm chart `values.yaml` via [extraConfiguration](/self-managed/deploy-to-production/kubernetes/configure/application-configs.md). See [RDBMS options](/self-managed/manage/databases/relational-database/configuration.md).

For Liquibase lock recovery behavior, including the `camunda.data.secondary-storage.rdbms.ddl-lock-wait-timeout` setting (default: `PT15M`) for heavy schema migrations, see [Liquibase lock recovery](access-sql-liquibase-scripts.md#liquibase-lock-recovery).
...
For a complete post-deployment checklist, see [validate RDBMS connectivity (Helm)](/self-managed/manage/databases/relational-database/validate-rdbms.md).
...
For details and examples, see [using AWS Aurora PostgreSQL with Camunda](/self-managed/manage/databases/relational-database/configuration.md#usage-with-aws-aurora-postgresql).
