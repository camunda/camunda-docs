...
| :------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Set up a snapshot repository in the secondary datastore. | <p>Configure a snapshot repository on the datastore itself:</p><p><ul><li>[Elasticsearch snapshot repository](https://www.elastic.co/docs/deploy-manage/tools/snapshot-and-restore/manage-snapshot-repositories)</li><li>[OpenSearch snapshot repository](https://docs.opensearch.org/docs/latest/tuning-your-cluster/availability-and-recovery/snapshots/snapshot-restore/)</li></ul></p><p><small>Note: For Elasticsearch configuration with the Camunda Helm chart on AWS EKS using IRSA, see [configuration example](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/irsa.md#backup-related).</small></p> |
...
:::note
The Camunda 8.8 release introduces breaking changes for [Operate and Tasklist](/self-managed/manage/back-up-and-restore/webapps-backup.md).
:::
...
For details, see the **standalone backup application** for [Elasticsearch](/self-managed/manage/databases/elasticsearch-opensearch/elasticsearch-without-cluster-privileges.md#standalone-backup-application) or [OpenSearch](/self-managed/manage/databases/elasticsearch-opensearch/opensearch-without-cluster-privileges.md#standalone-backup-application).
:::
...
This depends heavily on your setup. The following examples are based on those given in the [Management API](/self-managed/manage/back-up-and-restore/index.md#management-api) section for Kubernetes using either active port-forwarding or an override of the local `curl` command.

As noted in the [Management API](/self-managed/manage/back-up-and-restore/index.md#management-api) section, this API is typically not publicly exposed. Therefore, you will need to access it directly using any means available within your environment.
...
This will continue exporting records, but not delete those records (log compaction) from Zeebe. This makes the backup a hot backup, as covered in the [why you should use backup and restore](/self-managed/manage/back-up-and-restore/index.md#why-you-should-use-backup-and-restore).
...
This step uses the [web applications management backup API](/self-managed/manage/back-up-and-restore/webapps-backup.md).
...
This step uses the [Optimize management backup API](/self-managed/manage/back-up-and-restore/optimize-backup.md).
...
This step uses the [Zeebe management backup API](/self-managed/manage/back-up-and-restore/zeebe-backup-and-restore.md).
...
:::info
You can [restore a Web Modeler data backup](/self-managed/manage/back-up-and-restore/elasticsearch/restore.md#optional-restore-a-web-modeler-data-backup).
:::
...
- [Web Applications](/self-managed/manage/back-up-and-restore/webapps-backup.md#delete-backup-api)
- [Optimize](/self-managed/manage/back-up-and-restore/optimize-backup.md#delete-backup-api)
- [Zeebe](/self-managed/manage/back-up-and-restore/zeebe-backup-and-restore.md#delete-backup-api)
