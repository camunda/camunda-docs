...
:::tip
This procedure is the recovery step of [Cold Recovery](/self-managed/extend/availability-and-disaster-recovery/cold-recovery.md) when restoring into a secondary region after primary-region loss.
:::

## How RDBMS restore works

As described in the [architecture overview](/self-managed/manage/back-up-and-restore/rdbms/backup.md#architecture-overview), backups involve two independent systems: **primary storage backups** (Zeebe's log stream and snapshots in a blob store) and the **secondary storage backup** (the RDBMS).
...
| Backup available   | At least one Zeebe primary storage backup is available in the configured blob store. See [Create a backup](/self-managed/manage/back-up-and-restore/rdbms/backup.md).                         |
| Backup storage     | Zeebe is configured with the same backup storage as outlined in the [prerequisites](/self-managed/manage/back-up-and-restore/rdbms/backup.md#prerequisites).                                  |
...
The number of partitions backed up is also visible via the [backup management API](/self-managed/manage/back-up-and-restore/zeebe-backup-and-restore.md#list-backups-api).
...
Restore Zeebe to a specific point in time using `--to`. The restore app finds the closest backup to the provided timestamp. The configured [checkpoint interval](/self-managed/manage/back-up-and-restore/rdbms/backup.md#checkpoint-interval) determines how fine-grained the restore points are.

Use the [backup state actuator](/self-managed/manage/back-up-and-restore/zeebe-backup-and-restore.md#request-runtime-state) to inspect available backup ranges.
...
See [back up and restore Optimize independently](/self-managed/manage/back-up-and-restore/optimize-backup-and-restore.md#restore-a-backup) for the complete procedure.
