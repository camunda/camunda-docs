...
:::tip
For cross-region recovery using these RDBMS backups, see [Cold Recovery](/self-managed/extend/availability-and-disaster-recovery/cold-recovery.md).
:::
...
2. The system tracks **backup ranges** — unbroken sequences of consecutive backups. These ranges define the time windows available for restore and can be queried via the [backup state actuator](/self-managed/manage/back-up-and-restore/zeebe-backup-and-restore.md#request-runtime-state).
...
The checkpoint interval controls how frequently Zeebe injects marker checkpoints into the log stream. These markers serve as potential restore points — since a cluster can only be restored to a checkpoint that exists on all partitions, more frequent markers enable finer-grained [point-in-time restore](/self-managed/manage/back-up-and-restore/rdbms/restore.md#point-in-time-restore).
...
Use the [backup state actuator](/self-managed/manage/back-up-and-restore/zeebe-backup-and-restore.md#request-runtime-state) to inspect the current ranges for each partition.
...
If you opt out of the scheduled backup approach, you can still request on-demand backups using the [backup management API](/self-managed/manage/back-up-and-restore/zeebe-backup-and-restore.md). The `backupId` is generated automatically by the cluster.
...
After configuring backups, verify that they are working correctly by querying the [backup state actuator](/self-managed/manage/back-up-and-restore/zeebe-backup-and-restore.md#request-runtime-state):
...
See [back up and restore Optimize independently](/self-managed/manage/back-up-and-restore/optimize-backup-and-restore.md) for the complete procedure.
...
See [backup and restore Web Modeler data](/self-managed/manage/back-up-and-restore/modeler-backup-and-restore.md) for more details.
