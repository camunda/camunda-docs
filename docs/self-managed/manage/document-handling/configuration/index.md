...
- [Camunda 8 Run](/self-managed/manage/document-handling/configuration/camunda-8-run.md)
- [Docker Compose](/self-managed/manage/document-handling/configuration/docker.md)
- [Helm](/self-managed/manage/document-handling/configuration/helm.md)
...
- By using **external cloud file bucket storage**, documents can be stored in a secure and scalable way. Buckets are integrated per cluster to ensure proper isolation and environment-specific management. The following file bucket storage options are supported:
  ...
  - [**AWS S3**](https://aws.amazon.com/s3/) — including [S3-compatible object stores](/self-managed/manage/document-handling/configuration/helm.md#s3-compatible-object-storage) such as MinIO, Cloudian, or Garage (configured through the AWS S3 store with a custom endpoint)
...
For the per-tenant configuration model, including the root catalog, `assigned` restriction, field-level overrides, and startup collision examples, see [document store isolation](/self-managed/extend/tenancy/physical-tenants/configuration-reference.md#document-store-isolation).
