...
| `optimize.multitenancy.enabled`                                               | Move to `global.multitenancy.enabled` (not `extraConfiguration`). This is a platform-wide switch that requires Identity with an external database. See [multi-tenancy](/self-managed/extend/tenancy/configure-logical-multi-tenancy.md). |
