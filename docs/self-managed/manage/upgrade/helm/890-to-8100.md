...
| :-------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Confirm `8.9.x` baseline    | Confirm your deployment is running an `8.9.x` version before upgrading to 8.10. If you are upgrading from a version earlier than 8.9, upgrade one minor at a time. See [upgrade Helm chart](/self-managed/manage/upgrade/helm/index.md). For best stability, use the latest available `8.9.x` patch. |
| Switch to the Helm v4 CLI   | Camunda 8.10 (chart 15.x) requires the Helm v4 CLI. See [Move from the Helm v3 CLI to v4](/self-managed/manage/upgrade/helm-v3-to-v4.md).                                                                                                                                                            |
| Confirm upgrade eligibility | Review [Prepare for upgrade](/self-managed/manage/upgrade/prepare-for-upgrade.md) to confirm upgrade eligibility and complete any required pre-upgrade actions for 8.10.                                                                                                                             |
...
| Backups                     | Create and verify backups for your Camunda data. See [Backup and restore](/self-managed/manage/back-up-and-restore/index.md).                                                                                                                                                                        |
...
| `identityKeycloak`, `identityPostgresql`          | Migrate Keycloak and the Management Identity database to externally managed services. See [Migrate from Bitnami charts](/self-managed/manage/upgrade/migration-from-bitnami/index.md). |
...
| `optimize.multitenancy.enabled`                                               | Move to `global.multitenancy.enabled` (not `extraConfiguration`). This is a platform-wide switch that requires Identity with an external database. See [multi-tenancy](/self-managed/extend/tenancy/logical-multi-tenancy.md). |
