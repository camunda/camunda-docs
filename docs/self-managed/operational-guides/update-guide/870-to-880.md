...
::note This only affects records exported with the 8.8 version. During the migration, left over 8.7 records are exported as normal.
::
...
If there is still need for other record types to be exported you can still do so see `includeEnabledRecords` for [Elasticsearch](../../zeebe-deployment/exporters/elasticsearch-exporter.md#configuration) or [OpenSearch](../../zeebe-deployment/exporters/opensearch-exporter.md#configuration) exporter configuration respectively.
