...
### Explicitly enable export

Only record types necessary for Optimize are exported by default. To enable the export of custom list of record types,
the `exporters.<ES/OS exporter name>.args.exportLegacyRecords` property must be set to `true` in the configuration.
For full list of available configuration options check [Elasticsearch exporter](/self-managed/zeebe-deployment/exporters/elasticsearch-exporter.md)
and [OpenSearch exporter](/self-managed/zeebe-deployment/exporters/opensearch-exporter.md) documentation.
