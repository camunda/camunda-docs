...
The setup with more than one isolated Elasticsearch/OpenSearch instances for exported Zeebe data and Operate/Tasklist
data ia not supported anymore. Manual move of data to a single Elasticsearch/OpenSearch cluster must be performed, which
includes Zeebe, Operate and Tasklist indices, index templates, aliases and ILM policies.
