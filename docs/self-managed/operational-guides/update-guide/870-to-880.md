...
The new Camunda Exporter brings the importing and archiving logic of web components (Tasklist and Operate) closer to the distributed platform (Zeebe). This simplifies the installation, enables scalability for the web applications, reduces latency when showing runtime and historical data, and reduces data duplication (resource consumption).

![87-state](./img/87-state-refined.png)

Camunda web applications use importers and archivers to consume, aggregate, and archive historical data provided by the Elasticsearch (ES) or OpenSearch (OS) exporters. The new Camunda Exporter introduces changes in the architecture users should be aware of.

During the migration/update phase, the Operate and Tasklist importer and the Elasticsearch exporter are still required. This allows 8.7 data written to the Zeebe journal, or already in the Zeebe Elasticsearch indices, to be imported into the new harmonized index schema.
...
![harmonized-schema](./img/harmonized-indices-miro-v2.png)

This reduces the need for several indices and to make use of existing ones (Operate indices are used by Tasklist, etc.)

### Notes for updating to 8.8

When updating from Camunda 8.7 to 8.8, the following should be considered regarding the Camunda Exporter and harmonized indices.
...
For example, the following query shows the import latency. If the result is in a range of seconds, it is fine to update.
...
The migration application can be found in the `camunda/bin/migrate` directory.

:::note
When using the Camunda 8 Helm charts, the migration application will automatically deploy with the importers.
:::
...
it is recommended that you first back up the database, and test the migration script in a test environment first
