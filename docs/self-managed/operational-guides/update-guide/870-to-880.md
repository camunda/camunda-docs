...
The OpenAPI specification for the Camunda 8 REST API consolidates former alpha features and removes deprecated objects.
...
### Key attributes in OpenAPI objects
...
In previous releases, entity keys transitioned from `integer (int64)` to `string` types. The API still supported deprecated
...
### New OpenAPI objects for variable search

The API streamlines the variable search functionality, introduced as an alpha feature in 8.6. The OpenAPI objects
