...
With the introduction of the Camunda Exporter, Zeebe no longer exports all record types by default. Meaning that not all `zeebe-record` indices will be populated.

::note This only affects records exported with the 8.8 version by the new CamundaExporter. During the migration, left over 8.7 records are exported as normal.
::
...
If there is still need for other record types to be exported you can still do so by setting the configuration property :

- `ZEEBE_BROKER_EXPORTERS_ELASTICSEARCH_ARGS_ZEEBERECORDSEXPORTENABLED=true` for Elasticsearch
- `ZEEBE_BROKER_EXPORTERS_OPENSEARCH_ARGS_ZEEBERECORDSEXPORTENABLED=true` for OpenSearch
