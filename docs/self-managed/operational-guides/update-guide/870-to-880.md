...
If there is still need for other record types to be exported you can still do so see `exportLegacyRecords` for [Elasticsearch](../../zeebe-deployment/exporters/elasticsearch-exporter.md#configuration) or [OpenSearch](../../zeebe-deployment/exporters/opensearch-exporter.md#configuration) exporter configuration respectively.
