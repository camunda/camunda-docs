...
Zeebe stores its backup to an external storage and must be configured before the cluster is started. Refer to [Zeebe backup configuration](/self-managed/components-upgrade/backup-restore/zeebe-backup-and-restore.md#configuration) for additional information.
...
2. Trigger a backup `x` of the web applications. See how to take a [web application backup](/self-managed/components-upgrade/backup-restore/webapps-backup.md).
3. Trigger a backup `x` of Optimize. See how to take an [Optimize backup](./optimize-backup.md)
4. Wait until the backup `x` of web applications is complete. See how to [monitor a web application backup](/self-managed/components-upgrade/backup-restore/webapps-backup.md).
...
8. Take a backup `x` of Zeebe. See how to take a [Zeebe backup](/self-managed/components-upgrade/backup-restore/zeebe-backup-and-restore.md).
9. Wait until the backup `x` of Zeebe is completed before proceeding. See how to [monitor a Zeebe backup](/self-managed/components-upgrade/backup-restore/zeebe-backup-and-restore.md).
...
5. Restore the state of the [web applications](/self-managed/components-upgrade/backup-restore/webapps-backup.md) and [Optimize](./optimize-backup.md).
6. Restore `zeebe-records*` indices from Elasticsearch snapshot.
7. Delete the data directory of all Zeebe brokers
8. Restore [Zeebe](/self-managed/components-upgrade/backup-restore/zeebe-backup-and-restore.md).
