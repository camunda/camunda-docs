...
When the response is 200 OK, the response body consists of a JSON object representing the backup runtime state.

- `checkpointStates`: Latest checkpoint information per partition
  - `checkpointId`: The checkpoint id
  - `partitionId`: The partition id
  - `checkpointType`: The type of the checkpoint, can either be `MARKER`, `SCHEDULED_BACKUP`, `MANUAL_BACKUP`
  - `checkpointPosition`: Logstream position of the checkpoint record
  - `checkpointTimestamp`: Checkpoint creation timestamp
- `backupStates`: Latest backup information per partition
  - `checkpointId`: The checkpoint id
  - `partitionId`: The partition id
  - `checkpointType`: The type of the checkpoint, can either be `MARKER`, `SCHEDULED_BACKUP`, `MANUAL_BACKUP`
  - `checkpointPosition`: Logstream position of the checkpoint record
  - `checkpointTimestamp`: Checkpoint creation timestamp
  - `firstLogPosition`: First available logstream record in this backup
- `ranges`: List of the active backup ranges available, per partition
  - `partitionId`: The partition id
  - `start`: The first backup of a continuous backup range
    - `checkpointId`: The checkpoint id
    - `checkpointType`: The type of the checkpoint, can either be `MARKER`, `SCHEDULED_BACKUP`, `MANUAL_BACKUP`
    - `checkpointPosition`: Logstream position of the checkpoint record
    - `checkpointTimestamp`: Checkpoint creation timestamp
    - `firstLogPosition`: First available logstream record in this backup
  - `end`: The last backup of a continuous backup range
    - `checkpointId`: The checkpoint id
    - `checkpointType`: Type of the checkpoint, can either be `MARKER`, `SCHEDULED_BACKUP`, `MANUAL_BACKUP`
    - `checkpointPosition`: Logstream position of the checkpoint record
    - `checkpointTimestamp`: Checkpoint creation timestamp
    - `firstLogPosition`: First available logstream record in this backup
