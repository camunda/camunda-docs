...
| `zeebe_process_definitions_draining_count` | The number of process definitions currently draining: deleted, but retained until their running process instances finish. The `partition` label separates partitions. This gauge is authoritative on the partition leader; values reported by follower nodes may be inaccurate. |
