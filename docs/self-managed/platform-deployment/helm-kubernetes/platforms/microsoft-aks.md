...
**Azure Application Gateway Ingress cannot be used as an Ingress for Zeebe/Zeebe-Gateway** because Zeebe requires an Ingress controller that supports `gRPC`. Hence, you should use any other ingress controller that supports `gRPC`, like the [Ingress-NGINX controller](https://github.com/kubernetes/ingress-nginx).

At the time of writing, the Azure Application Gateway Ingress controller doesn't support `gRPC`. For more details, follow the upstream [GitHub issue about gRPC/HTTP2 support](https://github.com/Azure/application-gateway-kubernetes-ingress/issues/1015).
