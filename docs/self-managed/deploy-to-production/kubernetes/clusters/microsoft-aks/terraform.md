...
To try out Camunda 8 or for development purposes, consider signing up for our [SaaS offering](https://camunda.com/platform/). If you already have an AKS cluster, you can skip ahead to the [Helm guide](/self-managed/deploy-to-production/kubernetes/clusters/microsoft-aks/helm.md).
...
For more details on the PostgreSQL deployment with CloudNativePG Operator, see [PostgreSQL deployment in the operator-based infrastructure guide](/self-managed/deploy-to-production/plan/kubernetes-operators.md#postgresql-deployment) for a production-grade setup with automated scaling, upgrades, and built-in security.
:::
