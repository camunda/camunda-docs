...
**Secondary storage options:** This guide includes a managed OpenSearch example path. RDBMS (PostgreSQL, MySQL, MariaDB, or Oracle) is also a supported secondary storage backend for the Orchestration Cluster. If you prefer to use RDBMS instead of OpenSearch, you can configure this during the Helm installation step. See [configure RDBMS in Helm](/self-managed/manage/databases/relational-database/rdbms.md) for details.

To try out Camunda 8 or develop against it, consider signing up for our [SaaS offering](https://camunda.com/platform/), or if you already have an Amazon EKS cluster, consider skipping to the [Helm guide](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/helm.md).
...
This basic cluster setup is required to continue with the Helm setup as described in our [AWS Helm guide](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/helm.md).
...
- Use RDBMS (PostgreSQL, MySQL, MariaDB, or Oracle) as the secondary storage backend for the Orchestration Cluster. See [configure RDBMS in Helm](/self-managed/manage/databases/relational-database/rdbms.md) for details.

:::

:::note Migration to OpenSearch is not supported

Using Amazon OpenSearch Service requires [setting up a new Camunda installation](/self-managed/deploy-to-production/kubernetes/index.md). Migration from previous Camunda versions or Elasticsearch environments is currently not supported. Switching between Elasticsearch and OpenSearch, in either direction, is also not supported.
...
Now that you've exported the necessary values, you can proceed with installing Camunda 8 using Helm charts. Follow the guide [Camunda 8 on Kubernetes](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/helm.md) for detailed instructions on deploying the platform to your Kubernetes cluster.
