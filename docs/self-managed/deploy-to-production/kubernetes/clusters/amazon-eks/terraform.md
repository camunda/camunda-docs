...
import TerraformAwsAuth from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_terraform-aws-auth.md'
import TerraformS3Bucket from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_terraform-s3-bucket.md'
...
Camunda recommends this approach for a robust, sustainable setup. For a quicker trial or proof of concept, or if the Terraform module doesn’t meet your needs, see [Deploy an EKS cluster with eksctl](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/eksctl.md).
...
If Terraform feels complex at first, try the [eksctl guide](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/eksctl.md) for a simpler entry point, which shows the resources created and how they interact with each other.

- To test or develop against Camunda 8, consider signing up for our [SaaS offering](https://camunda.com/platform/).
- If you already have an Amazon EKS cluster, skip ahead to the [Helm setup guide](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/helm.md) to deploy Camunda 8.
...
After configuring and deploying your infrastructure with Terraform, follow these instructions to export key values for use in Helm charts to deploy [Camunda 8 on Kubernetes](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/helm.md).
