...
import VerifyConnectivity from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_verify-connectivity.md';

import IdpPrerequisite from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_idp-prerequisite.md'
import NoDomainIdpChoice from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_no-domain-idp-choice.md'
import WhyNoIdp from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_why-no-idp.md'
import SingleNamespaceDeployment from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_single-namespace-deployment.md'
import NoDomainInfo from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_no-domain-info.md'
import HelmUpgradeNote from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_helm-upgrade-note.md'
...
This guide gets Camunda 8 running on EKS. For deeper production hardening — scaling, reliability, security, observability, and backups — see the [production install guide](/self-managed/deploy-to-production/kubernetes/install/production/index.md).
:::
...
- A Kubernetes cluster; see the [eksctl](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/eksctl.md) or [Terraform](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/terraform.md) guide.
...
In addition to the infrastructure diagram provided in the [Terraform setup guide](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/terraform.md), this section installs Camunda 8 following the architecture described in the [reference architecture](/self-managed/reference-architecture/reference-architecture.md).
...
When using either standard authentication (network based or username and password) or IRSA authentication, specific environment variables must be set with valid values. Follow the guide for either [eksctl](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/eksctl.md#configuration-1) or [Terraform](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/terraform.md#export-values-for-the-helm-chart) to set them correctly.
...
Additionally, obtain these values by following the guide for either [eksctl](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/eksctl.md) or [Terraform](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/terraform.md), as they will be needed in later steps:
...
:::tip
Make sure to have `EXTERNAL_DNS_IRSA_ARN` exported prior by either having followed the [eksctl](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/eksctl.md#policy-for-external-dns) or [Terraform](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/terraform.md#outputs) guide.
:::
...
:::tip
Make sure to have `CERT_MANAGER_IRSA_ARN` exported prior by either having followed the [eksctl](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/eksctl.md#policy-for-cert-manager) or [Terraform](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/terraform.md#outputs) guide.
:::
...
For more configuration options, refer to the [Helm chart documentation](https://artifacthub.io/packages/helm/camunda/camunda-platform#parameters). Additionally, explore our existing resources on the [Camunda 8 Helm chart](/self-managed/deploy-to-production/kubernetes/install/index.md) and [guides](/self-managed/deploy-to-production/kubernetes/configure/index.md).
...
- **Terraform**: See [Configure the database and associated access](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/terraform.md#configure-the-database-and-associated-access) in the Terraform setup guide.
- **eksctl**: See [Create the databases](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/eksctl.md#create-the-databases) in the eksctl guide.
...
The following makes use of the [combined Ingress setup](/self-managed/deploy-to-production/kubernetes/configure/ingress/ingress-setup.md#combined-ingress-setup) by deploying a single Ingress for all HTTP components and a separate Ingress for the gRPC endpoint.
...
The Helm-chart Elasticsearch deployment uses deprecated Bitnami subcharts. Prefer managed Elasticsearch/OpenSearch services for long-term deployments, or deploy [Elastic Cloud on Kubernetes (ECK)](/self-managed/deploy-to-production/plan/kubernetes-operators.md#elasticsearch-deployment) if you need operator-based Elasticsearch with automated scaling, upgrades, and built-in security.
:::
...
Instead of using Bitnami subcharts for internal PostgreSQL, consider using [CloudNativePG operator](/self-managed/deploy-to-production/plan/kubernetes-operators.md#postgresql-deployment) for production-grade PostgreSQL clusters with automated backup, monitoring, and scaling capabilities.
:::
