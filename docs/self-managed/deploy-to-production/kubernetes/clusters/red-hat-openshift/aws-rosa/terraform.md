...
<!-- (!) Note: Please ensure that this guide maintains a consistent structure and presentation style throughout, as with ../amazon-eks/terraform.md. The user should have a similar experience when reading both guides. -->
...
import RosaHcpAuth from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_rosa-hcp-auth.md'
...
If you require managed services for PostgreSQL Aurora or OpenSearch, you can refer to the definitions provided in the [EKS setup with Terraform](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/terraform.md) guide. However, please note that these configurations may need adjustments to fit your specific requirements and have not been tested. This guide uses integrated Helm chart database services in its example path (PostgreSQL and Elasticsearch), but you can choose another supported secondary storage backend for the Orchestration Cluster, including RDBMS (PostgreSQL, MySQL, MariaDB, or Oracle) — see [configure RDBMS in Helm](/self-managed/deployment/helm/configure/database/rdbms.md).

For testing Camunda 8 or developing against it, you might consider signing up for our [SaaS offering](https://camunda.com/platform/). If you already have a Red Hat OpenShift cluster on AWS, you can skip ahead to the [Helm setup guide](/self-managed/deploy-to-production/kubernetes/clusters/red-hat-openshift/self-hosted/redhat-openshift.md).
...
Unlike the [EKS Terraform setup](/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/terraform.md), we currently support only one main variant of this setup:
