...
<!-- (!) Note: Please ensure that this guide maintains a consistent structure and presentation style throughout, as with docs/self-managed/deploy-to-production/kubernetes/clusters/amazon-eks/helm.md. The user should have a similar experience when reading both guides. -->
...
import DeployECKElasticsearch from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_deploy-eck-elasticsearch.md'
import SecondaryStorageOptionsNote from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_secondary-storage-options-note.md'
...
Infrastructure components are deployed using **official Kubernetes operators** as described in [Deploy infrastructure with Kubernetes operators](/self-managed/deploy-to-production/plan/kubernetes-operators.md):
...
For guidance on installing an Ingress controller, you can refer to the [Ingress Setup documentation](/self-managed/deploy-to-production/kubernetes/configure/ingress/ingress-setup.md).
...
To use permissive SCCs, simply install the charts as they are. Follow the [general Helm deployment guide](/self-managed/deploy-to-production/kubernetes/install/production/index.md).
...
Some components are not enabled by default in this deployment. For more information on how to configure and enable these components, refer to [configuring Enterprise components and connectors](/self-managed/deploy-to-production/kubernetes/install/production/index.md).

### Deploy prerequisite services

Before deploying Camunda, you need to deploy the infrastructure services it depends on. The core infrastructure (Elasticsearch and PostgreSQL) is deployed using Kubernetes operators as described in [Deploy infrastructure with Kubernetes operators](/self-managed/deploy-to-production/plan/kubernetes-operators.md). Keycloak can optionally be deployed as your OIDC provider:
...
For more details on the PostgreSQL deployment, see [PostgreSQL deployment in the operator-based infrastructure guide](/self-managed/deploy-to-production/plan/kubernetes-operators.md#postgresql-deployment).
...
For more details on the Keycloak deployment, see [Keycloak deployment in the operator-based infrastructure guide](/self-managed/deploy-to-production/plan/kubernetes-operators.md#keycloak-deployment).
...
For general deployment pitfalls, visit the [deployment troubleshooting guide](/self-managed/troubleshooting/index.md).
...
For more details, including accepted alternatives to a cluster-wide `Retain` policy, see the [production install guide](/self-managed/deploy-to-production/kubernetes/install/production/index.md#persistent-volume-reclaim-policy).
