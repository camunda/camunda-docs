...
To install Camunda with the default orchestration cluster, see [Install Camunda with Helm](/self-managed/deploy-to-production/kubernetes/install/index.md).

If managed databases or an external OIDC provider are not available in your organization, see [Deploy required dependencies](/self-managed/deploy-to-production/plan/kubernetes-operators.md) to set up PostgreSQL, Elasticsearch, and Keycloak on Kubernetes using official operators.
