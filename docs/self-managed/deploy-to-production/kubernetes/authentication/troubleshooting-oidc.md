...
:::note
Some providers, such as Keycloak, may not include the appropriate audience by default. Consult your provider's documentation on configuring token audiences. For Keycloak, see [External Keycloak](/self-managed/deploy-to-production/kubernetes/authentication/external-keycloak.md).
:::
...
1. Decode a token to see available claims. See [JWT token claims reference](/self-managed/deploy-to-production/kubernetes/authentication/jwt-token-claims.md) for instructions.
...
For a complete list of common claim patterns by provider, see [JWT token claims reference](/self-managed/deploy-to-production/kubernetes/authentication/jwt-token-claims.md#common-claim-patterns-by-provider).
