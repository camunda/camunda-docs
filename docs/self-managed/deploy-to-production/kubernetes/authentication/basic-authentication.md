...
In this section, you'll learn how to configure hybrid authentication with internal Keycloak. You can also apply this approach with other OIDC setups, such as [external Keycloak](/self-managed/deploy-to-production/kubernetes/authentication/external-keycloak.md) or an [external OIDC provider](/self-managed/deploy-to-production/kubernetes/authentication/external-oidc-provider.md)
...
Follow the [internal Keycloak guide](/self-managed/deploy-to-production/kubernetes/authentication/internal-keycloak.md) with these modifications:

1. When you [create a secret](/self-managed/deploy-to-production/kubernetes/authentication/internal-keycloak.md#create-a-secret), omit the following keys. They aren't needed for Basic authentication setups:
...
3. Skip the OIDC sections for the [Orchestration Cluster](/self-managed/deploy-to-production/kubernetes/authentication/internal-keycloak.md#configure-orchestration-cluster) and [Connectors](/self-managed/deploy-to-production/kubernetes/authentication/internal-keycloak.md#configure-connectors).
...
To access the additional components, refer to [Connect to the cluster](/self-managed/deploy-to-production/kubernetes/authentication/internal-keycloak.md#connect-to-the-cluster) in the internal Keycloak guide.

## Next steps

- [Enable centralized OIDC authentication for all components](/self-managed/deploy-to-production/kubernetes/authentication/internal-keycloak.md).
- [Integrate with an external identity provider](/self-managed/deploy-to-production/kubernetes/authentication/external-oidc-provider.md).
