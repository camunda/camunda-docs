...
| Provide centralized secrets               | Create one file per secret in the included `secrets/` directory and reference it with `=camunda.secrets.NAME`. See [use centralized secrets](./connectors-and-modeling.md#use-centralized-secrets).                                                                                                                                   |
| Provide legacy connector secrets          | Add local development secrets to `connector-secrets.txt` for `{{secrets.NAME}}` references. See [use legacy connector secrets](./connectors-and-modeling.md#use-legacy-connector-secrets). Do not put connector credentials in application YAML.                                                                                      |
