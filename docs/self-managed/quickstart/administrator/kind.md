...
import IdentitySecret from '@site/docs/self-managed/deploy-to-production/kubernetes/_partials/_identity-secret.md'
...
This setup is intended for **local development only** and shouldn't be used in production environments. For production deployments, follow our [cloud provider guides](/self-managed/deploy-to-production/kubernetes/clusters/index.md).
:::
...
With this guide, you deploy the full Camunda 8 platform with all components. For lighter setups or specific use cases, see the [Helm installation guide](/self-managed/deploy-to-production/kubernetes/install/index.md), which covers different installation profiles, such as core only, with Connectors, and with Web Modeler.
:::
...
Before deploying Camunda, you need to deploy the external services it depends on. These dependencies are deployed using Kubernetes operators as described in [Deploy infrastructure with Kubernetes operators](/self-managed/deploy-to-production/plan/kubernetes-operators.md):
...
- [Production Helm deployment](/self-managed/deploy-to-production/kubernetes/install/production/index.md): Learn more about Helm chart configuration for production
- [Cloud provider guides](/self-managed/deploy-to-production/kubernetes/clusters/index.md): Deploy to cloud platforms like AWS, Azure, or GCP
