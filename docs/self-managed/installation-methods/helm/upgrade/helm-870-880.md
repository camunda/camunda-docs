...
The Process Migration app upgrades process data storage from the old, separate indices used in Camunda 8.7 to the new unified index introduced in Camunda 8.8.

In 8.7 and previous versions there were two indices:

- `tasklist-process`
- `operate-process`

From 8.8 onwards, there will only be one `operate-process` index for the needs of the entire Camunda Cluster.

:::warning

- The process migration can't be ran on a fresh install of 8.8. The migration will only be successful on upgrade from 8.7.
- The migration can only run once.
- Before upgrading, ensure the 8.7 Zeebe workers have already processed some data. If there is no process data to migrate, the migration job will continue to wait for the importers until the default timeout is reached. Thus, failing the migration.
  :::

To enable the process migration, the following value can be modified:
...
By default, the tasklist, and operate importers are disabled in 8.8. When the above value is set to true, the tasklist and operate importers are eanbled. This is because the migration will wait for the importers to stop processing data. If the importers do not exist then the migration will not complete successfully.

A Kubernetes job will be created to start the migration. The job will continue to run for a prolonged period of time if needed to wait for the importers to finish processing.

To confirm a successful migration the job logs and health status can be monitored. Also, for the process migration specifically, some Prometheus metrics are exposed. The one that might be of interest would be `camunda_migration_processes_migrated`. This should eventually be equal to the number of documents in the `operate-process` index. This would signal that the migration process has completed successfully even if the process has not concluded for other reasons.
