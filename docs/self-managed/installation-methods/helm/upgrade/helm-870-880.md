...
The consolidation of `Zeebe`, `Zeebe Gateway`, `Operate`, and `Tasklist` to `Orchestration Cluster` has three groups:

**Group 1: Renamed keys**

Rename the top-level key `zeebe` to `orchestration`, and keys are used as they are.
...
**Group 3: Combined keys**

The following keys, if used, should be manually combined and they require more attention as they are free-style inputs.
...
To enable the Camunda Helm chart compatibility layer, set `global.compatibility.core.enabled: true` in the Helm chart values file.
