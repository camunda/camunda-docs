...
# Upgrade from Camunda 8.7 to 8.8

This guide describes the steps and requirements for upgrading from **Camunda 8.7** to **Camunda 8.8** by upgrading the Helm chart version from 12.x to 13.+.
...
Upgrading between minor versions of the Camunda Helm chart may require [configuration changes](#update-your-configuration). To upgrade between patch versions or when no configuration changes are required, see the [`helm upgrade`](#identity-disabled) instructions. Review the Camunda 8 Helm chart [version matrix](https://helm.camunda.io/camunda-platform/version-matrix/) to determine the application and Helm chart versions of your installation.
...
# Upgrade Camunda Helm charts from 12.x to 13+
...
- Determine your **Helm chart version** and **Helm CLI version** to the recommended version in our [version matrix](https://helm.camunda.io/camunda-platform/version-matrix/).
- Ensure you have implemented any required configuration changes specific to the version you upgrade to, before running the `helm upgrade` command.
...
You may run the **Identity Migration App** to migrate users, groups, roles, and tenants to the new orchestration cluster's Elasticsearch index, depending on your authentication configuration.
...
- Previously used only available in Operate/Tasklist.
...
- TLS/self-signed certs apply **only** in the orchestration cluster.
...
- Monitor Helm job logs (identity-migration, process-application-migration)
- Confirm migration finished via app logs and readiness
