...
In case you made use of [Resource Authorizations](../../../../../versioned_docs/version-8.7/self-managed/identity/access-management/resource-authorizations.md) with Camunda 8.7,
you need to toggle the following value to `true` to ensure that the migration to **Orchestration Cluster Identity** takes your Resource Authorizations into account:

```yaml
orchestration.migration.identity.resourceAuthorizationsEnabled: true
```

Otherwise the Migration will create wildcard resource authorizations for certain Roles, e.g. for the Operate role, that would grant existing users more access than they
were allowed based on the 8.7 Resource Authorizations.
