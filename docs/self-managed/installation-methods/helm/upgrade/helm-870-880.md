...
The Identity Migration App allows you to migrate data from Management Identity to the Orchestration Cluster Identity.</br>
The migration maps your current authorization setup in Management Identity to the new Authorization setup within the Orchestration Cluster Identity. If you would rather like to setup you authorizations within the cluster from scratch you can op-out of this migration.
The following points should be considered before attemping the migration:</br>

- The app will be ran as a Kubernetes job
- It is available as a standalone migration app for identity only, it will be run separate to the cluster and separate from the process-migration.
- The Identity migration is idempotent, meaning that there are no additional measures needed to prevent duplicate entites if the migration is reran.
- On any unhandled error when fetching data or creating an entity in the Orchestration Cluster, the app will emit logs with clear error details. For example: `“Failed to retrieve groups from <URL>: StatusCode: <code>, Body: <response body>.”`. It will also log a clear message that the migration failed and is to be retried once the error has been addressed. The application will terminate with an exit code.
- There is no tracking of the migration progress - thus if the migration is reran it re-performs all operations.
- The migration does not implement any retry behavior, except on back-pressure from the Orchestration Cluster.

It is possible to enable the migration by modifying the following value:
...
There are two auth scenarios that the Identity Migration App supports:
...
When the Identity migration is enabled through the `values.yaml`, the Managmenet Identity will automatically have a migration client configured through it's environment variables. The migration client will only have read permissions to the Identity Resource Server, and will talk to the zeebe brokers directly in the Orchestration Cluster.

The following entites will be migrated:

1. Roles and their permissions
2. Groups Relationships
3. Their members/users
4. Their role assignment (depends on Roles being migrated)
5. Their Resource Authorizations
6. User role assignment (depends on Roles being migrated)
7. Applications
8. Tenants (depends on Roles)

Once the migration is complete, the job, the configmap for the job, and the migration client in Management Identity can be safely deleted.
