...
- **Basic auth + Orchestration Cluster only** (via `zeebe` StatefulSet) is now the default.
  ...
  - By default, all applications except the Orchestration Cluster and Connectors are **disabled**.
  - Retention **per application** is no longer supported.
- **8.7 importers** remain part of 8.8 for data migration.
  - They run one to migrate the data, then stay idle after finishing.
...
- **Dedicated Elasticsearch/OpenSearch clusters per application** are no longer supported.
...
<!--
TODO: Add a values files from Helm CI to show a full example in tabs.
-->

The following shows an example of Camunda Helm chart values file migrated from 8.7 syntax to 8.8 syntax.
...
The following keys should be moved from global or the old components to `orchestration`.
...
These keys existed separately across components. They must now be combined under `orchestration`. If you use them, pay close attention, as most are free-form inputs.
...
It's highly recommended to migrate to the new values structure using the new key `orchestration`, but it's possible to enable a compatibility layer in the Camunda Helm chart. That means the old structure from Camunda 8.7 (Helm chart 12.x.x) will work with Camunda 8.8 (Helm chart 13.x.x) with several exceptions, documented as breaking changes.
...
In version 8.8, the Operate and Tasklist importers are disabled by default because they are no longer required for new clusters.
When upgrading from `8.7.x`, they are enabled for the importer job only. Once the cluster upgrade is complete, the importer job can be disabled.
...
Depending on your authentication setup, you may need to run the **Identity Migration** to migrate users, groups, roles, and tenants to the new Orchestration Cluster's Elasticsearch index.

Make sure authentication is configured as guided in the following sections:
...
Ensure your `values.yaml` matches the [basic authentication configuration guide](../configure/authentication-and-authorization.md#basic-authentication).

##### 1.3.2 OIDC

Ensure your `values.yaml` is configured according to the [OIDC guide](../configure/authentication-and-authorization.md#external-identity-provider).

##### 1.3.3 OAuth (Keycloak)

Ensure your `values.yaml` matches the [OAuth configuration guide](../configure/authentication-and-authorization.md#internal-keycloak).
...
Take a full backup before starting the upgrade process. Follow the [Backup and Restore guide](#2-perform-a-backup).

### 3. Identity migration

The Identity Migration application allows you to migrate data from **Management Identity** to the **Orchestration Cluster Identity**.  
The migration maps your current authorization setup in Management Identity to the new authorization setup within the Orchestration Cluster Identity. If you prefer to set up your authorizations from scratch within the cluster, you can opt out of this migration (it is disabled by default).

Consider the following points before attempting the migration:

- The application is run as a Kubernetes Job.
- It is available as a standalone migration application for Identity only. It runs separately from the cluster and the process migration.
...
- On any unhandled error while fetching data or creating an entity in the Orchestration Cluster, the application emits logs with clear error details.  
  For example:  
...
This will create a Kubernetes Job and a ConfigMap to configure the job.
...
The Identity Migration application supports two migration scenarios:
...
A Kubernetes Job is created to start the migration. This job can run for an extended period if needed, waiting for the importers to finish processing. No downtime is expected during this period.
...
<!--
TODO: Add more details about how to get this metric.
-->
...
```shell
# ⚠️ Uncomment the next line if Console is enabled.
...
export ORCHESTRATION_SECRET=$(kubectl get secret "camunda-zeebe-identity-secret" -o jsonpath="{.data.zeebe-secret}" | base64 --decode)
...
helm upgrade camunda camunda/camunda-platform \
  # ⚠️ Uncomment the next line if Console is enabled.
  # --set global.identity.auth.console.secret.inlineSecret=$CONSOLE_SECRET \
  --set global.identity.auth.optimize.secret.inlineSecret=$OPTIMIZE_SECRET \
  --set orchestration.security.authentication.oidc.secret.inlineSecret=$ORCHESTRATION_SECRET \
  --set connectors.security.authentication.oidc.secret.inlineSecret=$CONNECTORS_SECRET \
...
<!--
TODO: Add the exact kubectl command to watch the logs.
-->
