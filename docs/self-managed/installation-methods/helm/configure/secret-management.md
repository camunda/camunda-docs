...
### Auto-generated secrets (testing only)

The Helm chart can automatically generate secrets with random passwords for quick testing and development setups. This feature eliminates the need to manually create secrets during initial experimentation.

### Enabling auto-generated secrets
...
**Important**: Enabling `global.secrets.autoGenerated: true` only creates the secret with random values. You must also configure each component to reference the auto-generated secret by name and key.
