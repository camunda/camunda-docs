...
:::note
The following configuration uses the default name `camundaExporter`. To use a custom name, update `CAMUNDAEXPORTER` in the provided environment variables to match the name defined in your exporter [configuration](/self-managed/components/orchestration-cluster/zeebe-deployment/exporters/camunda-exporter.md).
:::
