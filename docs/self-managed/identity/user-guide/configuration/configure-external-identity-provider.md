...
1. Log in to the Keycloak Administrator Console. Open the URL you have configured for Keycloak in your browser.
   ...
   [Docker Compose](/self-managed/installation/deploy/local/docker-compose.md) setup, Keycloak
...
2. Click **Administrator Console** and log in using the Keycloak administrator credentials. The default administrator username is `admin`. When deploying Camunda 8 with [Helm charts](/self-managed/installation/overview.md),
   ...
   [secrets extraction](/self-managed/installation/upgrade.md#secrets-extraction).
   Using the example [Docker Compose](/self-managed/installation/deploy/local/docker-compose.md)
